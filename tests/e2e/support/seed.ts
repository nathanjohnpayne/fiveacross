...
export async function seedEmulatorEvent(
  ...
  // Self-cleaning on failure (CodeRabbit, PR #339): if the wipe or any seed
  // write below throws, the caller never receives the env, so its cleanup()
  // would never run and the leaked emulator client would haunt every later
  // spec in this emulators:exec process. Tear down here on the error path;
  // successful returns keep caller-owned cleanup.
  try {
