...
describe('firestore.rules — Event membership enforcement (#804)', () => {
  ...
  it('gates the #134 archive-lifecycle arms and the media-revocation tombstone', () => {
    // The reconciliation pin for the arms epic #134 added after #804 was
    // written (#1149/#1151/#1153). Each of them is an Event-scoped client write
    // and each therefore carries admission as a conjunct, exactly as its
    // neighbours do. The four dedicated lifecycle arms sit AFTER the general
    // Event arm and are matched by their shared stored-state preamble, so this
    // counts the admitted Admin predicate on them rather than restating each.
    //
    // Note what this does and does not buy today: while Decision D-A's
    // transitional `|| isAdminWithEvent(event)` disjunct is deployed,
    // `isAdmittedAdmin` is behaviourally identical to `isAdmin` — a roster
    // Admin is admitted by the roster itself. These conversions are what makes
    // the arms correct on the day D-A comes out, which is why they are pinned
    // at the source rather than only by behaviour.
    const lifecycleArms = RULES_SOURCE.split(
      '        && isAdmittedAdmin(eventId)\n',
    ).length - 1;
    if (lifecycleArms !== 4) {
      throw new Error(
        `expected the four #134 Event lifecycle arms to require an admitted Admin; found ${lifecycleArms}`,
      );
    }
    for (const clause of [
      // the Event document delete, still refused on both halves of the freeze
      '      allow delete: if isAdmittedAdmin(eventId)\n' +
        "        && !(resource.data.get('status', 'active') == 'archived'",
      // the media-revocation tombstone create (#1153)
      '        allow create: if admitted(eventId)\n' +
        '          && (isAdmin(eventId)\n' +
        '              || (isOwner(request.resource.data.uid) && eventOpenForPlay(eventId)))',
      // the Claims terminal-to-pending arm (#1151), whose getAfter projection
      // names the same Event document the admitted Admin check already read
      '        allow update: if isAdmittedAdmin(eventId)\n' +
        "          && (request.resource.data.get('status', '') != 'pending'",
      // the Proof read arm answering a missing document as not-found (#1157)
      '        allow read: if admitted(eventId)\n' +
        "          && (resource == null || isAdmin(eventId) || resource.data.status == 'active');",
    ]) {
      if (!RULES_SOURCE.includes(clause)) {
        throw new Error(
          `a #134 arm lost its admission conjunct:\n${clause}`,
        );
      }
    }
    // The server-only blocks stay server-only rather than gaining a client arm.
    for (const denied of [
      '      match /proofScans/{proofId} {\n        allow read, write: if false;\n      }',
      '        allow read, update, delete: if false;',
    ]) {
      if (!RULES_SOURCE.includes(denied)) {
        throw new Error(`a server-only block changed shape:\n${denied}`);
      }
    }
  });

  it('gates the media-revocation takedown on admission, in both cohort directions', async () => {
    // The one arm epic #134 added whose gate is observable while D-A is still
    // deployed: its owner branch is a NON-Admin client write. A revoked member
    // owning the Proof, and a foreign-Event member, are both refused the
    // takedown an admitted owner may take.
    const proofId = 'revocable-proof';
    // The rule binds the row to the Proof's OWN stored `storagePath`, and that
    // path embeds the Proof's document id — so the fixture and the takedown
    // both spell it from `${proofId}-${uid}`.
    const storagePath = (uid: string) =>
      `proofs/${EVENT_A}/${uid}/${proofId}-${uid}.jpg`;
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      const database = ctx.firestore();
      for (const uid of [MEMBER_A, REVOKED, MEMBER_B]) {
        await setDoc(
          doc(database, `${eventPath(EVENT_A)}/proofs/${proofId}-${uid}`),
          {
            ...proofData(uid),
            type: 'photo',
            storagePath: storagePath(uid),
            mediaURL: `https://example.invalid/${proofId}-${uid}.jpg`,
          },
        );
      }
    });

    const takedown = (database: Firestore, uid: string) => {
      const batch = writeBatch(database);
      batch.set(
        doc(
          database,
          `${eventPath(EVENT_A)}/proofStorageDeletes/${proofId}-${uid}`,
        ),
        { storagePath: storagePath(uid), uid, requestedAt: NOW },
      );
      batch.delete(
        doc(database, `${eventPath(EVENT_A)}/proofs/${proofId}-${uid}`),
      );
    ...
    await assertFails(takedown(authed(REVOKED), REVOKED));
    await assertFails(takedown(authed(MEMBER_B), MEMBER_B));
    await assertSucceeds(takedown(authed(MEMBER_A), MEMBER_A));
  });
