...
import { formatDateRange, eventTitle, segmentEmojiRuns } from './format';

describe('formatDateRange', () => {
  it('collapses a shared month and year to a day range with an en dash', () => {
    // The med-2026 event window.
    expect(formatDateRange('2026-07-15', '2026-07-24')).toBe('July 15–24, 2026');
  });

  it('renders a single-day window as a plain date, not a duplicate range', () => {
    expect(formatDateRange('2026-07-15', '2026-07-15')).toBe('July 15, 2026');
  });

  it('spans two months in the same year', () => {
    expect(formatDateRange('2026-07-28', '2026-08-03')).toBe('July 28 – August 3, 2026');
  });
  ...
  it('does not shift the day across timezones (parses ISO parts directly)', () => {
    ...
    expect(formatDateRange('2026-07-15', '2026-07-15')).toContain('15');
  });

  it('degrades to an empty range for missing or malformed dates (never throws)', () => {
    expect(formatDateRange('', '')).toBe('');
    expect(formatDateRange('', '2026-07-24')).toBe('');
    expect(formatDateRange(undefined as unknown as string, undefined as unknown as string)).toBe('');
    expect(formatDateRange('not-a-date', '2026-07-24')).toBe('');
  });

  it('rejects a numeric but out-of-range date rather than rendering a broken title', () => {
    expect(formatDateRange('2026-13-01', '2026-07-24')).toBe(''); // month 13
    expect(formatDateRange('2026-07-32', '2026-07-24')).toBe(''); // day 32
    expect(formatDateRange('2026-07-15', '2026-00-24')).toBe(''); // month 0
  });

  it('rejects an impossible calendar day (day exceeds the month, leap-year aware)', () => {
    expect(formatDateRange('2026-02-31', '2026-07-24')).toBe(''); // Feb 31
    expect(formatDateRange('2026-04-31', '2026-07-24')).toBe(''); // Apr 31
    expect(formatDateRange('2026-02-29', '2026-07-24')).toBe(''); // 2026 is not a leap year
    // A real leap day is accepted.
    expect(formatDateRange('2028-02-29', '2028-02-29')).toBe('February 29, 2028');
  });

  it('drops a reversed window where the end precedes the start', () => {
    expect(formatDateRange('2026-07-24', '2026-07-15')).toBe(''); // swapped same-month
    expect(formatDateRange('2026-08-01', '2026-07-31')).toBe(''); // swapped across months
  });
});
