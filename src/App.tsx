...
function EventApp() {
  ...
  // the record. So the Card tab routes to the standings instead of mounting the
  // Board — a redirect rather than a second copy of the surface, so the archive
  // keeps ONE mount point (`Leaderboard`, which routes an archived Event to the
  // frozen record and a merely CLOSING one to the live standings its quiesce has
  // already stopped) and the tab bar names where the Player actually is.
  //
  // Decided here rather than inside `Board` for the reason the Leaderboard's own
  // split states: a branch taken after the hooks have run still mounts the
  // component and opens its listeners, and `Board`'s mount is what deals a Day
  // Card. Returning a different element from the route table is what keeps it
  // unmounted.
  //
