...
/**
 * The archived Leaderboard (#1152, specs/post-sailing-archive.md): the frozen
 ...
 * subscription here, no `sortPlayers`, no `eventFirstBingoWinner`, and no
 * per-Day-meta read: every number, name and honour was decided once, at the
 * archive, by `buildEventArchive` — down to the LABEL on each honour chip, which
 * the record carries so a Day re-themed after the freeze cannot re-label a frozen
 * honour. That is the whole point of the freeze (ADR 0001 — a social record, not
 * a ledger to re-derive), and it is also what makes "opened later, the standings
 * persist unchanged" true rather than merely likely: nothing here recomputes a
 * stat, a rank or an honour.
 *
 ...
 *
 * The Share Card stays (issue #36's on-device rasteriser, ADR 0005 — no crawler
 * pages): a frozen leaderboard is the most shareable thing the Event ever
 * produces, and it prints from the same frozen rows the page shows.
 *
 * MODERATION IS THE ONE LIVE INPUT, and it is not a re-derivation. The freeze
 * deliberately leaves `bannedUids` editable (`firestore.rules`' write-once clause
 * protects `status`/`archivedAt`/`archivedUnder`/`archive` and nothing else)
 * precisely so a permanent record still has a takedown path (#808). The
 * repository's ban contract is that EVERY public Leaderboard view hides a banned
 * Player (`specs/w2-ban-console.md` § Leaderboard) — the live Leaderboard, the
 * Share Card, and now this — so the CURRENT roster is applied to the STORED rows
 * here.
 *
 * Hidden, never reassigned: a banned Player's row disappears and their honours go
 * blank, and NOTHING is promoted into the vacancy. The record is not recomputed —
 * `playerCount` and every remaining row's numbers keep exactly the values the
 * archive stamped — because a ban is a moderation decision about who is shown,
 * never a re-ruling of who won.
 */
export default function ArchivedLeaderboard({
  ...
  // Event field this surface reads — `days` never entered the Pick because the
  // honour chips render their own frozen label, and `name` did not because the
  // Share Card reads its title out of the record (#1151).
  ...
  // `dayLabel` can be absent at runtime whatever the contract declares. It stays
  // frozen-safe — derived from the honour's own index — rather than reaching back
  // into the live schedule the way the bug did.
  const dayChipLabel = (honor: ArchivedDayHonor): string =>
  ...
  //
  // THE PINNED ELEVENTH ROW FALLS BACK TO `firstBingoRow` (#1151, Codex P2 on PR
  // #1139). `standings` is a bounded prefix cut by RANK while the headline honour
  // is decided by who bingoed EARLIEST, so past `MAX_ARCHIVED_STANDING_ROWS` the
  ...
  // Event large enough to have truncated. The serializer keeps their row and their
  // true rank beside the honour for this; the rank is the one the COMPLETE
  // standings held at the freeze, which the retained prefix cannot recompute (and
  ...
  // THE EVENT NAME COMES OUT OF THE RECORD (#1151, Codex P2 on PR #1139), for the
  // reason the honour chip labels do. It used to be read off the LIVE
  // `EventDoc.name`, which the freeze deliberately leaves editable, so an Admin
  // renaming the Event afterwards re-titled a frozen card, and two people sharing
  // the same standings a week apart got two different images of them.
  //
  // The app's own name is still the fallback, exactly as it was for an Event with
  // no name: a record that carries none is one written by hand, and the fallback
  // stays frozen-safe rather than reaching back into the live document.
  const shareEventName = archive.eventName ?? shareCardAppName();
  const shareContextLine = archive.eventName ? `${archive.eventName} · Final standings` : undefined;
   ...
   * card shows re-renders rather than sharing a card with the wrong Player on it.
   * `.catch(() => null)` lives inside the cached promise (the Leaderboard's own
   * rationale): a render failure resolves null, `shareCardBlob` degrades to the
   * text/URL leg, and an unconsummated hover can never surface as an unhandled
   * rejection.
   */
  ...
  // the obvious one: the archive is IMMUTABLE. `EventDoc.archive` is write-once at
  // the rules boundary, so unlike the live Leaderboard (which re-renders on every
  // roster snapshot, and refused mount-eager rasterization for exactly that
  // reason) there is no per-snapshot churn to pay for. Pre-rendering it cannot
  // bake in anything a later snapshot would change, and it is what lets the
  // no-wait tap below still carry the image on the common cold mobile tap, where
  // `onPointerDown` gives a render only the length of the press.
  //
  ...
  const shareLeaderboard = async () => {
    ...
    // NOTHING is awaited before `shareCardBlob`. Start (or reuse) the render, then
    // take its blob ONLY if it has already settled: an unsettled render costs the
    // image, never the share. `shareCardBlob` is therefore invoked in the same turn
    // as the tap, so `navigator.share` runs while the transient activation is
    // unambiguously alive. The cached promise keeps rasterizing either way, so a
    ...
    try {
      await shareCardBlob({
        ...
        // Entry-point origin (#607, amended multi-domain policy #599), exactly as
        // the live Leaderboard's card carries it: every serving host brands
        // dynamically, so a rewritten link would land recipients under another
        // Edition's brand.
        url: shareOrigin(),
      });
    } catch {
  ...
  return (
    <>
      ...
      <div className="lb-honors" aria-label="Hall of fame">
        ...
        <div className="row">
          <div className="grow">
            <div className="name">{`⭐ ${editionLexicon().occasionWide} First to BINGO`}</div>
            <div className="sub">
              {headline ? `${headline.displayName} · ${when(headline.at)}` : 'No one got there.'}
            </div>
          </div>
        </div>
      ...
      <div className="lb-actions">
        {/* Warm-on-intent as well as mount-eager (the live Leaderboard's three
            handlers): the eager render covers the cold tap, and these cover a key
            change — a ban or unban — that invalidated it. */}
        <button
          type="button"
          className="btn"
