 ...
 * closing Event rather than a risk. And there is nothing to mirror: the archive
 * froze every standings row's `displayName`, the archived Leaderboard renders
 * that record rather than these rows, and the record stores no photo URL at all
 * (§ Design decisions). The rules are deliberately NOT loosened to admit an
 * identity-only update — the exemption would have to be carried by the one arm
 * the standings are frozen at, to refresh fields no archived surface reads.
 *
