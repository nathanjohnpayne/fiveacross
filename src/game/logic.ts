...
function mixedPicks(
  ...
  const easyShuffled = shuffle(easyPool, rnd);
  const easyUsed = Math.min(easyCount, easyShuffled.length);
  const easyPicks = easyShuffled.slice(0, easyUsed);
  ...
  // (those past the `easyUsed` the easy half already claimed, so no square repeats).
...
export function dealBoard(
  ...
  if (opts.stratify === false) {
    ...
    const drawable = easyCount > 0 ? mainUsable.length + easyItems.length : mainUsable.length;
