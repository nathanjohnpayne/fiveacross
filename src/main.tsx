...
if (atCentralAuthOrigin) {
  ...
  void bootstrapEventResolution()
    .then(async (resolution) => {
      ...
      // NOTHING TO DRAIN HERE ANY MORE (#1153). Child 1 booted a device-local
      // `localStorage` retry queue from this point, because `deleteProof`
      // commits before it revokes the media and a Storage failure left an
      // orphaned blob only this device knew about. The record is now the
      // server-side tombstone `deleteProof` writes inside the commit itself,
      // and `revokeDeletedProofMedia` finishes it — so the retry no longer
      // depends on this device, this tab, or this Player ever coming back.
