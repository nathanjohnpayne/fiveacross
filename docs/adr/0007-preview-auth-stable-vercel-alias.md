...
- **One preview at a time.** `preview` holds one branch. Two lanes wanting a device check must take turns. A second slot is possible—nothing about the design is single-slot and the branch-name budget has room—but it is not console-only: `preview2` would need both registrations *and* a code change adding its hostname to `FIRST_PARTY_AUTH_HOSTS`, merged and deployed, because that list matches exactly and no pattern may be substituted for it.
