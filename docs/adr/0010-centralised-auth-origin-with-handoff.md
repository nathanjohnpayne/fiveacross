...
> **Amended 2026-08-05 (#599):** the canonical Five Across domain is now `fiveacross.app`, so the central auth origin this ADR plans is `auth.fiveacross.app`, not `auth.fiveacrossbingo.com` (#547 tracks its provisioning). The body text below is left as written; read `auth.fiveacrossbingo.com` (and `<slug>.vacaybingo.com` in examples) as the historical names—the mechanism is unchanged.
