...
>
> The interim path, for Events provisioned before the handoff lands, is to register each Event's hostname by hand—one Firebase authorized domain, one OAuth redirect URI—and leave `VITE_AUTH_MODE=same_origin`. That is what makes the same-origin baseline in the consequences below a tested route rather than a hypothetical one. It expires at the first Event nobody pre-registered, which is the threshold the last consequence already names.
