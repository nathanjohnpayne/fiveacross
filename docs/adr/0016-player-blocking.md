...
**Direction disclosure is inherent.** Reciprocal client-side hiding requires the blocked Player's client to receive the pair record naming the blocker, so a blocked Player can enumerate who blocked them; an unblocker whose pair is confirmed still standing after their own direction left can infer the block was mutual (`stillHidden: true` alone is not proof: an orphaned pair or an unanswered follow-up listing reports the same). Accepted, and the copy is worded so it does not overclaim.
