...
- A compromised publisher runtime can use its intended KMS authority to sign bad contiguous revisions. Drift alerts and a separately authenticated human recovery path bound to a different identity/key are therefore part of the control plane, not optional operations polish. Recovery also requires a second, human-only source-attestor identity/key to sign the exact fresh Firestore source/ledger observation and bounded nonsampled Function/IAM/KMS readback, so recovery authority alone cannot invent canonical state or claim an inaccessible replacement publisher. Integrity recovery raises the per-host publisher epoch only after that evidence proves a replacement subject/key unavailable to the quarantined runtime, so its pre-signed higher revisions cannot replay after repair.
