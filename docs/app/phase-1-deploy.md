...
Deploys `moderateProof` (Storage trigger → SafeSearch flag + thumbnail) plus the admin-notification family: the three document triggers `notifyProofModeration`, `notifyItemModeration` (`onDocumentWritten` on `events/{eventId}/proofs/{proofId}` and `.../items/{itemId}`) and `notifyAbuseBugReport` (`onDocumentWritten` on the TOP-LEVEL `bugReports/{reportId}`, #670), and the scheduled `adminAlertDigest`. **Since #638 the document triggers no longer send mail**—they only APPEND to the server-owned `events/{eventId}/adminAlerts` queue, and `adminAlertDigest` (`*/5 * * * *` UTC) drains it into one Theme-styled email per Event per sweep. So the secret binding and the Cloud Scheduler job both belong to the DIGEST, and a delivery smoke-test must exercise the digest rather than the triggers (§ 1a-ii). Player stats are **not** server-recomputed—they stay client-authoritative by design (ADR 0001).
...
`adminAlerts` covers two different things now. A drained row is replaced by a payload-free tombstone (`{ sentAt, expiresAt }`) whose id keeps a delayed trigger redelivery from mailing the same transition twice, seven days out to match the redelivery window — that half is housekeeping, two numbers holding no user content. But since #670 an UNDRAINED row also carries an `expiresAt` (30 days), and that half is a retention control: a queue row holds a copy of user content — a pending Prompt's words, a hidden Proof's text, an abuse reporter's description — and the sweep only visits `active` Events, so a row whose Event is archived is never looked at again. Without the policy that copy outlives the source it describes and the retention sweep that deletes it.

`adminAlertBatches` is the sharper of the two: that document holds the FULLY RENDERED email — every pending and hidden item in the batch at once — and it persists for as long as delivery keeps failing. It expires **30 days after being frozen**, the same span as a pending row: it has to outlive every row it claims, or TTL could delete an in-flight freeze while its claimed rows survive and the next sweep would re-send a digest that already went out. Without this second policy the field is inert and the rendered copy stays forever.

**Enabling the policies is only half of an UPGRADE.** Firestore TTL only ever looks at documents that already carry a timestamp in `expiresAt`, and it does not reap one that lacks the field — so every row and frozen batch written *before* this deploy is invisible to the policy you just enabled. That legacy set is exactly the population this is for: pending rows under archived Events, which the active-Event sweep will never revisit, and frozen batches whose delivery kept failing. Leaving them is the worst outcome, because the policy is now configured and reaping nothing.
