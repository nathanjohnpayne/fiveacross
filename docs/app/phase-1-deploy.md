...
2. **Disable the Cloud Run invoker IAM check on `emailUnsubscribe`.** `gaycruisebingo`'s GCP project enforces Domain Restricted Sharing (`constraints/iam.allowedPolicyMemberDomains`), which rejects granting the function `allUsers` invoker access directly—and the `/unsubscribe` Hosting rewrite (`firebase.json`, ordered before the SPA catch-all `**` rewrite so it isn't swallowed) does NOT get around that: it forwards the unauthenticated request into the same gated Cloud Run service. This is the identical constraint `submitBugReport` hits (#158); run the same fix, targeted at this service:
   ...
   Re-run this after every `--only functions` deploy—a redeploy can reset it, exactly like the bug-report case. See `docs/app/bug-reports.md` § Repeat-deploy hardening for the full mechanism; both scripts are thin wrappers over the shared `scripts/set-cloud-run-invoker.sh`.
3. **The unsubscribe endpoint answers**—`curl -sI "$EMAIL_UNSUBSCRIBE_URL?e=x&u=y&t=z"` should return `200` with an HTML confirmation page (a GET never changes state; only a POST does). A `403` means step 2 above hasn't run yet (or was reset by a later redeploy)—that invoker check is the only thing gating this endpoint once the `/unsubscribe` rewrite and `EMAIL_UNSUBSCRIBE_URL` are in place (the structural test in `src/recon-share-og.test.ts` covers those two).
