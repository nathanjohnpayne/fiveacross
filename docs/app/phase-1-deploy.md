...
- `EMAIL_FROM`—default `Five Across <hello@fiveacross.app>`. Reached by an **editionless** Event, not merely an unknown one: `resolveEventOrigin` returns `edition: null` for an Event with no active `hostnames` mapping, and `fromAddressFor` cannot resolve a register for `null`, so the send lands on this param even when all three `EMAIL_FROM_*` overrides are set. The default is therefore the occasion-neutral platform identity rather than any one Edition's ([#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102)); the prior value branded such an Event `Gay Cruise Bingo`, asserting a specific Edition over an Event that had not claimed one. Set it explicitly per project to the same value.

  > ⚠️ **`EMAIL_FROM` MUST sit on a domain verified in Resend.** The Resend account verifies two sending domains: **`fiveacross.app`** ([#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102) — sending enabled, receiving disabled, no tracking), which is what the default above uses, and **`mail.nathanpayne.com`** (#633), which the pre-#1102 default used. Resend rejects a send from an unverified domain, and `sendEmail` swallows that into a logged `false` (ADR 0001's never-throw contract)—so an override that lands on an unverified domain fails the same way: quietly, with the moderation notifiers and the daily email both logging and delivering nothing. Confirm any per-project override in `functions/.env.<projectId>` is on one of those two before deploying it, and **do not treat this param as vestigial**: as the bullet above explains, an editionless Event reaches it even when all three Edition overrides are set, so a stale project-local value here still ships on real mail.
- `EMAIL_FROM_GCB` / `EMAIL_FROM_VACAY` / `EMAIL_FROM_FIVEACROSS`—Edition-aware overrides of `EMAIL_FROM` (#671): both `dailyEngagementEmail` and `adminAlertDigest` resolve the Event's Edition (gcb / vacay / fiveacross) from its host and use the matching one of these, falling back to `EMAIL_FROM` when it is unset. **One domain serves every Edition** ([#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102), owner decision 2026-09-03): `fiveacross.app` is the verified sending domain for all three, with the display name carrying the Edition and the domain carrying the platform. Set all three, on both projects, to `Gay Cruise Bingo <hello@fiveacross.app>` / `Vacay Bingo <hello@fiveacross.app>` / `Five Across <hello@fiveacross.app>`. `gaycruisebingo.com` and `vacaybingo.com` are deliberately not verified and are not planned to be. § 1a-iii's deploy-time param-coverage guard (#767) requires every declared param to have SOME line in `functions/.env.<projectId>`—an omitted line, not a blank value, is what it rejects—so **all three keys must be present in both projects' env files before their next Functions deploy**, or that deploy fails at the coverage-guard step. Present AND assigned: a blank `EMAIL_FROM_GCB=` satisfies the coverage guard but leaves `fromAddressFor` without an override, so the send falls back to `EMAIL_FROM` and the unified sender never takes effect. Blank lines were the correct pre-verification state and are no longer. Run `npm run verify:functions-env:gaycruisebingo` / `:fiveacross` ahead of time to confirm.
...
- `EMAIL_REPLY_TO`—default empty, meaning **no `Reply-To` header at all** (unchanged behaviour for any project that does not set it). Applies to every transactional send, not just the daily email. **Leave it empty on every project** ([#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102)): now that `hello@fiveacross.app` is both the `From:` for every Edition and an address that receives, a reply goes to the `From:` by default and this param has no work left to do. A value here is worse than redundant—it is project-wide while [ADR 0008](../adr/0008-five-across-second-firebase-project.md) splits projects by **cohort**, so one value is necessarily wrong for some Edition the project serves. The `fiveacross` project hosts the `vacay`-Edition Bodega Bay Event, so the former `nathanpayne.com` value had Vacay-branded mail inviting replies to a personal domain.
...
`adminAlertDigest` (an `onSchedule` trigger, `*/5 * * * *` UTC) and `settleAdminAlertsOnArchive` ship with the same `RESEND_API_KEY` secret above; no additional secret is needed. The digest schedule also owns `runAbuseEscalationSweep`, which needs the same Admin-SDK identity to read the retry task's current Event and Player relationship and to atomically create an authorized queue row. Both functions pin that identity, as do the two `notify*Moderation` triggers now that they write the queue. Full behaviour is in `specs/admin-notification-emails.md` and `specs/w4-bug-report-inbox.md`.
...
**This is now automatic.** `scripts/deploy.sh` reconciles the selected subset of `submitBugReport` and `emailUnsubscribe` as Step 2.5—right after `op-firebase-deploy`, before the Cloudflare purge and the post-deploy synthetic. A full Functions deploy selects both; a scoped deploy for one of those exact endpoint names selects only that endpoint, so a first `--only functions:emailUnsubscribe` deploy never fails on the not-yet-created `submitBugReport` service (and vice versa). Firebase also permits `functions:<selector>` to name a codebase or function group, not only one export; unfamiliar selectors therefore conservatively check both endpoints rather than risking an endpoint that was included in the group remaining 403. If that selector was actually an unrelated Function, either protected service may legitimately remain absent after deploy, so the conservative post-deploy checks tolerate only that missing-service result; exact endpoint scopes remain strict after publish. The underlying scripts are idempotent and no-op when the check is already disabled, so a routine `npm run deploy:gaycruisebingo` needs no separate manual step. It used to need one, and relying on someone to remember it is exactly how this broke twice, at different times, for each endpoint (#158, #768). Skip it with `scripts/deploy.sh --skip-invoker` (or `npm run deploy:gaycruisebingo -- --skip-invoker --`) if you need to. Five Across is deliberately stricter because its checked-in client already names the central handoff: `skipInvokerReconcile` is `false` there (#547, completed 2026-09-10, after `firebase-deployer@fiveacross.iam.gserviceaccount.com` was confirmed to hold `run.services.update` and `AUTH_HANDOFF_PROJECT=fiveacross scripts/apply-auth-handoff-deploy-readiness.sh` forced a successful update on both callables), and the target refuses `--skip-invoker`. The named target pins its repository root and passes a readiness marker into `deploy.sh`; the single Firebase option/scope parser runs the pinned firebase-tools grammar and repository Hosting-config validators before the remaining nonmutating guards, then the exact-SA readiness wrapper before any build or publish when Hosting or a handoff callable may be released. Unknown Hosting sites and other invalid Firebase filters fail locally before readiness, while a valid exclusion for this repository's only Hosting site removes that surface from the readiness decision. Firebase `--dry-run` skips this wrapper's readiness/post-deploy IAM updates and publishes no app or Function release after those same guards, though Firebase may still enable project APIs. Rules/storage and exact known-unrelated Function scopes do not trigger the live update, and Five Across refuses `--skip-invoker` so a Functions release cannot suppress its post-deploy handoff repair. Automatic pre/post reconciliation also clears readiness-only impersonation controls before selecting its own credential, so an ambient operator export cannot redirect those repairs.
...
The trigger is deliberately the deploy's *scope*, not its error text. One Node adapter owns the raw Firebase argv parse, pinned target/filter validation, Hosting config/site selection, and the complete invoker/readiness classification; `scripts/deploy.sh` consumes that result instead of maintaining a second shell parser. Named targets reject every `-P`/`--project` and `-c`/`--config` spelling before readiness, build, or publish. What resets the invoker annotation is a **successful** Functions release, which can complete long before the overall command exits: a multi-surface deploy that releases Functions and then fails on Hosting, or a Functions deploy where `emailUnsubscribe` updates before a different function errors, both end nonzero without the org-policy string anywhere in the output. Keying the recovery on that string therefore skipped it in precisely those states, leaving both endpoints 403ing because something unrelated also failed. So `scripts/deploy.sh` reads `--only` / `--except` off the deploy arguments instead: if Functions could have been released, it reconciles, whatever happened afterwards. An unfamiliar `functions:<selector>` in `--only` conservatively checks both endpoints. Firebase gives any `--only` precedence and ignores `--except` when both are present. Without an allowlist, only exact top-level `--except functions` removes Functions; endpoint- and codebase-qualified exclusions are no-ops under Firebase's vendored target filter and therefore keep both endpoints selected. Inside `scripts/deploy.sh`, a `--only hosting` deploy skips the ordinary invoker steps because it cannot reset the annotation. The handoff-enabled Five Across target adds stronger pre-build readiness in that same canonical process: a hosting scope forces both handoff updates before the client build, a handoff Function scope does the same before release, and scopes that can affect neither skip the update.

**Its credential is the deploy's own service-account identity, and it is checked before anything is published.** The reconciliation shells out to `gcloud`, which resolves its **own** credential chain. For ordinary `deploy.sh` reconciliation, an absent `GOOGLE_APPLICATION_CREDENTIALS` makes the wrapper materialize the same project Firebase-vault SA key that `op-firebase-deploy` will use and hold it only for the deploy; the invoker helper activates it in an isolated gcloud config. A deploy preflight still avoids that extra vault read and prepares every deploy secret in one burst, but it is no longer required for the ordinary named deploy path. Step 1.6 runs each selected invoker script read-only (`--dry-run`) before `op-firebase-deploy`, and aborts with `NOTHING HAS BEEN PUBLISHED` if its credential or service check fails. That read-only check proves describe access, not update permission. The stronger Five Across readiness wrapper accepts a direct credential only when its `client_email` is exactly `firebase-deployer@fiveacross.iam.gserviceaccount.com`; any other usable source is repinned through `--impersonate-service-account` to that exact identity only after the Firebase vault explicitly reports the project key absent, preserving the project's keyless path. Authentication, biometric, and infrastructure lookup failures abort before gcloud and never fall through to ambient impersonation. An accepted direct key must also activate successfully—readiness forbids the ordinary helper's ambient-ADC fallback—so effective identity cannot drift after the JSON check. It then passes `--prove-update`, which forces the idempotent update on both callables even when the annotation is already true. A missing/wrong/unusable credential, impersonation failure, permission failure, or either callable failure aborts inside `deploy.sh` after the canonical source guard and before `BUILD_CMD` or Firebase, and wrapper-owned credential files are trap-cleaned. `--skip-invoker` remains available for ordinary Gay Cruise Bingo break-glass use but is rejected by the handoff-enabled Five Across named target, whose post-Functions repair is non-optional.
