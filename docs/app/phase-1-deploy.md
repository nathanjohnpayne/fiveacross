...
**This is now automatic.** `scripts/deploy.sh` reconciles both `submitBugReport` and `emailUnsubscribe` as Step 2.5 of every deploy—right after `op-firebase-deploy`, before the Cloudflare purge and the post-deploy synthetic. It runs unconditionally (both underlying scripts are idempotent and no-op when the check is already disabled), so a routine `npm run deploy:gaycruisebingo` needs no separate manual step for this anymore. It used to need one, and relying on someone to remember it is exactly how this broke twice, at different times, for each endpoint (#158, #768). Skip it with `scripts/deploy.sh --skip-invoker` (or `npm run deploy:gaycruisebingo -- --skip-invoker --`) if you need to. The `fiveacross` target skips it automatically (`skipInvokerReconcile: true` in `scripts/build-target.mjs`)—that project's deploy credential is not provisioned with IAM access to a gaycruisebingo Cloud Run service, so running this unconditionally there would fail on a permissions error rather than a no-op; see the comment on that target for the manual override if fiveacross ever needs the same fix.

**It runs even when Firebase calls the deploy a failure.** This is the part that makes the automation worth having. When a Functions deploy re-tries the rejected `allUsers` binding, `firebase deploy` reports the whole run as FAILED (`Unable to set the invoker for the IAM policy on the following functions:`) even though the function is published and serving—so a `set -e` script would abort at the deploy and never reach the reconciliation, skipping the recovery in exactly the failure mode it exists for. `scripts/deploy.sh` therefore captures the deploy's exit status instead of aborting on it, classifies that specific partial failure, reconciles, and only then honours the status. The deploy **still fails** afterwards (a nonzero `firebase deploy` means something missed its intended state, and the Cloudflare purge and synthetic are skipped as before)—but the endpoints are no longer left 403ing while you debug, and a re-run is a cheap idempotent way to finish. A deploy that failed for any *other* reason skips the reconciliation and fails closed exactly as it did before.

**Its credential is checked before anything is published.** The reconciliation shells out to `gcloud`, which resolves its **own** credential chain—`op-firebase-deploy` authenticates with a per-project Firebase-vault SA key materialized into a temp file that it deletes on exit, and `gcloud` never sees it. A deploy started without `eval "$(scripts/op-preflight.sh --agent <agent> --mode deploy)"` can therefore publish to Firebase perfectly and then have nothing usable left to reconcile with. So `scripts/deploy.sh` runs both invoker scripts read-only (`--dry-run`) as Step 1.6, *before* `op-firebase-deploy`, and aborts with `NOTHING HAS BEEN PUBLISHED` if that fails. `--skip-invoker` skips the check and the reconciliation together.

There is a second-order trap worth naming, because the obvious remedy causes it: deploy preflight exports `GOOGLE_APPLICATION_CREDENTIALS` pointing at the per-project Firebase-vault **service-account** key, and `scripts/gcloud/gcloud` rejects a service-account key outright—it mints tokens from an `authorized_user` ADC only, and answers `GOOGLE_APPLICATION_CREDENTIALS points to an unusable credential file`. `unset GOOGLE_APPLICATION_CREDENTIALS` before the `gcloud` calls so the wrapper resolves its normal ADC chain; this is the same rule `docs/agents/deployment-process.md` already states for non-deploy `gcloud` work. Step 1.6 prints `gcloud`'s own error verbatim and names this case, so the message points at the fix instead of at a renamed service.

**It is also detected independently, not just prevented.** The annotation can be reset by a `firebase deploy` run outside `scripts/deploy.sh`, so automatic reconciliation alone is not enough. `tests/synthetic/unsubscribe-invoker.spec.ts`—part of the Step 4 post-deploy synthetic gate (`npm run test:synthetic`), and of the every-15-minutes `synthetic-uptime` workflow—sends one unauthenticated GET to `/unsubscribe` with no query params and fails loudly if it gets back `403` (blocked at the Cloud Run invoker layer) instead of the healthy `400` (reached application code, just missing a token). It **skips** rather than fails on a `200` carrying the SPA shell, which is what an origin whose Hosting has not yet received the `/unsubscribe` rewrite returns—so the scheduled monitor cannot page during the merge-to-deploy window, and it starts enforcing by itself on the first Hosting deploy that ships the rewrite, with no flag to flip. The one case that trade gives up—a change that *deletes* the rewrite—is covered structurally by `src/recon-share-og.test.ts`.
