...
- `EMAIL_FROM`—default `Five Across <hello@fiveacross.app>`. Reached by an **editionless** Event, not merely an unknown one: `resolveEventOrigin` returns `edition: null` for an Event with no active `hostnames` mapping, and `fromAddressFor` cannot resolve a register for `null`, so the send lands on this param even when all three `EMAIL_FROM_*` overrides are set. The default is therefore the occasion-neutral platform identity rather than any one Edition's ([#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102)); the prior value branded such an Event `Gay Cruise Bingo`, asserting a specific Edition over an Event that had not claimed one. Set it explicitly per project to the same value.
  ...
  > ⚠️ **`EMAIL_FROM` MUST sit on a domain verified in Resend.** The Resend account verifies two sending domains: **`fiveacross.app`** ([#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102) — sending enabled, receiving disabled, no tracking) and **`mail.nathanpayne.com`** (#633), which is what the default above uses. Resend rejects a send from an unverified domain, and `sendEmail` swallows that into a logged `false` (ADR 0001's never-throw contract)—so an override that lands on an unverified domain fails the same way: quietly, with the moderation notifiers and the daily email both logging and delivering nothing. Confirm any per-project override in `functions/.env.<projectId>` is on one of those two before deploying it. In practice this param is no longer reached—see the Edition-aware overrides below—but it remains the safety net if one of them is ever unset.
