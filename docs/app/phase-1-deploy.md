...
**It is also detected independently, not just prevented.** The annotation can be reset by a `firebase deploy` run outside `scripts/deploy.sh`, so automatic reconciliation alone is not enough. `tests/synthetic/unsubscribe-invoker.spec.ts`—part of the Step 4 post-deploy synthetic gate (`npm run test:synthetic`)—sends one unauthenticated GET to `/unsubscribe` with no query params and fails the deploy loudly if it gets back `403` (blocked at the Cloud Run invoker layer) instead of the healthy `400` (reached application code, just missing a token).
