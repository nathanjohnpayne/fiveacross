...
Five Across deployment is currently fail-closed. Because its central handoff origin is active while `skipInvokerReconcile` is still `true`, every repo-owned named Five Across deploy refuses before build or publish. Complete [#547](https://github.com/nathanjohnpayne/gaycruisebingo/issues/547) first: grant the Five Across deploy service account `run.services.update` on `fiveacross`, successfully apply `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh` to both existing callables, then change `skipInvokerReconcile` to `false` in a reviewed change. Once the flag is false, every hosting-only or full named Five Across deploy re-applies that idempotent two-callable readiness operation before it constructs or spawns the build/deploy command. A failed half aborts with nothing built or published, and ambient handoff project/region/service overrides are scrubbed so an earlier repair cannot redirect the proof. Wrapper flags cannot bypass either state.
...
`validateTargetOperationalMetadata` refuses a target that omits any of those three, so a missing field fails the build rather than picking a default. `skipInvokerReconcile` is required for the same reason `skipCloudflarePurge` is, only sharper: `false` asserts that the selected project's deploy credential and expected Cloud Run services are ready for the repo's pre/post-deploy reconciliation. A handoff-enabled target additionally runs the real two-callable application before any build/deploy invocation. That claim must never come from an omitted field or an unrelated project's successful check. Answer two questions to choose:
