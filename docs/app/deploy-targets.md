...
Five Across deployment is currently fail-closed. Because its central handoff origin is active while `skipInvokerReconcile` is still `true`, every repo-owned named Five Across deploy refuses before build or publish. Complete [#547](https://github.com/nathanjohnpayne/fiveacross/issues/547) first: grant `firebase-deployer@fiveacross.iam.gserviceaccount.com` `run.services.update` on `fiveacross`, successfully run `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh --prove-update` so both existing callables exercise that permission even when already configured, then change `skipInvokerReconcile` to `false` in a reviewed change. Once the flag is false, the named target pins the repository root and a Five Across readiness marker into the canonical `deploy.sh` process. That process parses the Firebase scope once, runs the pinned firebase-tools grammar and repository Hosting-config validators, and then runs every remaining nonmutating pre-build guard (including Functions parameter coverage). Unknown Hosting sites and other invalid filters fail locally before readiness; a valid site-qualified exclusion can remove this repository's only Hosting surface. `scripts/apply-auth-handoff-deploy-readiness.sh` runs before any build or publish only when the validated scope can release Hosting or either handoff callable. Rules/storage and exact known-unrelated Function scopes skip the live update; unfamiliar Function groups/codebases remain conservative. Firebase `--dry-run` still validates/builds without an app or Functions release and skips this wrapper's readiness/post-deploy IAM updates after the same guards; Firebase documents that it may still enable project APIs. Deploy-command and global option values (including `--account` and `--token`) cannot masquerade as that flag, and option-looking required values such as `--only --dry-run` fail before readiness or build. Named targets reject every split, equals, and attached-short `-P`/`--project` or `-c`/`--config` override so Firebase cannot replace the target registry's project or config by last-value precedence. `--skip-invoker` is refused because a Functions release must retain its post-deploy handoff repair. The readiness wrapper reuses the canonical deploy-credential materializer, accepts a direct credential only when it is the exact Five Across deploy service account, and otherwise pins keyless impersonation to that identity only after the vault reports the project key is absent. A vault authentication, biometric, or infrastructure failure aborts before gcloud rather than falling through to ambient credentials. Ambient project/region/service choices cannot redirect either path, and readiness-only identity overrides are scrubbed from ordinary reconciliation. A failed half aborts with nothing built or published, and every wrapper-owned credential file is removed on success, failure, or signal.
