...
`.vercelignore` is what makes the command possible at all, and it is not optional. The Vercel CLI does **not** read `.gitignore`, so without it `vercel deploy` walks the whole checkout — here that is `node_modules/` plus `.claude/`, which holds the agent worktrees and their own `node_modules/`. The first real manual mirror deploy failed exactly there: *"`files` should NOT have more than 15000 items, received 48318."* With the ignore file the upload is 325 files. It also keeps `.env.local` — the live Firebase config — off Vercel entirely; the build gets its config from the project's own Environment Variables. Do not delete it, and if you add a build input at the repo root, check it is not swept up by one of its patterns.
