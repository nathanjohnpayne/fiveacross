...
A multi-service family (event invitations, admin callables) derives its strict set per Functions codebase from what each codebase's index exports (#1282). A strict service that is missing after publish fails the deploy, while an unexported or unselected peer (such as `approveprompts` before #1275 deploys) is tolerated. Only an export proven to be a runtime value is strict. A name exported under the rule used before #1282 (an export-modified variable, function or class, or an export-clause element not marked type-only), which the type analysis cannot prove to be a value, still keeps its family selected. Its service is then reconciled if present and allowed to be absent. By scope:

- **Whole codebase** (`--only functions:default` or `--only functions:<codebase>`): strict for exactly the family's services that codebase exports. `functions:default` selects a family when that codebase exports one of its services, strictly or as a mention (see above), as before #1282. Any other codebase selector keeps both families selected, as it always did, and reconciles every service not proven exported with absence allowed. A callable that only the built artifact reveals (#1283) therefore still gets its invoker repair.
...
- **Uninventoried codebase**: a codebase whose surface its TypeScript index cannot prove. A whole-codebase or all-codebase scope that releases it also selects both families, with every service not proven exported allowed to be absent; an exact callable selector into it selects only that callable's family, with nothing strict. A codebase is uninventoried when it is any of:
  - a kit (whatever its `source`), a `remoteSource` codebase, or a non-Node `runtime`;
  - every codebase, even one whose index already exists, when any `predeploy` hook in the project (in any Functions codebase or any other target, whether or not this deploy selects it) is not a codebase's own compile step, or when any hook exists and one Functions source directory or its `lib/` is, or lies inside, another codebase's (compared as real paths, so a symlink cannot hide it). Firebase runs those hooks before Functions prepare loads any source, and a hook can generate or rewrite any codebase. The compile step is the generated hook alone (`npm --prefix "$RESOURCE_DIR" run build`, or `npm --prefix "%RESOURCE_DIR%" run build` on Windows), running a `build` script that is `tsc`, optionally followed by same-path `cp src/<file> lib/<file>` copies of literal paths (no glob or other shell expansion) onto no file `tsc` emits from a TypeScript source and through no symlink or hard link, with no `prebuild` or `postbuild` script (this repository's hook and `build` script qualify);
  - a source directory that does not exist yet in a codebase with any `predeploy` hook, a source directory with a `functions.yaml` manifest (it decides discovery before the index loads), a source directory with no `package.json` (the CLI then infers Python or Dart), a `package.json` that is unreadable, invalid, or names a `main` other than `lib/index.js`, a source directory with no `src/index.ts`, or a `tsconfig.json` that does not compile `src/index.ts` to `lib/index.js` on every build (an `incremental` or `composite` build can leave a stale entry), with or without a hook (this repository's does);
  - an index, or a local module it reaches through `export *`, that references CommonJS `exports`, `module` or a top-level `this`, uses `export =` or `export default`, or exports anything but an identifier-named variable, function, class, interface or type alias (such as a destructuring pattern or `export import`);
  - an index or local module whose exports, followed through re-exports, include an `import name = a.b` alias, a `const enum`, or a namespace holding only types and const enums, since TypeScript emits the alias only when the entity it names is a value and the enums only when compiler options preserve them; a name looked up through a local module that exists only as a declaration file or JavaScript; or a type-only lookup that loops back on itself through re-exports. A name that reaches a module only through `export type *` is a type.

The bug-report, email-unsubscribe and auth-handoff families are not yet scoped by codebase (#1299). A Functions deploy whose index exports an HTTPS function that is in no family, and not listed with a reason in `PRIVATE_HTTPS_EXPORTS`, fails classification naming the export before anything is built or published. So does any Functions deploy of a project with a codebase that sets `prefix`, because the prefix renames every Cloud Run service that codebase publishes and no invoker family reconciles a renamed service. Register a new callable in a family (and that family's wrapper and `deploy.sh` wiring) in the same change that exports it. For the first `approvePrompts` release in a project, follow [`phase-1-deploy.md` §1c](phase-1-deploy.md#1c-first-deploy-of-the-approveprompts-callable-1275-adr-0015) before the full deploy.
