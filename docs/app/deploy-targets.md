...
Five Across deployment is currently fail-closed. Because its central handoff origin is active while `skipInvokerReconcile` is still `true`, every repo-owned named Five Across deploy refuses before build or publish. Complete [#547](https://github.com/nathanjohnpayne/gaycruisebingo/issues/547) first: grant `firebase-deployer@fiveacross.iam.gserviceaccount.com` `run.services.update` on `fiveacross`, successfully run `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh --prove-update` so both existing callables exercise that permission even when already configured, then change `skipInvokerReconcile` to `false` in a reviewed change. Once the flag is false, every hosting-only or full named Five Across deploy runs `scripts/apply-auth-handoff-deploy-readiness.sh` before it constructs or spawns the build/deploy command. That wrapper reuses the canonical deploy-credential materializer, accepts a direct credential only when it is the exact Five Across deploy service account, and otherwise pins keyless impersonation to that identity; ambient project/region/service/identity choices cannot redirect the proof. A failed half aborts with nothing built or published, and every wrapper-owned credential file is removed on success, failure, or signal. Wrapper flags cannot bypass either state.
