...
`.env.local` remains the generic local-development config. Production deploys select `.env.gaycruisebingo` or `.env.fiveacross` instead, so the main checkout can build either Firebase project safely. These files contain **non-secret client identifiers**—they are baked into the client bundle by design, and security is enforced by the Firestore/Storage rules + Auth, not by hiding them. They are gitignored. Copy `.env.example` into each target file and regenerate every value from that target's registered web app rather than copying values from another project. `firebase apps:sdkconfig` requires authentication in a fresh shell (`.before(requireAuth)` in the pinned `firebase-tools@15.26.0`'s `commands/apps-sdkconfig.js`; `requireAuth` falls through to Google's Application Default Credentials and fails before printing any config if none resolve): run `eval "$(scripts/op-preflight.sh --agent <agent> --mode deploy)"` first, which exports `GOOGLE_APPLICATION_CREDENTIALS` in the current shell (`docs/agents/deployment-process.md`)—the script only prints export statements, so it must be wrapped in `eval "$(...)"` per its own usage header (`scripts/op-preflight.sh`) or the variable never reaches the shell that runs `firebase apps:sdkconfig`—or `gcloud auth application-default login` (the same credential §4 uses for seeding), which writes the well-known ADC file Google's auth library falls back to without that variable. Deploy preflight's `.firebaserc`-based project detection (`detect_firebase_project` in `scripts/op-preflight.sh`) always resolves to this repo's single default project, `gaycruisebingo`; regenerating `.env.fiveacross` must therefore set `OP_PREFLIGHT_FIREBASE_PROJECT_ID=fiveacross` while `op-preflight` runs. These deployer credentials are project-scoped (`docs/agents/deployment-process.md` § deploy credential precedence), so authenticating with `gaycruisebingo`'s key and then requesting `fiveacross`'s config fails authorization even with a valid ADC. With the matching project's credential active, `firebase apps:sdkconfig WEB --project <target-project-id> --non-interactive` prints the current config JSON to map into that target's `.env.<target>` file (see the field mapping below). Keep every `VITE_*` entry, including intentionally blank optional values: target builds reject an incomplete file rather than inherit `.env.local`.

Map the JSON fields into the matching target file: `apiKey`→`VITE_FIREBASE_API_KEY`, `authDomain`→`VITE_FIREBASE_AUTH_DOMAIN` (**do not copy verbatim—override, see below**), `projectId`→`VITE_FIREBASE_PROJECT_ID`, `storageBucket`→`VITE_FIREBASE_STORAGE_BUCKET`, `messagingSenderId`→`VITE_FIREBASE_MESSAGING_SENDER_ID`, `appId`→`VITE_FIREBASE_APP_ID`, `measurementId`→`VITE_FIREBASE_MEASUREMENT_ID`. Target builds verify every value against the reviewed target registry in `scripts/build-target.mjs`. When Firebase rotates a web API key, update both that target's environment file and the registry in the same reviewed change; otherwise the build deliberately rejects the new key. `VITE_EVENT_ID` is a build-mode switch, not a default: a non-empty value (the legacy deployment uses `med-2026`) marks a single-Event build that never consults the `hostnames/{host}` lookup, while leaving it empty produces a hostname-resolved multi-Event build (ADR 0009)—see § Event id below. `VITE_EDITION` (`gcb`, `vacay`, or `fiveacross`) brands the pre-auth sign-in shell of a single-Event build, and is baked into its document title and PWA identity at build time (`src/editions.ts`). Every target build must set it explicitly alongside `VITE_EVENT_ID`; hostname-resolved builds take the Edition from `hostnames/{host}.edition` instead. `VITE_RECAPTCHA_SITE_KEY` is Phase-1 (App Check). Leave it blank until provisioned, then set it in each affected target file—never `.env.local`, which production target builds deliberately ignore.

> **Five Across preflight ordering:** place the project override **inside** the command substitution, so it is present while `op-preflight` detects and loads the credential: `eval "$(OP_PREFLIGHT_FIREBASE_PROJECT_ID=fiveacross scripts/op-preflight.sh --agent <agent> --mode deploy)"`. Prefixing `eval` itself is too late—the shell expands `$(...)` first and would provision the default `gaycruisebingo` credential.

**`VITE_FIREBASE_AUTH_DOMAIN` must be a bare hostname (no `https://`): `gaycruisebingo.com` for the Firebase build and `gaycruisebingo.vercel.app` for the Vercel build.** At runtime the app pins known production hosts (`.com`, `.vercel.app`, and `.firebaseapp.com`) to their own first-party handler regardless of a stale build variable. A signed-out `.web.app` visitor is moved once to the same-project `.firebaseapp.com` app before sign-in because that Google callback is already authorized. Firebase serves the OAuth helper at `<authDomain>/__/auth/handler`; keeping it first-party prevents storage-partitioned browsers from losing the sign-in state. Every surface whose handler is first-party uses one top-level redirect rather than a popup tab or window (#765), which avoids iOS private-browsing window loss and desktop standalone windows where OAuth popups can be blocked or hidden; only a cross-origin handler (local development, the Auth Emulator) still uses the popup.
...
> **⚠️ The prompt pool lives in Firestore, not in the deployed JS bundle**—the app renders `events/{id}/items`, which only this seed writes. Changing the pool in `src/data/seed.ts` / `scripts/seed.mjs` and deploying the app does **not** reach players: you must re-run the seed against the live project. A frontend change (e.g. the 🔞-toggle) ships with its named target hosting command; a **pool** change additionally requires a reseed. This is exactly how the #129 87-prompt update reached players' cards late—the code merged and the bundle deployed, but the reseed was skipped. Whenever `ITEMS` changes, reseed, then confirm with the drift check:
>
> ```bash
...
#    before the app goes live. Choose ONE project for this rollout.
#    (Rules compile-check happens here — a bad rule fails this step, not at runtime.)
npm run deploy:gaycruisebingo -- --only firestore:rules,firestore:indexes,storage
npm run deploy:fiveacross -- --only firestore:rules,firestore:indexes,storage
...
- **Event id**: `VITE_EVENT_ID`—a **non-empty value** is the single-Event build-mode switch (#543, ADR 0009). Set it (the legacy deployment bakes `med-2026`) and the build serves exactly that Event, skipping the `hostnames/{host}` lookup entirely; set it to the exact empty string and the build resolves its Event from `window.location.hostname` before first paint. The shared Five Across target now requires the empty form, so `fiveacross.web.app` can serve as the Worker's generic origin without every wildcard host mounting Bodega. The Five Across Vercel mirrors are not built by that target: each keeps the single-Event pin its own Vercel Production `VITE_EVENT_ID` holds (§ 7 Vercel production mirror). The schema is event-scoped either way, so future Events are new Event documents.
- **Edition** (`VITE_EDITION`): which branded product line a single-Event build serves—`gcb`, `vacay`, or `fiveacross` (Theme scoping from #617, chrome and vocabulary register from #608). It scopes every Theme picker, player and admin, selects the Edition default Theme above, and supplies the **Lexicon**—the vocabulary register its copy speaks (`src/editions.ts`, CONTEXT.md § Lexicon). A non-empty `VITE_EVENT_ID` lets Vite bake this Edition's browser/PWA chrome. A hostname-resolved build ignores ambient `VITE_EDITION` for that decision; the trusted Five Across target registry separately pins a Vacay **static fallback** so today's direct-serving Bodega hosts keep their install and pre-JavaScript identity. Runtime lookup still installs each host's own Edition before mount. A non-Vacay hostname must not be attached to the wildcard until its static identity is per-host: #546 moved the PWA manifest to the edge Worker, and #1118 owns the crawler metadata that is still baked per build.
- **Adult content** (`VITE_ADULT_CONTENT`): the 18+ posture for a SINGLE-EVENT build only (#608). Every registered target defines it for completeness, but a hostname-resolved build ignores it: the live `hostnames/{host}` document supplies and proves the posture before mount. On a single-Event build, only a literal `false` opts out; all other values keep the gate. It is a **seed, not a pin**: the app re-reads `hostnames/{host}` for the posture alone, so a build that baked `false` still observes a later server-side raise—and if no routing document exists for its origin, the posture returns to gated, because an opt-out nothing can withdraw is not one this design offers.
