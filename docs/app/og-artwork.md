...
**The domain line is the one deliberate exception.** It defaults to the `ogUrl` hostname, which for `gcb` and `fiveacross` *is* the brand's apex. Vacay's `ogUrl` is Event-scoped (`bodega-bay.fiveacross.app`) because a build can bake only one origin — the edge HTML rewrite (#1118) replaces the served `og:url` per hostname, but the brand row it overrides is still what this renderer reads — and an unfurl is a brand impression, so that row overrides the domain to `vacaybingo.com` in the renderer with a comment saying why.
...
It opens the wireframes document in Playwright's chromium at `deviceScaleFactor: 2`, rewrites the footer line from the brand table (`${appName} ${lexicon.shareMark}`, uppercased by the artboard's own CSS), and screenshots the artboard. The capture is staged beside its destination and only replaces the committed picture once it is 600×750, it is 8-bit non-interlaced truecolor, and, for every Edition but Vacay (whose card is cream end to end and is exempted), its upper-right quadrant is not mostly cream — the #887 overlay, expressed as a check. That last one is the same check `src/recon-share-og.test.ts` runs on the files already in the tree, from the same constant and the same predicate (`scripts/og/share-card-overlay.mjs`), and it refuses a capture that reaches the cap rather than only one that exceeds it: the committed-file guard accepts a share strictly below the cap, so anything looser here would let a run report success and red the suite on the file it had just written. Every capture, Vacay's included, must also decode as a structurally complete PNG (`readPngPixels` in `scripts/og/png-pixels.mjs`) before any of that is scored: every chunk inside the file with a matching CRC, an empty IEND chunk that ends the file, and image data that inflates to exactly the scanlines IHDR declares, so a capture truncated after its last IDAT chunk or carrying a corrupted CRC is refused before it can replace a committed card. The format half of that is `scripts/og/share-raster-format.mjs`, split out so it is testable without a browser: dimensions alone let a correctly sized capture in the wrong PNG format through, and since the recon guard requires colour type 2 of the committed files, the first thing that would notice is a red suite with the good picture already overwritten. `--edition` is required and `--all` is the explicit opt-in, for the same reason the unfurl renderer gives, and both share-card scripts refuse the two together rather than quietly rendering the one Edition named; `--out <dir>` writes to a scratch directory for a first look, and `--check` reports without writing. Both share-card scripts refuse an unrecognised token before they launch Chromium or touch the filesystem, naming it. That is not tidiness: `--check` is the flag that makes a run write nothing, so a typo in it used to be the one mistake that turned a report into a publish.
...
**How that line looks is not this tool's decision.** The type size, tracking, family, casing and ink all come from the artboard the card was captured from: `scripts/og/share-card-footer-style.mjs` reads the `.shc .foot` rule out of `plans/daily-cards-wireframes.html`, scales it by the capture's `deviceScaleFactor` of 2, and resolves the `var(--dim)` it names against the Edition's own `data-theme` — in that same document, because the wireframes links no stylesheet for those tokens and declares its own `[data-theme]` blocks inline. Those blocks are what Chromium applies to the card a full render screenshots, so they are what a footer refresh has to match. `src/theme/themes.css` is a near-copy maintained separately and the two have already diverged (`fiveacross-slate` carries a different `--primary`, `--secondary` and `--on-gradient` in each); `themeTokenDisagreements` reports every such divergence by name and `render-share-footer.test.mjs` pins the inventory, so the next one shows up in the suite rather than in a rendered asset. Reconciling the two tables is a separate job. So the repaint draws 17px type with .16em tracking in that theme's dim ink, which is what the capture drew. It used to keep its own copy of all of that, and the copy had drifted both ways at once: 16px type with 3px tracking on every card, and a Vacay ink of `#8a857b` where `fog-froth-farewells` sets `#66625a`. A refresh documented as changing one line of copy therefore restyled the type on all three cards and reverted Vacay's colour. The band's own geometry (`y: 694`, 32 rows) stays measured off the committed assets, because the artboard lays the footer out with `margin-top: auto` and there is no rule to read an offset from.
