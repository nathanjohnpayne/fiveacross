...
- **Emulator prerequisite**: the emulator-backed entry points (`test:rules`, `test:offline`, `test:e2e`, `npm run emulator`) need a runnable JDK. Each sources `scripts/lib/ensure-java.sh`, which probes by running `java -version` (a `command -v` check passes on macOS's non-functional `/usr/bin/java` stub) and falls back to `JAVA_HOME`, `/usr/libexec/java_home`, and Homebrew's keg-only `openjdk` before failing with a `brew install openjdk@21` hint (#1018). Its own suite is `npm run test:ensure-java`.
