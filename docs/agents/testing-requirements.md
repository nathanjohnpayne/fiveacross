...
- **Origin integration** (`npm run test:origin`): the Five Across production-origin seam in `scripts/fiveacross-origin.integration.test.mjs`, which spawns a full `vite build --mode production` under the real fiveacross deploy environment and then boots hostname resolution against a mocked Firestore. Its own layer (`vitest.origin.config.ts`) and its own app-ci step, kept out of `npm test` so the unit suite carries no dependency on a working production build (#965).
...
- **Emulator prerequisite**: the emulator-backed entry points (`test:rules`, `test:offline`, `test:e2e`, `npm run emulator`) and `scripts/marketing-shots.sh` need a runnable JDK 21+ — firebase-tools rejects older majors. Each sources `scripts/lib/ensure-java.sh`, which probes by running `java -version` (a `command -v` check passes on macOS's non-functional `/usr/bin/java` stub) and reading the reported major, then falls back to `JAVA_HOME`, `/usr/libexec/java_home`, and Homebrew's keg-only `openjdk` before failing with a JDK 21+ install hint naming both a vendor build and `brew install openjdk@21` (#1018). Its own suite is `npm run test:ensure-java`.

CI (`.github/workflows/app-ci.yml`) runs typecheck → unit/component → origin integration (`test:origin`) → build → deployment safety (`test:deploy`) → the shared JDK probe's own suite (`test:ensure-java`) → functions → rules → offline on every PR and on pushes to `main`; e2e is the local smoke layer. The two emulator-backed steps (rules, offline) each boot their own `firebase emulators:exec` and must stay separate sequential steps—never two live emulator instances in one job. Spec↔test alignment is enforced by `scripts/ci/check_spec_test_alignment`, with per-spec test mappings in `.repo-template.yml` (`spec_test_map`) for the cases where a test filename doesn't match its spec basename.
