...
When a review round changes a claim—a behavior, an ordering, or a guarantee that some comment, docstring, spec sentence, or caption asserts about the system—the author sweeps every tracked prose surface that restates it in the same commit (code comments, `specs/**`, `plans/**` captions, README prose) and updates the PR body in the same review round, since a PR body is GitHub metadata and cannot be part of any commit. Grep the tree for the claim's key terms rather than trusting the current diff to surface every place it lives, because the trigger is "this round changed a claim," not "this round changed behavior"—a prose restatement can go stale with no code change ever touching the line it sits on, since it was accurate for the round that wrote it and nothing after that round had reason to look at it again. PR #1207 is the motivating example: a post-merge audit of that one PR found five separate instances of exactly this staleness surviving across its sixteen review rounds. A reviewer who finds a leftover restatement of a claim a later round already retracted treats it as a P1, not a nit.
