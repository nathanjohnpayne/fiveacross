...
- **The prompt pool is Firestore data, not the bundle** ([ADR 0003](../adr/0003-pool-is-pre-cruise.md)): changing `ITEMS` requires a reseed, not just a deploy—see [`docs/app/README.md`](../app/README.md) §4 and run the target's verifier: `npm run verify:seed` for Gay Cruise Bingo or `npm run verify:seed:fiveacross` for Five Across.
- **The OG renderers' committed destinations are lock-guarded, not append-only.** Three scripts publish committed artwork this way and no other: `scripts/og/render-og-editions.mjs` writes `public/og-*.png` and their `plans/og-images/` mirrors, and `scripts/og/render-share-rasters.mjs` and `scripts/og/render-share-footer.mjs` write `plans/og-images/share-final-photo-*.png`. All of them go through `og-stage-commit.mjs`'s all-or-nothing `commitStaged`, itself held under `og-commit-lock.mjs`'s per-destination cross-process lock for the whole commit-or-rollback phase—see [`specs/og-renderer-commit-safety.md`](../../specs/og-renderer-commit-safety.md) for the fail-closed output guards and atomicity contract. Any new caller that writes to any of those destinations must acquire the same lock for its whole commit phase first, or the concurrent-writer race the lock exists to close reopens; `commitStaged` performs no locking of its own. **A caller whose capture READS the file it replaces must hold that lock across the read too**—the share-card footer refresher repaints a committed card, so it passes `readsDestination: true` to `renderCardSet` and reads the card from inside its capture seam; locking only the commit phase lets a concurrent full render land between the read and the publish and be silently overwritten by a repaint of the stale snapshot.
