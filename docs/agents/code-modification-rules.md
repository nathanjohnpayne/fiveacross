...
- **The OG renderer's committed destinations are lock-guarded, not append-only.** `scripts/og/render-og-editions.mjs` writes `public/og-*.png` and their `plans/og-images/` mirrors only through `og-stage-commit.mjs`'s all-or-nothing `commitStaged`, itself held under `og-commit-lock.mjs`'s per-destination cross-process lock for the whole commit-or-rollback phase—see [`specs/og-renderer-commit-safety.md`](../../specs/og-renderer-commit-safety.md) for the fail-closed size-cap and atomicity contract. Any new caller that writes to those destinations must acquire the same lock for its whole commit phase first, or the concurrent-writer race the lock exists to close reopens; `commitStaged` performs no locking of its own.
