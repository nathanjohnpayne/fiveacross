...
| Domain | Role |
|---|---|
| `fiveacross.app` | **Canonical Brand domain going forward** ([#599](https://github.com/nathanjohnpayne/fiveacross/issues/599) as amended). New Events provision here first; analytics canonicalize here (one canonical host per Event); this is the name we print on things. Share links deliberately do **not** canonicalize—a guest shares the host they are standing on ([#607](https://github.com/nathanjohnpayne/fiveacross/issues/607)). |
| `gaycruisebingo.com` | The Gay Cruise Bingo Edition's domain. |
| `vacaybingo.com` | Travel Namespace; the Bodega Bay Event's original host, still live—and still named canonical by its own `hostnames` doc until the post-event repoint ([#601](https://github.com/nathanjohnpayne/fiveacross/issues/601)). |
| `fiveacrossbingo.com` | Original Brand domain — a registered zone, never a Namespace. **Being retired**, not a serving host: no Event has ever been dealt from this origin, and the zone becomes a 301 to `fiveacross.app` once the Bodega migration completes ([#630](https://github.com/nathanjohnpayne/fiveacross/issues/630)). Today it carries a redirect **hostname** — deliberately not an Alias, which by definition serves in place with no edge redirect — plus DNS-only `auth.` / `d.` infrastructure. |
...
Each Event has exactly one **canonical hostname** and any number of validated **aliases**. An alias serves the Event in place with correct branding—no edge redirect ([#599](https://github.com/nathanjohnpayne/fiveacross/issues/599) as amended); sessions, installed PWAs, and offline storage are per-origin facts of whichever host a guest uses, and share links carry that entry-point host ([#607](https://github.com/nathanjohnpayne/fiveacross/issues/607)). What the canonical hostname does is keep the Event's analytics whole: every host reports the one canonical host, so cross-host traffic aggregates. Ingest is Brand-level besides, and one PostHog project carries `brand_id` / `edition_id` / `event_id`.
...
Whether an Event shows the 18+ acknowledgement and adults-only copy is a property of **the Event's prompt pool**, not of its Edition ([#608](https://github.com/nathanjohnpayne/fiveacross/issues/608)). A `fiveacross` Event with spicy Prompts is 18+ with general vocabulary; a `gcb` Event with a tame pool is not 18+ at all. The two axes are independent, and copy must never assume otherwise.
