...
## Hostname drift report: brand-mirror replacement audit

`reconcileHostnameReplicas` (`hostname-reconciler.mjs`) is the hostname drift report: with injected Admin listings it classifies every host's source, ledger and Durable Object state (`specs/event-router-registry.md` § Provisioning, mutation, and deletion). Every report also carries `mirrorReplacementAudit` (#1295): `activeMirrors`, and one finding per active brand-mirror route whose Event has no other active, converged route at a non-mirror host for the mirror's Event, Edition and slug. A finding names the mirror host, the Event, Edition and slug, and each host in the listing that was or could be the replacement, with its condition (`disabled`, `archived`, `repointed`, `edition-mismatch`, `deleted`, `missing`, `root-marker`, `unreadable`, `locked` or `not-edge-converged`). The audit is read-only and detection only: it writes nothing, refuses no lifecycle intent, and a finding is a report row rather than a failure, so a run fails only when its input cannot be read. The operator then disables the mirror or restores the replacement deliberately.
