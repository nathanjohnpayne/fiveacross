...
# `npm run test:offline --` used to land on emulators:exec itself rather than on
...
# `npm run test:offline -- tests/offline/echo-marks.test.ts` reaches vitest
...
cmd="vitest run --config vitest.offline.config.ts"
for arg in "$@"; do
  cmd+=" $(printf '%q' "$arg")"
done

exec firebase emulators:exec --only auth,firestore "$cmd"
