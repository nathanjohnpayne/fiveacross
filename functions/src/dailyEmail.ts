...
export interface DailySendResult {
   ...
   *  halves of the post-Event freeze — archived and closing (#1150). */
 ...
 * (#1150). `runDailyEmailSweep` selects `status == 'active'`, which excludes an
 * archived Event but NOT a closing one — the quiesce deliberately leaves
 * `status` alone — and the selection happens once per run, before any Event is
 * processed, so a run selected moments before the freeze still arrives
 * afterwards. Left unguarded, a paused or failed archive keeps mailing "here is
 * today's card" at an Event where every gameplay write the mail invites is
 * denied at the rules boundary.
 *
...
export async function sendDailyEmailForEvent(
  ...
  // standings snapshot — and it is exactly the elapsed time a sweep selected just
  // before the freeze spends arriving here. Re-reading the ONE document that
  // carries the freeze is what turns "this Event was active when the sweep
...
export async function runDailyEmailSweep(
  ...
  // This selection is NOT the freeze check (#1150). It excludes an archived
