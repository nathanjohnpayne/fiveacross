...
Nothing the classification runs can undo those three. Before the first predeploy hook, the deploy-scope classifier puts every program it is about to start under a platform write-containment mechanism — `sandbox-exec` on macOS, `bwrap` or an unprivileged user+mount namespace on Linux — and proves it with two checks inside that containment: a write into **this checkout** must fail, and a write into the classifier's own staging directory must succeed. Both are required; a mechanism that denied everything and one that denied nothing each pass one of them alone. The classifier keeps its scratch root and the system temp dir writable because a build's toolchain cannot work without them, and carves this checkout back out of that writable set as a nested read-only override — so a repository checked out beneath `$TMPDIR` is contained exactly like one anywhere else. If either check fails, or the checkout cannot be expressed as read-only, the classification is conservative and no hook is run at all; `FIREBASE_DEPLOY_CLASSIFIER_DEBUG=1` names which. On a machine that can prove no mechanism at all, both suites ask `probeWriteContainment()` first, skip every case that needs a predeploy hook to have run inside a containment, and prove the refusal itself in their place: every selector is refused, the candidates that could not be proved are named, nothing is staged and no hook runs. `app-ci` was that machine until #1164 — its `ubuntu-latest` image ships no `bwrap`, and Ubuntu 24.04 refuses the unprivileged user namespace the `unshare` fallbacks need — so the Linux `bwrap` path was exercised nowhere. The workflow now installs `bubblewrap` before either suite, drops `kernel.apparmor_restrict_unprivileged_userns` only if the probe does not already name `bwrap` — a probe that named some other mechanism counts as not naming it — and fails the job unless the probe after that drop names `bwrap`: an `unshare` fallback winning quietly would run the cases while leaving the `bwrap` argv as untested as before. The step prints the `bwrap` version, the sysctl value and the mechanism the probe named, which is where a later regression is read off. Set `FIREBASE_DEPLOY_CLASSIFIER_FORCE_NO_CONTAINMENT=1` to make a machine that *can* contain a hook answer the same way, which is now how that arm is exercised everywhere, on a development Mac and on `app-ci` alike. What this does **not** cover is a writer the classifier did not start — another window, a background `git fetch`, a process that was already running — which is what the fingerprints, the re-run below and guard 3 are for.
