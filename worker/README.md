...
Two related claims are checked elsewhere rather than here, and deliberately: that the router declares no Durable Object, KV, cache or Firebase binding is asserted against the committed file by `src/routerBinding.test.ts` (§ *what the router's configuration no longer declares*), and the guarded deploy runs only from a clean `main` that matches `origin/main`, so the file CI checked is the file that uploads.
