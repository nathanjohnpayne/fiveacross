...
It answers exactly two addresses itself, both from the same lookup that decided the request may proceed: the exact `GET /.well-known/fiveacross-path-capability` projection described above, and `GET`/`HEAD /manifest.webmanifest`, with the installed-app identity of the Edition the requested hostname resolved to ([#546](https://github.com/nathanjohnpayne/fiveacross/issues/546)). That route sits **after** the namespace guard and **after** resolution, so it is not an exemption like `/__/auth/*` — an address that does not serve an app does not serve an app identity either. The document comes from `src/web-manifest.ts`, the same module `vite.config.ts` emits `dist/manifest.webmanifest` from, so the origin's file and the edge's response are the same bytes for the same Edition.

It also REWRITES one response rather than answering it: the proxied `index.html`'s `<head>`, per hostname ([#1118](https://github.com/nathanjohnpayne/fiveacross/issues/1118)). A link-preview crawler runs no JavaScript, so the `og:*` / `twitter:*` block, `<meta name="description">` and `<meta name="theme-color">` a multi-Event bundle baked at build time have no runtime repair path; `HTMLRewriter` replaces them as the document streams, with `og:url` set to the requested hostname's own origin (a per-Event fact no build can bake) and the theme colour read back out of the same manifest builder, so the two stay exactly equal per Edition. It is gated on the same resolution as the manifest route and on a `200` HTML body, so every fail-closed outcome, the `/__/auth/*` exemption, an origin failure, a `206` answer to a `Range` request and every non-HTML asset are relayed untouched — a partial representation especially, because rewriting inside a window its own `Content-Range` frames by byte offset is how a resumed download reassembles corrupted HTML. Caching is the other half of getting it right: a document `GET` reaches the origin without its `If-None-Match` / `If-Modified-Since`, because the origin's baked `index.html` is unchanged after a hostname is repointed and its truthful `304` would hand the client back the previous Edition's unfurl, and the rewritten response carries neither the origin's `ETag` nor its `Last-Modified`, so that window closes after one fetch instead of recurring. An asset request keeps its validators and its cheap `304`.
...
**And `worker/` may carry no dotenv file.** The deploy wrapper refuses `CLOUDFLARE_ENV` and `WRANGLER_CI_OVERRIDE_NAME` in the shell it was started in, but Wrangler 4.129 reads more than that shell: every command resolves `.env`, `.env.local` and `.env.<env>*` against its working directory and merges them into `process.env` inside the yargs validation pass — after the guard has run, and before the command it guards does anything. A value written into `worker/.env.local` therefore applies the very override those two refusals exist to prevent, past a guard that saw a clean shell — and `worker/.env`, `worker/.env.local` and `worker/.dev.vars` are gitignored, so the clean-tree guard does not see them either. So `scripts/worker-deploy.sh` refuses the deploy (exit 65, naming the file) when any of `worker/.env`, `worker/.env.*`, `worker/.dev.vars` or `worker/.dev.vars.*` exists. Their PRESENCE is the refusal and nothing parses them: a parser here would be a second definition of what Wrangler would have loaded, and one that can disagree with Wrangler's is the failure this guard is fixing. The router carries no secrets — ADR 0014 removed the Firestore reader, `wrangler.toml` declares one service binding and no `[vars]`, and the deploy's own readback asserts the published Worker holds no credential at all — so there is no legitimate file here to lose. Belt and braces, every Wrangler command the wrapper runs carries `--env-file /dev/null`, which REPLACES Wrangler's default file list rather than adding to it: that covers the half the presence check deliberately does not, since `npm exec` keeps the caller's working directory and so resolves the secret readback's dotenv files at the repository root, where `.env.local` is the app build's own required input.
...
```bash
npm --prefix worker run types:router
```

`npm run`, not `npm exec`, and the difference is the command working or not. `npm exec` keeps the CALLER's working directory even with `--prefix worker`, so the same flags spelled `npm --prefix worker exec -- wrangler types …` look for a root-level `wrangler.toml` and `wrangler.registry.toml` when run from the repository root, and regenerate nothing. `npm run` sets the working directory to `worker/`, which is where both configurations are. Keeping the flags in `worker/package.json` also keeps them checkable: `src/routerBinding.test.ts` asserts `check:router-types` is `types:router` plus `--check`, so the committed evidence file cannot be regenerated under one set of flags and verified under another.
...
**After publishing** (and, on a route-bearing deploy, before it too), it lists the deployed Worker's secrets and exits `1` if any `FIREBASE_`-prefixed secret is still bound — `FIREBASE_API_KEY`, `FIREBASE_PROJECT_ID`, a `FIREBASE_SERVICE_ACCOUNT`, anything under the prefix — naming every one it found. The code no longer reads it, so it grants nothing on its own; it is refused because R0's evidence claim is that the public router carries no Firebase, KV, cache or Durable Object binding at all, and a credential that outlived the code that used it would quietly falsify that. An uninspectable Worker exits `75` — a failed verification, not a skipped one, so automation cannot record an unverified deploy as a verified one. The comparison is an anchored prefix match — the name must START with `FIREBASE_` — so an unrelated `OLD_FIREBASE_API_KEY` neither trips the refusal nor masks a live binding, while `FIREBASE_API_KEY_BACKUP` does trip it. To check by hand at any time:

```bash
npm --prefix worker exec -- wrangler secret list --config worker/wrangler.toml --env-file /dev/null   # names and types only — never values
```
...
A correct router is not sufficient. Four properties block the cutover, each in a different way. Two are ORDERING constraints on shipped code — the #546 bundle deploy, and now [#1118](https://github.com/nathanjohnpayne/fiveacross/issues/1118)'s per-host `<head>` rewrite, which is code that exists but answers nothing until the routes are attached; one is shipped code whose live verification is still owed under [#852](https://github.com/nathanjohnpayne/fiveacross/issues/852); and one is evidence still owed under [#888](https://github.com/nathanjohnpayne/fiveacross/issues/888) now that #972 has shipped its router half.
...
- **A non-Vacay hostname gets per-host static HTML identity only from behind this Worker (#1118).** The `fiveacross` origin builds with an empty `VITE_EVENT_ID`, so runtime Event, Edition and adult-content posture come from `hostnames/{host}` before mount; since #546 the PWA manifest comes from this Worker per host, and since #1118 so does the crawler-facing `<head>` — the `og:*` / `twitter:*` block, `<meta name="description">` and `<meta name="theme-color">` — which a crawler reads without running JS and which no runtime code in the page can repair. That rewrite happens in the proxied response, so it does nothing at all for a hostname Firebase Hosting still serves directly: the bundle's baked block is what such a host emits, and the trusted Vacay static fallback is what keeps today's Bodega hosts correct. Do not attach a non-Vacay Edition hostname to a wildcard without confirming, on that host, that `curl -s https://<host>/ | grep og:site_name` names its own Edition — the rewrite is the reason it can be attached, and a host that answers before the route exists is a host serving the wrong brand to every group chat it lands in.
- **Sign-in on a newly provisioned host: shipped in code, verification still owed (#852).** The [ADR 0010](../docs/adr/0010-centralised-auth-origin-with-handoff.md) handoff is the Five Across default since #547: the `fiveacross` target builds with `VITE_AUTH_MODE=handoff` and `VITE_AUTH_HANDOFF_ORIGIN=https://auth.fiveacross.app`, `auth.fiveacross.app` is provisioned on the live project, and `resolveSignInStrategy` (`src/auth/authMode.ts`) routes a hostname with no direct same-origin route (not a registered first-party host, not a build whose `authDomain` equals the serving hostname, not a local or `web.app` redirect host) through that origin whenever the build is in handoff mode with a valid handoff origin, so an arbitrary new `*.fiveacross.app` label served by the handoff-mode Five Across build reaches a sign-in button with no change to `src/**` and no redeploy (a `same_origin` build or an invalid handoff origin reports `unavailable` at mount instead) (pinned by the production-wildcard-handoff case in `scripts/fiveacross-origin.integration.test.mjs`). `FIRST_PARTY_AUTH_HOSTS` is now only the direct same-origin override for the registered first-party hosts, not the source of truth for reachability. What #852 still owes is human verification: a real sign-in on a freshly provisioned wildcard host on all four platforms, which cannot be done from a checkout.
