...
The validator parses the **whole** configuration with a TOML parser (`smol-toml`) and then judges the parsed object, rather than scanning for lines that look like a binding. That matters because Wrangler reads a `services` array from a named environment too, and `CLOUDFLARE_ENV` selects an environment without a command-line flag — so a second, control-plane binding written under `[env.staging]`, as a dotted `env.staging.services = […]`, or inside an inline `env = { … }` would deploy while a line scanner reported one clean binding. The rule is therefore counted across the whole document: **exactly one** service binding may exist in this file, it must be declared at the top level, and it must be the lookup-only one. Repeating even the identical binding under `[env.<name>]` is refused, because a second copy is a second thing to keep correct that no reviewer of the top-level block would see; wanting a routed environment changes what this gate claims and belongs in [`specs/event-router-registry.md`](../specs/event-router-registry.md) first. Anything the parser cannot read — an undefined escape, a duplicated key, an `env` that is not a table of tables, a `services` value that is not an array of tables — is refused rather than skipped, because "unreadable" and "absent" must not be the same answer in a capability check.
...
**Before it installs or publishes anything**, it parses the committed `wrangler.toml` through the shared validator in `scripts/event-router-registry/harness-config.mjs` and exits `65` unless the file's one and only service binding is `REGISTRY`, at the top level, bound explicitly to `RegistryLookupEntrypoint` — environments included, per § The registry lookup binding. The binding travels with the upload, so the committed configuration *is* the deployed configuration — which is exactly why the check can, and must, run while nothing has changed yet. The validator is a Node module that imports a root devDependency, so run `npm ci` at the repository root before deploying; it exits non-zero and names the missing install rather than reporting a binding problem that does not exist.
