...
It also REWRITES one response rather than answering it: the proxied `index.html`'s `<head>`, per hostname ([#1118](https://github.com/nathanjohnpayne/fiveacross/issues/1118)). A link-preview crawler runs no JavaScript, so the `og:*` / `twitter:*` block, `<meta name="description">` and `<meta name="theme-color">` a multi-Event bundle baked at build time have no runtime repair path; `HTMLRewriter` replaces them as the document streams, with `og:url` set to the requested hostname's own origin (a per-Event fact no build can bake) and the theme colour read back out of the same manifest builder, so the two stay exactly equal per Edition. It is gated on the same resolution as the manifest route and on a `200` HTML body, so no fail-closed outcome is rewritten (those responses are generated by the router before the proxy path runs, so there is no origin body to relay), and the `/__/auth/*` exemption, an origin 4xx or 5xx response, a `206` answer to a `Range` request and every proxied non-HTML asset are relayed untouched — a partial representation especially, because rewriting inside a window its own `Content-Range` frames by byte offset is how a resumed download reassembles a corrupted resource. Relayed means an origin body passed through: `/manifest.webmanifest` and the path capability are constructed by the router, and a rejected origin fetch becomes the router's own `502`, so none of those is relayed either. What a DOCUMENT subrequest asks for is the other half of getting it right, and one predicate decides it. A request is a document candidate when it is a `GET` or a `HEAD` and either HTML is acceptable to it with a positive quality, at any path — `text/html` or `application/xhtml+xml` outright, or `text/*` when neither is named, in each case with a `q` above zero, because `application/json, text/html;q=0` names HTML only to refuse it — or its `Accept` states no preference at all (absent, or nothing but a `*/*` it is willing to receive) and the path is shaped like a document — the root, an extensionless path, or one ending in `/` or `/index.html`. That second arm IS the crawler arm: `facebookexternalhit`, `Twitterbot`, `Slackbot`, `LinkedInBot`, `Discordbot` and the iMessage fetcher name no media type, so without it the clients this rewrite exists for would be the ones it never reached. A candidate reaches the origin without its `If-None-Match` / `If-Modified-Since`, because the origin's baked `index.html` is unchanged after a hostname is repointed and its truthful `304` would hand the client back the previous Edition's unfurl with no body to rewrite; without its `Range` / `If-Range`, because the rewrite refuses a `206` and a relayed one would give this URL a second, baked representation of a different length from the rewritten one every ordinary `GET` receives, which a client resuming or assembling the document would splice together; and with `Accept-Encoding: identity`, because the runtime negotiates `br, gzip` on a subrequest by itself and an origin that honours it answers with bytes no HTML parser can read — the rewrite would then match nothing, change nothing, report success and serve the baked Edition. The rewritten response carries neither the origin's `ETag` nor its `Last-Modified`, so that window closes after one fetch instead of recurring, and no `Content-Encoding` and no `Vary: Accept-Encoding`, neither of which describes the bytes it now contains. A `200 text/html` that still arrives encoded is relayed whole rather than parsed, so a failed negotiation costs the rewrite rather than the response. Everything outside the predicate — a request naming a non-HTML media type, and a path with a file extension asked for with a wildcard — reaches the origin exactly as it arrived, so an asset keeps its validators, its cheap `304`, its `Range` and its compression, and an asset is where a `206` can still arise.
