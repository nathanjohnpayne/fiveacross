...
A correct router is not sufficient. Four properties block the cutover, each in a different way. One is code that does not exist yet, with its own follow-up issue — [#1118](https://github.com/nathanjohnpayne/fiveacross/issues/1118); one is shipped code whose live verification is still owed under [#852](https://github.com/nathanjohnpayne/fiveacross/issues/852); one is evidence still owed under [#888](https://github.com/nathanjohnpayne/fiveacross/issues/888) now that #972 has shipped its router half; and the first is an ORDERING constraint on shipped code, the newest of the four.
...
`lookup-unavailable` remains the only reason that is about the router rather than about the address: the registry binding is absent, or the service call rejected or exceeded the 2,000 ms bound. Seeing it on **every** address, including `/__/auth/*`, points at the binding — the registry service missing, or the `[[services]]` block wrong. Seeing it intermittently points at the registry Worker or its Durable Object. Seeing `replica-malformed` on one host points at that host's committed projection and should page: it will not heal on a retry. Seeing it on **every** committed host at once, immediately after a registry deploy, points instead at envelope skew: each lookup result carries the `schemaVersion` its record was committed under and the canonical `host` it was projected from, the router refuses any version outside the exact set its own build interprets and any hostname that is not byte-exactly the one it asked about, and both checks run before it reads the projection. The fix is to ship the matching router rather than to touch the registry. The canonical-host binding (#1133) is not skew-tolerant in either direction — an older router refuses the field and a newer one requires it — so the registry and the router are deployed together, registry first, and neither is rolled back alone.
