...
It also REWRITES one response rather than answering it: the proxied `index.html`'s `<head>`, per hostname ([#1118](https://github.com/nathanjohnpayne/fiveacross/issues/1118)). A link-preview crawler runs no JavaScript, so the `og:*` / `twitter:*` block, `<meta name="description">` and `<meta name="theme-color">` a multi-Event bundle baked at build time have no runtime repair path; `HTMLRewriter` replaces them as the document streams, with `og:url` set to the requested hostname's own origin (a per-Event fact no build can bake) and the theme colour read back out of the same manifest builder, so the two stay exactly equal per Edition. It is gated on the same resolution as the manifest route and on a 2xx HTML body, so every fail-closed outcome, the `/__/auth/*` exemption, an origin failure and every non-HTML asset are relayed untouched.
