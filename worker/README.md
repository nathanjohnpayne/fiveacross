...
| `src/manifest.ts` | The per-hostname PWA manifest response (#546) — the router's one constructed body besides the fail-closed page. |
...
A correct router is not sufficient. Four properties block the cutover, each in a different way. Three are code that does not exist yet, with their own follow-up issues — [#1118](https://github.com/nathanjohnpayne/fiveacross/issues/1118), [#852](https://github.com/nathanjohnpayne/fiveacross/issues/852), [#888](https://github.com/nathanjohnpayne/fiveacross/issues/888). The first is an ORDERING constraint on shipped code, and it is the newest one.
