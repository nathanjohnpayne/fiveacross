...
```bash
npm ci                        # ROOT: the deploy guard's TOML validator (smol-toml)
npm --prefix worker ci        # pinned wrangler, from the committed lockfile
npm run worker:deploy         # from the repo root
```

Both installs, and in that order. The guard verifies the registry binding **before** it installs the Worker toolchain, and it does that by parsing `wrangler.toml` with `smol-toml`, a root devDependency — so on a fresh deployment checkout a missing root install stops the deploy at the guard rather than at the publish. `npm ci`, never `npm install`: both lockfiles are part of the reviewed deploy.
