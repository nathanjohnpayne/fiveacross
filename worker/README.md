...
- **Firestore App Check enforcement needs a compatible lookup design.** This router intentionally has only the Firebase web API key, no App Check token and no service-account credential. Enforced Cloud Firestore rejects unverified REST reads, so an uncached hostname lookup would fail closed even though the document remains world-readable by rules. Before routes are attached while Firestore App Check is enforced, #888 must provide a least-privilege compatible path; do not weaken enforcement merely to make the route work.
