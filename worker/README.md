...
`npm run worker:deploy` therefore verifies it as part of deploying, and **exits non-zero when the binding is absent or cannot be inspected** — an uninspectable Worker is a failed verification, not a skipped one, so automation cannot record an unverified deploy as a verified one. The name is compared for exact equality, so a near-miss binding like `OLD_FIREBASE_API_KEY` does not satisfy it. On a **route-bearing** deploy (one where `routes` is uncommented, i.e. the cutover itself) the check runs **before** publishing as well as after, because by then a failure would already have changed live traffic. To check by hand at any time:
