...
describe('registry sync request contract', () => {
  ...
  it.each([
    ...
    [
      'extra desired field',
      { ...route(), desired: { ...route().desired, canonicalHost: SYNTHETIC_HOST } },
    ],
    ...
    [
      'slug/host mismatch',
      { ...route(), desired: { ...route().desired, slug: 'different-slug' } },
    ],
    [
  ...
  it('requires the exact application/json content type', () => {
    expect(() => parseSyncRequest(JSON.stringify(route()), 'application/json; charset=utf-8')).toThrow(
      'content-type',
    );
  });
  ...
  it('admits only the guarded synthetic root-test class with a null path capability', () => {
    ...
    const payload = route({
      host,
      desired: { kind: 'root', root: 'not-found', edition: 'vacay', pathNamespace: null },
    });
    ...
    const ordinaryRoot = route({
      ...
      desired: { kind: 'root', root: 'doorway', edition: 'vacay', pathNamespace: null },
    });
...
describe('projection digest', () => {
  it('uses the one exact route array and an independently pinned SHA-256 literal', async () => {
    ...
    expect(await projectionDigest(route())).toBe(
      '99bf68a026a95b1138b7c4817574612436f29c8ae0ea1fe8fd623012026f9755',
    );
  });
  ...
  it('keeps tombstones distinct from routes and roots', async () => {
    ...
    const root = route({
      ...
      desired: { kind: 'root', root: 'not-found', edition: 'fiveacross', pathNamespace: null },
    });
