...
This is not a theoretical objection. [#844](../../issues/844), filed from PR #838's review, reached it independently and from the other direction: `resolveAbuseEscalation` (`functions/src/bugReports.ts`) gates an admin alert on the reporter holding a Player document, and Codex correctly observed that the check is a fact the client asserts rather than one the platform knows. That issue's own conclusion — *"Some Event-scoped notion of legitimate participation that a client cannot mint"* — is what this spec specifies, and its first listed candidate ("a server-written membership fact — join goes through a callable that records something clients cannot write") is the shape adopted below.
...
`src/data/eventMembership.ts` exports `membershipPath()` and `membershipRulesPath()` so the client, the Functions, the seeds and the two rules transcriptions all derive that string from one place.
...
|---|---|---|---|---|
| `boards` create+update — **reshuffle** | `1106-1148` | **9 / 5** | **12 / 6** | **Over the 10-call limit** |
...
**1. The reshuffle board write has one call of headroom, and the predicate needs three.** `reshuffleOk()` already costs nine textual calls across `reshuffleCounterPaired` (`:199-203`), `reshuffleSpendMarked` (`:225-228`) and the path-constructor-that-is-secretly-a-read `reshuffleSpendMarkerDoc` (`:222-224`); adding the predicate takes it to twelve. It survives today only because it collapses to five distinct (path, operation) pairs, and the spec will not spend a "may be cached". **The recommended shape gates the create and not the mutation**: append `&& (resource != null || admitted(eventId))` to the existing clause. A reshuffle is an update, so `resource != null` short-circuits it to zero added calls, while a first deal pays the full three on top of one, for four. This is sound because a board that exists was created by someone who passed the create gate — and because `boards` is already `isOwner(uid) || isAdmin(eventId)` on every arm (`:1105-1149`), so an ungated update only ever touches the caller's own board. The residual is a non-member who dealt a board *before* the flip and keeps updating it, which is #805's rollout problem and is bounded to their own document. **Generalise the principle:** gate the create, not the mutation, wherever the document's existence already implies an earlier gated create.
