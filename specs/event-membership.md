...
1. **Invitation redemption** (#803) — a callable that consumes a single-use invitation and writes the membership in the same Firestore transaction that reads it. **Redemption is create-only: it writes the membership document only when none exists at that path** (Codex P2 on PR #891). "Writes the deterministic path" alone would be satisfied by a `set`, and a `set` re-invites a REVOKED uid straight back to `'active'` — silently undoing a revocation and overwriting the grant provenance (`grantedBy`, `grantedAt`, `invitationId`) that records who admitted them and on what authority. § Glossary keeps revoked records precisely so re-invitation cannot quietly re-admit; a blind write there would delete the evidence and the decision in one operation. If reactivation is ever wanted it is a DIFFERENT operation with its own audit fields and its own decision, not a side effect of redeeming an invitation — and the transaction must read the existing record to tell the two apart, which it can do at no extra cost because it is already reading that path, exactly as `exchangeAuthHandoff` consumes a handoff code (#548 / PR #842: *"a Firestore document rather than a stateless signed blob because single-use cannot be enforced statelessly"*). `invitationId` records which invitation was burned.
...
The model instead requires the invariant **admission is a precondition for privilege**: every uid in `admins` holds an active Membership. `adminsMissingMembership()` returns the violating set, which is precisely the set of Admins a flip would lock out of their own Event, and #805 must drive it to empty before enforcing. The break-glass if one is missed is the enforcement switch — **and, once D-A is deployed, its transitional disjunct as well.** An earlier draft of this sentence said "not a rules clause" and sat immediately above the section prescribing exactly that rules clause (Codex P1 on PR #891); it predates D-A and is corrected rather than deleted, because it remains true of the final posture, which is what the rest of this paragraph describes. A transitional `|| isAdmin(eventId)` disjunct during rollout is the ruled shape (§ Decisions, D-A): #804 ships it, and a follow-up removes it once #805's backfill is verified on both Events.
...
Firestore's published limits are **10 `exists()`/`get()`/`getAfter()` calls per single-document request *and* per query request**, 20 for multi-document reads, transactions and batched writes *with the 10 still applying to each operation inside them*, and **1,000 evaluated expressions per request**. Cloud Storage's cross-service limit is different in kind: **no more than two Firestore ACCESS CALLS per rules evaluation**. Both count calls; what differs is the ceiling (10 vs 2), not the unit. **Saying Storage "counts documents" was wrong and actively dangerous** (Codex P2 on PR #891), and it contradicted this spec's own correct statements at § The shared predicate and § the two files deliberately differ. An implementer budgeting from the wrong unit reasons "the Event document plus the membership document is two documents, so it fits" — then writes two Event reads plus one membership read, spends three ACCESSES on two documents, and the rule denies every valid request. That is exactly the three-call error this spec already made once and fixed; the summary line was the copy that never got corrected.
...
Both authorize through `isEventAdmin(event, uid)` (`functions/src/unlockDay.ts:479`), which is `event.admins.includes(uid)` and nothing else. A UID added to the array by any existing Admin can therefore unlock a Day or rewrite a Day's snapshot while holding no admission at all. Both conjoin an active-membership check against the caller's own record, per § The role model's general rule, and both are named in § Acceptance criteria so the requirement is testable rather than implied.

**That check belongs INSIDE the committing transaction, not at the current pre-flight authorization site** (Codex P2 on PR #891). Saying only "conjoin a membership check" would have been satisfied by adding it where `isEventAdmin()` is called today — before the transaction opens — which does not keep the promise D5 makes. The Admin SDK bypasses rules, so nothing re-checks at commit: an Admin whose membership is revoked *after* the pre-flight read and *before* the write commits still lands the write. Both `stampDaySnapshot` and `resnapshotDayIfNoBoards` already re-read the Event document inside their transactions, so the live `admins` roster and the caller's membership must be read and checked **there**, in the same transaction that commits. A pre-flight check is an optimisation that fails closed early; it is not the authorization decision. Neither takes Decision D-A's transitional bypass: like #803's invitation callable, these MINT or REWRITE state that outlives the unenforced window, and § The role model scopes D-A to the two rules surfaces.
...
**Revocation does nothing until its Event is enforced, and that has to be said out loud** (Codex P2 on PR #891). Every predicate here short-circuits on the switch — `admits()` returns `admitted-unenforced` before it ever inspects the record, and both rules forms do the same — so flipping a record to `status: 'revoked'` during § Rollout steps 3 through 8 removes nobody. The hazard is not the delay; it is that an Admin performs a revocation, receives success, and reasonably believes the person is out while that person keeps reading and writing until the flip. D5 rules that revocation stops future reads and writes; a silent no-op that reports success contradicts the ruling in the direction most likely to be discovered by the wrong person. #803 therefore either withholds the revoke operation until its Event is `'enforced'`, or performs it and reports plainly that it takes effect at enforcement — but it must not present a no-op as a completed removal. The record written now is still correct and still what the backfill and the flip consume; it is the *reporting* that must not overstate.
...
8. **Ship the Storage gate and verify it at the emulator (#806) — BEFORE any flip, not after** (Codex P1 on PR #891). Step 6 deploys only `firestore.rules`, and an earlier draft of this list went straight from the backfill to the flip. Followed literally it produced an Event advertised as `'enforced'` with **half its boundary ungated**: Firestore content isolated while `storage.rules:30` still lets every signed-in account read every Event's proof media. Proofs are photographs players took, so that half is the more sensitive one, and "enforced" would have been a claim the system did not keep. The flip is not a Firestore milestone; it is the moment BOTH files must already agree, and the emulator run that proves the Storage predicate denies a non-member is the gate on reaching step 9 rather than a follow-up to it.
9. Flip that Event to `'enforced'` — a server-side write, since the freeze step (4) made the field non-client-writable. Flipping back is one field and is the break-glass.
...
- **Given** an Event with an Admin who holds no membership **and Decision D-A's transitional disjunct NOT deployed (the final posture)**, **when** the predicate runs, **then** the Admin is **not** admitted; `adminsMissingMembership()` names them, and #805 must return the empty set before that Event is enforced.
- **Given** that same Admin **while D-A's disjunct IS deployed**, **when** the predicate runs, **then** the Admin **is** admitted, with the outcome distinguishable as `admitted-admin-transitional`. Both criteria are stated because an emulator test written from the first one alone asserts the opposite of the behaviour #804 is required to ship, and would lock out exactly the backfill-missed Admin D-A exists to protect (Codex P1 on PR #891).
