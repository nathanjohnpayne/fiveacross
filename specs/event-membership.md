...
**This lands on the rule that is already over the expression cap**, alongside the `membershipEnforcement` immutability clause from finding 2. Both additions target `events/{eventId}` update, which #850 shows already fails at ten Days — so #850 is not merely a prerequisite of #804, it is the prerequisite that two separate parts of this design queue behind.
...
3. **Freeze the switch before anything reads it** (#804, deploy 1): ship the immutability clause alone, so no client can write `membershipEnforcement`, while no rule yet consults it. This deploy changes no behaviour.
4. **Scrub and verify**: assert that every Event document carries `membershipEnforcement` absent or `'off'`. **This step is not optional and the window it closes is real** (Codex P1 on PR #891). Between this spec landing and step 3, the field exists in the contract while the Event update arm still accepts arbitrary writes to it — so an Admin can set `'enforced'` today. Shipping the freeze and the gate in one deploy would then freeze that pre-set value *and* immediately honour it, enforcing an Event before its backfill and locking out its whole cohort — the exact outage the switch exists to prevent, delivered by the deploy that was supposed to be a no-op.
5. The rules ship **with the switch consulted and every Event off** (#804, deploy 2), after #850 and #807. Now a no-op for both live cohorts, because step 4 proved there is nothing to honour.
6. Backfill each Event and drive `adminsMissingMembership()` to empty (#805).
7. Flip that Event to `'enforced'` — a server-side write, since step 3 made the field non-client-writable. Flipping back is one field and is the break-glass.
...
Epic #801 surfaced nine. Four are answered here because the code forces them, one has been ruled on by the owner, and the rest are still open — plus **D-A**, which this spec adds. Nothing in § Contract depends on the open ones.
