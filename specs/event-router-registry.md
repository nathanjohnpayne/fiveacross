...
Each page contains at most 100 recovery records ordered by sequence. `nextAfter` is the last returned sequence only when more records exist; the command follows it to `null` and includes every record in its audit output. `lookup` is computed by the same point-lookup helper as the private service binding without exposing its payload. For each response the per-host DO returns `requestId = base64url(nonce || HMAC-SHA256(doProbeKey,[1,host,lookup,observedAt,probeLocation,nonce]))`; `probeLocation` is a validated closed deployment-region identifier supplied by the probe harness, and the DO key is created with the host state, never exported, and is not an account credential. `confirm-block` verifies the MAC, exact SafetyProbe fields, age at most five minutes, three distinct locations/receipts, and current safety-block revision before accepting them. The endpoint exposes no route payload, list, or mutation. Pagination over hostnames exists only on the Firestore source list; recovery pagination remains inside one already-named host. Audit-token theft permits guessed point reads of public metadata and that host's break-glass evidence, not enumeration or mutation.
