...
| Source attestor | one transactionally consistent Firestore source/ledger read supplied by the human command | sign that exact fresh source observation with a human-only keyless identity/key | invoke recovery; sign an invented projection without the interactive source read |
| Recovery operator | one separately attested Firestore source/ledger observation and one compared DO state | lock, monotonic source-attested repair, audit history, and a separately authorized exact-host WAF rule | use the publisher/source-attestor identity or key; lower a revision; invent a projection; attach routes |
...
The exact audience is the separately deployed registry service's `workers.dev` origin plus `/__internal/hostname-replicas/v1`; it is immutable deployment configuration on both sides. That service has no wildcard route. Its public `fetch` surface admits only the signed sync, audit, recovery, and regional-probe attestation paths below; point lookup is an RPC/service-entrypoint method available only through the Event router's Cloudflare service binding. The Event router's own `workers.dev` behavior remains out-of-Namespace. Local tests inject token/signature acquisition and verification; no test key or bypass compiles into production.
...
Recovery records live under sequence-addressed keys in the same object's storage rather than in an unbounded array. A transaction that changes committed/lock state increments `recoverySequence` and writes the corresponding record atomically; no path updates or deletes an existing record. Short-lived regional-probe attestations use a separate key prefix, expire after five minutes, and are consumed atomically by the `acquire-lock`, `clear-lock`, or `abort-lock` action that accepts them; they are evidence inputs, not recovery history.
...
type SourceAudit = { revision: string; digest: string; observedAt: string; payload: RouterReplicaDesired; attestorSub: string; attestorKeyVersion: string; attestationIssuedAt: string; attestationSignature: string };
type ProbeObservation =
  | { phase: 'blocked-before-worker'; observedAt: string; rayId: string; host: string; expectedStatus: 403; observedStatus: 403 }
  | { phase: 'canonical-after-unblock'; observedAt: string; rayId: string; host: string; expectedStatus: number; observedStatus: number; expectedReason: null | 'inactive' | 'unknown-host'; observedReason: null | 'inactive' | 'unknown-host'; expectedRevision: string; observedRevision: string; expectedServesOrigin: boolean; observedServesOrigin: boolean; originRequestId: string | null };
type WafEvidence = { zoneId: string; rulesetId: string; ruleId: string; host: string; verifiedAt: string; providerRule: { enabled: true; action: 'block'; expression: string; responseDigest: string }; probeAttestationIds: [string, string, string] };
type PublisherReplacement = null | { quarantinedEpochCeiling: string; nextPublisherEpoch: string; replacementSubject: string; replacementKeyVersion: string; registryConfigDigest: string };
type RecoveryAction =
  | { kind: 'acquire-lock'; wafEvidence: WafEvidence }
  | { kind: 'apply'; lockId: string; publisherReplacement: PublisherReplacement }
  | { kind: 'clear-lock'; lockId: string; wafRemovedAt: string; probeAttestationIds: [string, string, string] }
...
Recovery uses `POST /__internal/hostname-replicas/v1/recover` with exact `application/json` and the `RecoveryRequest` union above. A different human-impersonated recovery service account, audience, and dedicated KMS signing key authorizes the complete request; the publisher has neither. Independently, the human command transactionally reads `hostnames/{host}` plus `routerReplicas/{host}` with its interactive Admin credential, then impersonates a human-only source-attestor service account and asks its separate KMS key to sign `v1\nsource-audit\n<host>\n<revision>\n<digest>\n<observed-at>\n<attestation-issued-at>\n<sha256-of-exact-payload>`. The registry pins a source-attestor subject/key/audience distinct from publisher and recovery, verifies its short-lived OIDC token from `X-Source-Attestor-Authorization: Bearer <token>` plus the embedded signature, requires the signed attestation time within 60 seconds, and never persists either token. Thus recovery authority alone cannot mint source evidence.

The complete request body is bounded to 16 KiB before parsing and the recovery signature covers every expected state, action, source audit/attestation, incident URL, and reason. `sourceAudit` must be no older than five minutes nor future; its payload must hash to its digest, carry the same host/revision, and byte-match the exact ledger projection observed by the source-attestor command. A nonempty HTTPS incident URL and reason are mandatory. Extra/missing fields, the same subject/key used for both authorizations, or evidence on the wrong action are rejected.

Public recovery evidence comes only from three pre-provisioned regional probe runners. Each runner has a distinct keyless service identity and dedicated KMS signing key mapped in immutable registry configuration to exactly one deployment region. It makes the real public request, then within 60 seconds submits the exact `ProbeObservation` to `POST /__internal/hostname-replicas/v1/probe-attest` with its own OIDC token and KMS signature. The registry derives the region from the accepted subject/key mapping—no body/header may name it—and the per-host DO stores a random one-use attestation ID with the observation, region, subject, key version, and receipt time. Recovery accepts exactly three fresh IDs with distinct subjects, regions, nonces, and Ray IDs; the phase/host/result must match the action and committed state. One runner cannot mint another region's proof, and no sampled analytics or caller label participates.
...
1. **Contain and acquire.** The human command installs an exact-host Cloudflare WAF block with a separate 1Password-backed token limited to `Zone WAF Edit` on the two served zones. Its nonsampled configuration API readback must name the exact zone/ruleset/rule, enabled `block` action, and canonical expression `http.host eq "<normalized-host>"`; the signed response digest and three fresh `blocked-before-worker` regional attestation IDs form `WafEvidence`. The DO revalidates those stored attestations, exact rule fields, expected committed state, and source freshness, consumes the IDs, then atomically creates `recoveryLock`. Publisher sync is rejected for the entire lock lifetime. If state changed before acquisition, CAS fails and the operator starts from a new audit.
2. **Apply canonical source.** `apply` retains the lock and atomically replaces committed state plus appends history. It may repair a different payload at the equal revision or jump to a higher source revision, recording every skipped revision range; it never lowers the high-water mark. If the DO is ahead of Firestore, recovery fails `409 source-behind`: an explicit Admin recovery transaction must first advance `routerReplicas/{host}` beyond the DO revision with the current canonical hostname projection, then a fresh signed audit may apply it. An equal-revision repair or any incident where publisher integrity is not proven requires `publisherReplacement`: `quarantinedEpochCeiling` must be at least `highestAuthenticatedPublisherEpoch`, `highestQuarantinedPublisherEpoch`, and every epoch the quarantined runtime could access in the signed registry/IAM evidence, while `nextPublisherEpoch` must be greater than that ceiling and map to a replacement subject/CryptoKey unavailable to the old runtime. The registry verifies `registryConfigDigest` against its active epoch mapping and the recovery history stores the signed configuration/IAM evidence digest. The same transaction raises `highestQuarantinedPublisherEpoch` to the ceiling and `minimumPublisherEpoch` to `nextPublisherEpoch`; it does not claim the replacement epoch was authenticated until a replacement sync arrives. Pre-signed requests from every quarantined-accessible epoch are then rejected even when they carry a higher revision. A simple missing-delivery jump from an otherwise trusted publisher may send null.
3. **Prove and clear.** Before WAF removal, private audit must show lock held, source = committed, no drift, and any required replacement publisher epoch active while every quarantined-accessible epoch is fenced. The command removes the exact-host rule while the lock still rejects all publisher changes, then obtains three `canonical-after-unblock` regional attestation IDs. A failure reinstalls the WAF rule and keeps the lock. `clear-lock` requires fresh source = committed and atomically validates/consumes three distinct fresh attestations whose host/result/revision equal committed state, appends them, and clears the lock. Queued publisher retries may proceed only afterwards; if Firestore advanced during recovery, the fresh-source compare prevents clear until that revision is applied.
4. **Abort without mutation.** `abort-lock` is allowed only when a fresh source audit exactly equals committed state and new `WafEvidence` independently re-proves that the exact-host rule still blocks in three regions. The transaction consumes those attestations, records the evidence/reason, and clears the lock without changing committed state or bypassing the monotonic rule, but it does not authorize WAF removal: public traffic stays contained. Returning that host to service requires a new fresh `acquire-lock` followed by the same canonical `clear-lock` evidence path; removing the WAF directly after abort is a contract violation.

The recovery history record includes action (`acquire-lock|apply|clear-lock|abort-lock`), before/after, skipped range, source audit and attestor identity/key/signature, publisher epoch/config evidence, WAF rule/config/probe evidence where required, public probe results, recovery-operator subject/key version, incident URL, and reason. It excludes OIDC tokens, is append-only, and is paginated. No adaptive analytics or sampled log is an admission source. A compromised publisher cannot invoke recovery; a compromised recovery identity still lacks the independent source-attestor signature needed to write a fresh, non-decreasing projection for one compared host.
...
1. **R0 — resources and IAM:** provision private Firestore collection/rules; separate registry service; registry-owned DO namespace/migration with exact `wnam` location hint; point-only router service binding; publisher/audit/recovery/source-attestor plus three regional probe identities/audiences; dedicated publisher/recovery/source-attestor/probe CryptoKeys; rate limiter; exact-host WAF capability; and observability. Verify the publisher has only event/logging plus use-to-sign on its one key, recovery/source-attestor/probes only their own keys, audit no data/signing role, source attestation requires interactive human impersonation rather than a runtime principal, no user-managed key/Secret Manager/Cloudflare credential, and the public router no DO/KV/Firestore binding.
...
3. **R2 — failure/recovery and data-plane rehearsal:** prove auth/signature/epoch rotation/replay/gap/conflict, old-epoch higher-revision refusal, transaction response loss, concurrent sync/recovery-lock races, provider-rule plus regional-runner WAF evidence, equal-repair/higher-jump/lower-refusal/source-behind recovery, failed-probe relock, hibernation/reset, DO storage/unavailability/malformed state, random-valid-host floods, pre-provision unknown lookup with fixed placement followed by publisher admission, non-enumerable binding, exact path capability, auth pass-through, and no Firebase/KV/cache request. Execute the exact remote cold/warm method and numeric thresholds above, then run five passes 30 seconds apart from every probe region against active/inactive/root/tombstone/unknown hosts and record revision/result equality, timeout headroom, object/error/cardinality cost, and spend. The single DO state is strongly consistent; these probes prove the synchronous public path and latency, not replica convergence.
...
- Recovery tests: distinct recovery and human-only source-attestor identities/keys, exact Firestore source/ledger attestation bytes/freshness and cross-role refusal, nonsampled exact-rule readback plus three independently attested blocked probes before lock, publisher quarantine/epoch ceiling raise and old-epoch higher-revision rejection, source advance while locked, distinct one-use regional canonical attestations, failed probes keep/reinstate containment, clear only on fresh source = committed, abort requires fresh blocked evidence and leaves WAF containment until reacquire-to-clear, forged/stale/wrong-host/duplicate-runner evidence, and monotonic replay refusal.
...
- Registry/router contracts: only `lookup(host)` crosses the binding; router has no DO/KV/list; registry public fetch has no lookup/list; route/root/capability, failure table including namespace-valid auth bypass, no Firebase/Cache API, auth ordering, revision header, namespace-before-binding, malformed/unavailable DO, fixed location hint on every access, and random-host rate/cost controls.
...
- Remote unrouted integration: exact cold-marker/warm-sample thresholds plus the complete R2/Gate 3 regional probe and cost matrix with App Check enforced; disabling it is never setup.
