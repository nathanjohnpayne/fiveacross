...
Normal sync first rejects any non-null `recoveryLock` with retryable `503 recovery-locked`, then applies this table inside one DO-storage transaction:
...
The recovery history record includes action (`acquire-lock|apply|clear-lock|abort-lock`), before/after, skipped range, source audit, WAF rule/config/probe evidence where required, public probe results, operator subject, key version, incident URL, and reason. It is append-only and paginated. No adaptive analytics or sampled log is an admission source. A compromised publisher cannot invoke recovery; a compromised recovery identity can write only a fresh, source-attested, non-decreasing projection for one compared host.
...
| publisher token/body signature rejected | sync `401` | Function throws; no state change |
...
- Durable Object transaction tests: initial 1, successor/equal/lower/gap/conflict, response loss, concurrent arrivals, recovery lock versus queued publisher, equal repair, higher jump with skipped-range audit, lower/source-behind refusal, lock persistence across hibernation/reset, atomic append-only history/pagination, tombstone non-reuse, and no external storage call.
- Recovery tests: separate identity/key, exact Firestore source/ledger attestation and freshness, WAF-before-lock/CAS, source advance while locked, failed probes keep/reinstate containment, clear only on fresh source = committed, abort without mutation, forged/stale/wrong-host evidence, and monotonic replay refusal.
