...
The sync request is `POST /__internal/hostname-replicas/v1`, exact `application/json`, at most 2 KiB, and contains the four fields above with `updatedAt` encoded as RFC 3339. Extra/missing fields, a non-canonical hostname/revision, an unknown status/version/Edition/root/path namespace, a reserved or invalid label, a route whose `slug` does not equal the host's first label (Namespace apex exception preserved), an invalid body signature, and any non-POST method fail before storage. `pathNamespace` is explicit null or one of the two namespace apexes and is independent of route/root shape; non-null is valid only on the matching Namespace apex/brand mirror, never an Event subdomain or GCB host. Root shape is valid only on the apex/mirror classes defined by `specs/path-addressing-and-root.md` and forbids `eventId`, `status`, and `slug`. Validation reuses the router's Namespace, Slug, and Edition modules; it does not fork their lists. `edition`, root shape, and capability are copied because #546 and that accepted spec must render per-host identity and `/.well-known/fiveacross-path-capability` at this edge without a second App Check-incompatible lookup.
...
For comparison and audit, the projection digest is SHA-256 over the UTF-8 JSON array `[1, revision, host, "route", eventId, status, slug, edition, pathNamespace]`, `[1, revision, host, "root", root, edition, pathNamespace]`, `[1, revision, host, "tombstone"]`, or the recovery-only `[1, revision, host, "safety-block"]`; constructing that array is the only canonicalizer. Explicit JSON `null` prevents absent/capable values from colliding. The Durable Object is named from the normalized host. An uninitialized object accepts revision 1 only. Thereafter it compares the incoming canonical decimal revision with its stored high-water state:
...
Before applying this table, the DO checks its recovery latch. While `safetyBlock` is non-null, every publisher mutation returns retryable `503 recovery-safety-block`; only the recovery identity may confirm the block, apply the source-equivalent canonical value, and clear the latch through the sequence below. A queued publisher successor therefore cannot replace the fail-closed KV value even after the recovery operation clears `pending`.
...
2. Before yielding to the KV write, the DO enters [`blockConcurrencyWhile`](https://developers.cloudflare.com/durable-objects/api/state/#blockconcurrencywhile) and keeps that input lock through steps 2–3. Write the exact pending payload to KV. A failure leaves pending durable and returns `503`; retry resumes that same write. No publisher, audit, or recovery event may interleave with an active KV write owned by the current instance. If the callback reaches Cloudflare's 30-second limit, the object is reset and that execution is cancelled; a newly created instance observes only durable pending state, never a still-running abandoned call.
3. Under the same input lock, in one DO-storage transaction require the same pending record and predecessor, advance committed revision/digest/payload, and clear pending. A failure leaves the fence in place; retry may rewrite the identical KV value and finalize. If another identical resumption already cleared pending and committed the same revision/digest, finalization is an idempotent success.
...
```ts
type BlockProbe = { phase: 'blocked-before-worker'; location: string; observedAt: string; rayId: string; host: string; zoneId: string; ruleId: string; expectedStatus: 403; observedStatus: 403; expectedProviderAction: 'block'; observedProviderAction: 'block'; providerEventQuery: { dataset: 'firewallEventsAdaptive'; queriedAt: string; matches: 1; edgeColoName: string; responseDigest: string } }; type SafetyProbe = { phase: 'safety-block'; location: string; observedAt: string; registryRequestId: string; host: string; expectedReason: 'safety-block'; observedReason: 'safety-block'; expectedRevision: string; observedRevision: string; expectedServesOrigin: false; observedServesOrigin: false }; type CanonicalProbe = { phase: 'canonical-after-unblock'; location: string; observedAt: string; registryRequestId: string; rayId: string; host: string; expectedStatus: number; observedStatus: number; expectedRouterReason: null | 'inactive' | 'unknown-host'; observedRouterReason: null | 'inactive' | 'unknown-host'; expectedRevision: string; observedRevision: string; expectedServesOrigin: boolean; observedServesOrigin: boolean; originRequestId: string | null; providerEventQuery: { dataset: 'httpRequestsAdaptive'; queriedAt: string; matches: 1; edgeColoName: string; responseDigest: string } };
type ReplicaAuditResponse = {
  schemaVersion: 1; requestId: string; observedAt: string; probeLocation: string;
  host: string;
  durable: null | {
...
Each page contains at most 100 recovery records ordered by sequence. `nextAfter` is the last returned sequence only when more records exist; the command follows it to `null` and includes every record in its audit output. `lookup` is computed by the same point-lookup helper as the private service binding without exposing its payload. For each response the registry derives `probeLocation` from Cloudflare's server-populated `request.cf.colo` value, validates it as an uppercase three-letter IATA colo, and the per-host DO returns `requestId = base64url(nonce || HMAC-SHA256(doProbeKey,[1,host,lookup,observedAt,probeLocation,nonce]))`. No request field or header may override that location. The DO key is created with the host state, never exported, and is not an account credential. `confirm-block` verifies the MAC, exact SafetyProbe fields, age at most five minutes, three distinct server-observed colos/receipts, and current safety-block revision before accepting them. `clear-block` performs the same receipt/MAC/age/colo checks for each `CanonicalProbe`, requires its signed lookup fields to equal the canonical public observation, and verifies that probe's exact Ray ID through the authenticated provider event below. The endpoint exposes no route payload, list, or mutation. Pagination over hostnames exists only on the Firestore source list; recovery pagination remains inside one already-named host. Audit-token theft permits guessed point reads of public metadata and that host's break-glass evidence, not enumeration or mutation.

Repair uses a different human-impersonated recovery service account, audience, and Cloud KMS signing key; the publisher has neither. `POST /__internal/hostname-replicas/v1/recover` is exact `application/json`, has its own 16 KiB limit, and carries `{schemaVersion:1,host,expected:{committed,pending,safetyBlock:<the exact audit values>},action:{kind:'apply',replacement:<RouterReplicaDesired>}|{kind:'resume-pending'}|{kind:'abort-pending'}|{kind:'confirm-block',probes:[SafetyProbe,SafetyProbe,SafetyProbe]}|{kind:'clear-block',removedAt,probes:[CanonicalProbe,CanonicalProbe,CanonicalProbe]},sourceAudit:{revision,digest,observedAt},blockEvidence:null|{zoneId,ruleId,host,verifiedAt,probes:[BlockProbe,BlockProbe,BlockProbe]},incidentUrl,reason}` and the same signed-request envelope. Every request requires an exact DO-state compare, a nonempty HTTPS incident URL/reason, and an operator-attested Firestore audit no older than five minutes (nor future); `apply` additionally requires no pending and a source digest equal to its replacement. `resume-pending`/`abort-pending` require the compared pending to be at least one minute old and can begin only in a newly admitted event, never by interleaving with the current instance's input-locked KV write. Resume requires source audit to equal pending and takes over its stored payload. Only an abort whose source-audit revision and digest exactly equal stored committed may restore that payload; every other abort must first verify an exact-host Cloudflare block, then writes the KV safety-block value instead of restoring a possibly active committed value.

The human recovery command holds a separate 1Password-backed Cloudflare token with only `Zone WAF Edit` and `Account Analytics Read`, resource-limited to the Five Across and Vacay Bingo zones; it has no DNS, Workers, KV, routes, logs, or account-write permission, and no runtime receives it. For each distinct-location block probe, the command reads the exact Ray ID back from Cloudflare's authenticated `firewallEventsAdaptive` API and requires the expected zone, host, rule, `block` action, 403, provider-recorded edge colo equal to `location`, and exactly one event. For each canonical public probe it queries `httpRequestsAdaptive` for the exact Ray ID and requires exactly one event with the expected zone/host, provider-recorded edge colo equal to both `location` and the paired registry receipt, and edge status equal to the public observation. The three canonical probes must have distinct Ray IDs, receipt nonces, and provider-recorded colos; one public response cannot be copied across receipts. The provider records are the proof that each named public request reached the claimed edge location. The WAF event additionally proves the blocked request terminated before Worker/origin; separate negative log evidence would be retention/sampling-sensitive as well as redundant, so no log backend or credential is part of this contract. The command binds each closed observation and response digest into the recovery signature/history; arbitrary, caller-relabeled, duplicated, or missing provider events fail before admission. Admission atomically replaces the compared pending with a recovery-owned operation containing the output and exact history record. KV is written, then one transaction commits output/history, clears pending, and retains `safetyBlock` when used. Partial failure leaves the fence resumable. The semantic fingerprint is SHA-256 over every canonical request/evidence field plus operator `sub`, excluding expiring auth/signature envelope; a fresh authenticated signature with the same fingerprint resumes it.

While `safetyBlock` is set, the external rule stops public traffic before the Worker/point lookup and the DO rejects every publisher mutation before admission. `confirm-block` requires refreshed matching WAF evidence, the exact safety-block revision, and three authenticated audit lookups with distinct server-observed Cloudflare colos whose expected and observed host, reason, revision, and `servesOrigin:false` agree, then records them and `confirmedAt` atomically. Publisher sync and recovery finalization never clear the latch; canonical recovery `apply` requires that confirmation plus refreshed matching WAF evidence. The external block survives crashes through safety write, DO/history commit, canonical apply, zero-pending audit, and three-colo canonical DO/KV convergence. Only then may the operator remove it. `clear-block` requires pending null, source = committed = KV and three distinct public probes paired with authenticated registry receipts and provider events from distinct server-observed colos; the closed expected/observed host, status, reason, revision, serve, and origin results must equal each other, each receipt's signed lookup, the provider's Ray/colo/status record, and the stored canonical payload. It rejects wrong-phase, stale, duplicated-Ray, caller-relabeled, or mismatched evidence, records removal, and clears the latch atomically. A crash before removal stays blocked; a crash after removal has already-converged canonical KV/DO and may retry only the history/latch transaction. Recovery history is bounded/auditable and never deletable.
...
- **Ordinary update:** non-projected fields do not churn the edge. A status, Edition, root, or path-capability change produces one new revision. Root/route transitions and capability enablement obey `specs/path-addressing-and-root.md`'s archive transaction and deployment barrier. An `eventId` or `slug` change while active is rejected.
...
The resolver calls only the registry service binding's `lookup(host)` method, whose registry implementation performs the exact KV point read. The public router receives no KV namespace binding. From the same result it serves exact `GET /.well-known/fiveacross-path-capability` as `{schemaVersion:1,pathNamespace:'fiveacross.app'|'vacaybingo.com'|null,revision:string}` with `Cache-Control: no-store`; missing/malformed/inactive/tombstone/safety-block lookup fails closed and returns no capability, and no Event id/catalogue is exposed. Root markers are valid configured origins: `doorway` or `not-found` controls the app's `/` outcome, not whether the edge may serve its shell/capability. Both services remove the current `caches.default` positive envelope and add no application TTL, negative cache, stale-serve cache, or Firestore fallback. KV's own edge caches and eventual propagation remain the single cache layer. A write acknowledgement proves acceptance, not global visibility; operational convergence is established by probes. A region may continue to observe an older accepted revision during propagation, which is why an urgent withdrawal uses the independent Cloudflare block rather than waiting on KV. The existing strict digit parser keeps the lookup timeout at 2,000 ms by default and bounds the service-binding call as one operation.

The Namespace/reserved/Slug guard runs before the service binding, so foreign and structurally invalid traffic costs no registry/KV read. The exact well-known path is handled as reserved before slug parsing but after the host guard. Wildcard zones must have Cloudflare rate-limit/WAF rules for unknown-host floods before a route is attached, plus the reserved-label exclusions already required by `specs/event-router.md`. The registry's public sync/audit/recovery paths enforce exact route/method, content type where applicable, and the 2 KiB sync / 16 KiB recovery limits before JSON/JWT work; use Cloudflare's per-IP rate-limiting binding for invalid callers; cache valid JWKS by their response policy and cooldown; and never reflect validation details. Rate limits are an abuse signal, not an authentication substitute. Automated DDoS protection and account spend alerts remain enabled.
...
| publisher mutation while recovery safety block is latched | sync `503 recovery-safety-block` | recovery identity confirms, applies canonical source, and clears the latch; publisher never overwrites the fail-closed value |
...
| recovery takeover before pending is one minute old, or while its KV write still owns the DO input lock | `409 pending-not-stale`, or event remains unadmitted until the writer finishes/resets | wait or use the independent hostname block; never clear the fence |
...
- a synthetic active, inactive, root/path-capable, tombstone, and unknown host probe, with alerts carrying hostname, expected revision, observed reason, and router version.
...
3. **R2 — failure rehearsal:** automated/deployed checks prove unauthorized/expired/wrong-audience sync, stolen-bearer altered-body refusal, out-of-order/gap/equal-conflict revisions, pending resume/abort before and after KV/commit failure, a delayed KV write cannot interleave through recovery and is cancelled on the 30-second object reset, non-source abort refusal without verified WAF plus safety-tombstone fencing through canonical convergence/unblock, publisher refusal for the entire safety-block latch, stale/CAS refusal and durable recovery history, non-enumerable public binding, malformed KV, KV/JWKS/origin failures, unknown/inactive/tombstone/root hosts, exact path-capability behavior, auth passthrough, and no Firestore outbound request. Probes from at least three server-observed Cloudflare colos, five passes 30 seconds apart, must observe every serving host's expected revision; caller-supplied or relabeled locations fail verification. This is evidence, not a false global-consistency proof.
...
- Pure schema/hostname/revision parser tables, including decimal precision, reserved/apex cases, route/root/tombstone/safety shapes, pathNamespace validity/null, Edition validation, extra fields, and exact 2 KiB sync / 16 KiB recovery boundaries.
...
- Registry/public-router contract tests proving only `lookup(host)` crosses the service binding, the public router has no KV/list capability, and the registry's `fetch` surface exposes no public point-get/list route. Router tests also prove route/root handling, the exact no-store well-known capability response (and its inactive/malformed failure), no Firebase fetch or Cache API envelope, every failure-table row, auth bypass ordering, revision header, and namespace-before-binding ordering. Audit/recovery tests prove server-generated probe-receipt MAC, field/freshness/location/revision validation, and forged/replayed receipt refusal.
