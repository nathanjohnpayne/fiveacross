...
- **A re-proof preserves the original marker.** Attaching a Proof to an ALREADY-marked square refreshes `uid`, `eventId`, `displayName`, `itemText`, and the supplied Day identity with merge semantics while preserving the original `markedAt`: the who-list is chronological by FIRST mark, and re-stamping `now` would reorder it by proof-attach time. Merge semantics are load-bearing during #1072 because an old proof payload must not erase a migrated Event identity or the Tally Card fields. `attachProof` reads the existing marker INSIDE the transaction—alongside its other reads, because a Firestore transaction requires ALL reads before its FIRST write—and stamps `now` only when no marker exists yet.
...
- A signed-in Player may create their own attributed marker at `tally/{itemId}/markers/{uid}` (`{ uid, eventId, displayName, markedAt }`); unmarking deletes it. During the explicit compatibility cutoff only, a truly missing `eventId` is accepted for old clients and normalized server-side. A present mismatch or non-string value is denied.
...
- Marking a non-free Square adds a marker write at `tally/{itemId}/markers/{uid}` carrying `{ uid, eventId, displayName, markedAt }` in the SAME batch as the board + player writes; unmarking issues a `batch.delete` at that exact path instead.
...
- `proof_required` `attachProof` merge-writes the attributed marker at `tally/{itemId}/markers/{uid}` (`{ uid, eventId, displayName, markedAt, itemText, dayIndex? }`) in the SAME `runTransaction` as the proof + board + player (one `runTransaction`, no second write path).
