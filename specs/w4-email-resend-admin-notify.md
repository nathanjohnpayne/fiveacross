...
The trigger source is `onDocumentWritten`, so a Proof CREATED already `flagged` still notifies, while a normal create into `active` and any delete do not. That shape used to have a named producer: `moderateProof`'s merge-set could create the doc itself in the upload-before-document race, never producing an update. It no longer creates the doc at all ([`cloud-vision-moderation.md`](cloud-vision-moderation.md) § "The scanner never creates the Proof", #1143), so a Vision verdict now arrives as an update either way. The create arm stays because the predicate is about transitions rather than about which writer makes them, and a create straight into a moderation state must never be the one transition nobody is told about.
...
Setting `RESEND_API_KEY` in Secret Manager from the 1Password item and redeploying the bound functions is a human finishing step, not part of this code PR. The `mail.nathanpayne.com` Resend domain was already verified when this shipped, so it needed no new DNS; `fiveacross.app` was verified later under [#1102](https://github.com/nathanjohnpayne/fiveacross/issues/1102), which did add DNS. The apex `nathanpayne.com` is NOT verified for this project.
