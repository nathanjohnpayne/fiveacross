...
The award computes in a new sibling best-effort beat inside `runFinaleBeats` (`functions/src/unlockDay.ts`), keyed to the same `atFreeze` transition that stamps `frozenAt`—it piggybacks on the proven `frozenAt` machinery rather than carrying a freeze boundary of its own (see § Deviations #1). `finaleActions` returns `computeMostLoved` only while the Event is still unfrozen and the award is absent. Unlike the podium, this beat is deliberately coupled to the freeze: an award retry must never rebuild its eligibility from later moderation state. The podium now has a coupling of its own, and a narrower one: the award never holds it back, but a freeze this run OWES and does not get does (`specs/d15-finale.md` § Contract, `EventDoc.frozenPlayRecorded`).

`freezeStandingsAndPersistMostLovedAward` reads the Event, `events/{eventId}/proofs`, and `events/{eventId}/hearts` inside one Firestore transaction, maps the collection snapshots defensively onto plain shapes, delegates to the pure `buildMostLovedPhotoAward` in `functions/src/finaleContent.ts`, then writes `frozenAt` and the explicit award record together. The transaction's read snapshot captures Proof visibility, the event report threshold and ban roster, and Heart eligibility at the one successful freeze. If an award-input read fails, neither frozen field lands; the next tick retries the complete snapshot rather than combining an old freeze with new moderation data. **And the podium waits with them** (CodeRabbit on [PR #1242](https://github.com/nathanjohnpayne/fiveacross/pull/1242)): the beat keeps its own retry guard and its own try/catch, but a run whose freeze does not land has no `frozenPlayRecorded` to quote, and its Moment can never be amended—so it defers to the sweep that gets the freeze rather than posting a record the successful freeze can no longer complete (`specs/d15-finale.md` § Contract, `EventDoc.frozenPlayRecorded`). A freeze that landed on an EARLIER run never holds it back.

**The cutoff is the SCHEDULED freeze instant** (`times.standingsFreezeAt`), never the run clock: Hearts whose Firestore `createTime` is after the cutoff are excluded, even if the transaction begins late. Once the atomic write succeeds, its explicit award record is immutable for all later ticks, report actions, Proof moderation, and roster changes. Residual, accepted: if a scheduler cannot obtain the input snapshot at all, it delays the entire freeze until a later healthy run rather than claiming a historically exact award it cannot reconstruct.
...
1. **ADR 0011 landed (#551), and the award moved with it by construction.** The award freezes against the `atFreeze` transition, whose boundary is `times.standingsFreezeAt`—the Event's resolved Standings Freeze: the configured `EventDoc.standingsFreezeAt` when the doc carries a usable one, else the first ceremonial Day's `unlockAt`. This was written while that field had no runtime consumer, so the deviation recorded was "freezes against `frozenAt`, not a consumed `standingsFreezeAt`". Nothing about the beat changed when the migration landed: the freeze and the award read the SAME boundary, so they move together, and the field the code names is simply a resolved freeze now rather than a closing Day's unlock. For Bodega the seed pins the two instants equal on purpose (`standingsFreezeAt == days[3].unlockAt`), so its cutoff is unchanged to the millisecond. The parity test ADR 0011 demands ships, and now compares the full podium.
...
6. **An earlier draft of the wireframe's crown row carried a stat; it was dropped, and the artboards now match the renderer.** The first `fx-share-final-photo-*` artboards showed a stat beside the First-to-BINGO crown row (a Day-and-time stamp on vacay/fa, "11 bingos" on gcb), but `FarewellShareCardData.firstBingo` carries only `displayName` and the card system renders no timezone-formatted times, so the photo-hero card renders the crown row as rank + name + role with no stat (`src/components/ShareCard.tsx`, `buildFarewellCardNode`). The artboards were redrawn to that shape (#867) and the GCB frame's caption points back to this item, so this is recorded history rather than a live gap: the renderer still carries no time or count for the crown row, for the reason above.
