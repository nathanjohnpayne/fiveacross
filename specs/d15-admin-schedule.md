...
- `tests/rules/d15-admin-schedule.test.ts` (rules-emulator, time-gated against a fixed `request.time` via `PAST()`/`FUTURE()` helpers)—an admin CAN change `days[i].theme` for a Day with a future `unlockAt`; an admin CANNOT change `days[i].theme` for a Day whose `unlockAt` has passed; a non-admin can never write `days[]` at all, locked or unlocked Day, any field; and both the allow and deny cases run against Gay Cruise Bingo's full ten-Day schedule.
