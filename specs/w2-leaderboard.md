...
## Amended: the comparator is TOTAL over any row the rules accept (#1145, #1142 item 10)

The tie-break above assumed `bingoCount`, `squaresMarked` and `firstBingoAt` really are numbers. `players/{uid}` validates none of them, so a Player could leave a row holding anything at all where a stat belongs, and `comparePlayers`' two subtractions turned that into a thrown `TypeError`—out of `sortPlayers`, which is what `useLeaderboard` returns and what the archive builder ranks with, so one row took down every roster consumer including the Admin console's Game settings. `comparePlayers` now reads each field through a coercion instead of subtracting it raw: an unreadable count ranks as the `0` the row itself displays, and an unreadable instant as the `null` that already sorts last. Well-formed input is ordered exactly as the table above says—this changes nothing for a roster that was always fine, and it decides nothing about who won (ADR 0001). `useLeaderboard` additionally normalises the same three fields on each row before sorting, because the row is also PRINTED and React throws on an object child. See [`post-sailing-archive`](post-sailing-archive.md) § "The surfaces".
