...
- **Given** two Player rows carrying distinct counts (or first-bingo instants) above the archive's magnitude bound, **when** the live Leaderboard ranks them and when the freeze ranks them, **then** the two orders are the same—`src/game/w2-leaderboard.test.ts` § "withReadableRanking applies the archive's own bounds — one mechanism, two paths", which ranks one roster through `withReadableRanking` and through `withReadableDayStats` and compares the results, with the identical clamped values read back off both and an in-bounds row still returned by identity.
...
## Amended: the roster is made READABLE before it is ranked (#1145, #1142 item 10)

The tie-break above assumed `bingoCount`, `squaresMarked` and `firstBingoAt` really are numbers. `players/{uid}` validates none of them, so a Player could leave a row holding anything at all where a stat belongs, and `comparePlayers`' two subtractions turned that into a thrown `TypeError`—out of `sortPlayers`, which is what `useLeaderboard` returns and what the archive builder ranks with, so one row took down every roster consumer including the Admin console's Game settings. The guard runs BEFORE the sort rather than inside the comparator: `useLeaderboard` maps each row through `withReadableRanking` on its way into `sortPlayers`, and the archive builder normalises the same three fields through `withReadableDayStats` on the roster it re-reads (#1151). One mechanism, applied at each ranking entry point, so the ORDER and the row that is printed or frozen read the same numbers off the same row; a coercion inside `comparePlayers` as well would let those two disagree again, and would break the comparator's byte-identity with its `functions/src/finaleContent.ts` mirror. An unreadable count ranks as the `0` the row itself displays, and an unreadable instant as the `null` that already sorts last. **And a finite value outside the archive's magnitude bound is CLAMPED on both paths** (Codex P2, PR [#1165](https://github.com/nathanjohnpayne/fiveacross/pull/1165)). The archive clamped—its record has to satisfy `firestore.rules`' `finiteArchiveNumber`—while the live normaliser passed any finite value through, so two rows above that bound could order one way on the last live Leaderboard and another way in the frozen record: two distinct oversized `bingoCount`s collapse to a tie under the clamp and then reorder on squares. That makes the frozen standings stop copying what Players last saw, which is the record's whole promise. `clampArchiveNumber` and `MAX_ARCHIVE_NUMBER` therefore live in `src/data/eventLimits.ts`—the one module with no imports of its own, so `src/game/logic.ts` and `src/data/eventArchive.ts` can share the bound without the cycle that a `game`→`eventArchive` import would create—and both normalisers read it from there. Ordinary rosters are untouched: no real count or instant is within nine orders of magnitude of the bound, and an in-bounds row is still returned by identity. `withReadableRanking` returns a well-formed row by identity, so a healthy roster is ordered exactly as the table above says and pays one array—and it decides nothing about who won (ADR 0001). The row is also PRINTED, and React throws on an object child, which is the other half of why the normalisation belongs on the roster rather than on the comparison. See [`post-sailing-archive`](post-sailing-archive.md) § "The surfaces".

## Superseded once the Event is archived (#1152)

Everything above describes the LIVE Leaderboard. Once the Event is archived, `Leaderboard` becomes a routing component that renders `ArchivedLeaderboard` from the frozen `EventDoc.archive` record instead—same ranked rows and same First-to-BINGO pin, but read from a stored snapshot rather than re-derived, and with the presentational filters gone (a frozen record has one shape). The freeze COPIES what this surface showed; it never recomputes it. The live view moves into a `LiveLeaderboard` child so an archived visit never mounts `useLeaderboard`, `useDayMetasStatus` or `useProofKindsByUid` at all, and the routing branch waits for a server-resolved Event before choosing between them—or, on an offline client, for that subscription's own cache result, since `useOnline` reports offline before the first snapshot has arrived and routing on that alone mounted the live child over no Event at all. A confirmed archive is then LATCHED, so the ban this spec's own moderation rule leans on cannot unmount it: `hasPendingWrites` marks the whole Event snapshot, and treating an Admin's later ban as an unconfirmed archive put the live view back on screen over the frozen record. Two rules from this spec carry over unchanged into the frozen surface: the pin tracks earliest-bingo rather than rank, and a banned Player is hidden without anyone being promoted into the vacancy. See [`post-sailing-archive`](post-sailing-archive.md) § "The surfaces".
