...
2. **Resolve.** The serving host's own `hostnames/{host}` document is read exactly as today. **Whether that host does path addressing is decided by one optional field on it, `pathNamespace`—not by whether it is an Event document or a root marker.** When `pathNamespace` is present and a slug was parsed, resolution reads `hostnames/{slug}.{pathNamespace}` and serves that Event if the selected regime's eligibility rule passes; otherwise the host's own document answers and the path segment is an app route.
...
**Enabling a host is a deployment-barrier operation, even though the capability's source of truth is data.** The endpoint and capability-aware worker ship first; all existing root-scoped workers are forcibly advanced and their precaches retired (D4); only then may the `pathNamespace` field and the eligible target routing documents be published. That ordering prevents an older prompt-waiting worker, which cannot consult the capability endpoint, from serving its env-pinned flagship shell for the first live path. Once that barrier is in place, adding or removing target documents remains a data change and the app resolver, edge projection and worker all read the same host field.
...
- **Given** a host is about to gain `pathNamespace`, **when** rollout is performed, **then** the endpoint and capability-aware worker deploy and all root-scoped workers are forcibly advanced before the field and eligible targets are published, so a prompt-waiting old worker cannot serve its env-pinned flagship at the first path URL.
...
- **Non-installability** (D4) — a worker-source guard extending `src/sw-auth-handler-denylist.test.ts`'s existing pattern, plus component tests that no manifest link and no install affordance render on a path-addressed surface. The worker tests stub the same-origin capability endpoint for a path-enabled host and a host without `pathNamespace`, prove an unavailable endpoint sends only a non-reserved candidate slug to the network (never the shell), and prove rollout cannot publish `pathNamespace` before the capability-aware worker has forcibly replaced old root-scoped workers.
...
- **The archive interlock** (D8) — rules and resolver tests over the status matrix **and the `apexPath` eligibility matrix** (an active Event in a path-enabled namespace must be not-found at an apex path even when an invalid document carries `apexPath`, while remaining reachable on a mirror; only an archived target with `apexPath` serves at the apex), a single-transaction test covering every routing document plus `EventDoc.status` together, and — the case the first draft of this spec got wrong — a test that a **fresh** cached `active` entry does not silently outlive the flip.
