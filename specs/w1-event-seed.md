...
The August 5 Bodega text correction was intentionally an in-place data edit: live documents retain their original content-hash IDs because Day 0’s frozen snapshot and already-dealt cards refer to those IDs. Bodega’s seed module records that exact one-to-one historical ID list as `VERIFY_ITEM_IDS`. `verify:seed:fiveacross` accepts either the current text hash (a fresh Event) or that documented live identity for the corresponding prompt, then still checks text, pool, status, spicy flag, free-space flag, and report threshold. It does not make an identity-changing `RESEED=1` safe: the snapshot interlock continues to reject a replace that would orphan a frozen Day. (Test: "verifies the live in-place-edited pool against its recorded legacy ids".)
