...
`scripts/seed.mjs` is the run-once Admin script that establishes `events/{id}` and the dense Prompt pool the Event opens with. This spec pins the Phase-0 reconciliation of the seeded payload to the accepted ADRs: the ADR 0001 Claim Mode rename, the ADR 0003 pool-density requirement, and the ADR 0004 dead-config removal, plus the `ADMIN_UID` roster flow pending decision #15. Seed payloads are per-Event modules (#563): each seedable Event owns a `scripts/seed-data/<event-id>.mjs` module exporting its `EVENT_SEED` payload and pools (`ITEMS`/`EASY_ITEMS`/`CLOSING_ITEMS`/`ALL_ITEMS`), registered in `scripts/seed-data/index.mjs`; `scripts/seed.mjs` keeps only the Event-agnostic helpers and resolves its target from `VITE_EVENT_ID` or the resolved project's default (`gaycruisebingo` → `med-2026`, `fiveacross` → `bodega-bay-2026`), failing loudly on an Event id with no registered data. Every payload module is import-safe—seeding runs only when the script is the entry module, so importing needs neither Firebase credentials nor the dev-only `firebase-admin` install—and every claim below maps to an assertion in `src/test/w1-event-seed.test.ts` plus the per-Event pins in `src/test/seed-pools-per-event.test.ts` (layer: unit).

The Claim-Mode and roster claims below hold for EVERY registered Event payload. The settings claims are scoped: every Event seeds `reportHideThreshold: 4` and a `spicyRatio` (Bodega Bay: `0`), never the removed ADR 0004 flag — but the EXACT-key criterion (`settings` having only `reportHideThreshold` + `spicyRatio`) is the med-2026 payload's own pin; other Events may carry additional settings (Bodega Bay adds `easyMixRatio: 0.5`, pinned in `seed-pools-per-event.test.ts`). The pool-density section's 80-entry pin is likewise med-2026's own (each Event pins its own counts—Bodega Bay pins 40/40/40).

### Bodega’s retained live prompt identities

The August 5 Bodega text correction was intentionally an in-place data edit: live documents retain their original content-hash IDs because Day 0’s frozen snapshot and already-dealt cards refer to those IDs. Bodega’s seed module records that exact one-to-one historical ID list as `VERIFY_ITEM_IDS`. `verify:seed:fiveacross` accepts one complete generation—the current text hashes for a fresh Event or the recorded historical identities for the live Event—then still checks text, pool, status, spicy flag, free-space flag, and report threshold; it rejects a mixed generation, which could silently break a frozen snapshot. During a safe replace, matching seed docs’ `createdAt` values are clamped to the oldest due-Day-safe entry time as well, so a delayed earlier seed cannot leave an unstamped but due Day with a permanently short snapshot. Neither rule makes an identity-changing `RESEED=1` safe: the snapshot interlock continues to reject a replace that would orphan a frozen Day. (Tests: "verifies the live in-place-edited pool against its recorded legacy ids", "requires one complete identity generation", and "clamps a matching prompt written after a due Day cutoff".)
...
- **Given** the static seed payload **when** inspected **then** `settings` has exactly the `reportHideThreshold` and `spicyRatio` keys (the latter added by `seed-and-composition`, #129, which absorbed and extended this spec's remaining seed-script scope), and the seed source never assigns the removed ADR 0004 flag a literal value. (Tests: "seeds no blackoutEnabled—ADR 0004 removed it as dead config"; "never seeds a literal value for blackoutEnabled—the seed source references it only as the delete target".)
...
- **Given** the med-2026 seeded pool **when** counted **then** it is that Event's canonical 80-entry pool; other Events pin their own counts (Bodega Bay: 40 easy / 40 main / 40 closing, `seed-pools-per-event.test.ts`). (Test: "seeds the canonical 80-entry pool (w1-seed-and-composition)".)
- **Given** the seeded pool **when** de-duplicated **then** every `text` is unique and non-empty, and every entry's `spicy` is a boolean. (Test: "seeds unique, non-empty prompt { text, spicy } entries—content-hash doc ids (hashed on text) collapse duplicates".)
