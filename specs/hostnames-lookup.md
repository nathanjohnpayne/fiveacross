...
Four lifecycle writes replace a hostname document rather than patching it: that mirror-root conversion, `repoint`, `convert-to-root` and `convert-to-route` (#1251). A Firestore `update` has no way to drop a field, and a root marker may not carry `eventId`, `status` or `slug`, so the two route-to-root conversions build the marker by removing exactly those three and `apexPath` from the stored document; `adultContent`, `preview`, `canonicalHost` and `isCanonical` therefore survive them unchanged, and the three writers named above stay the only things that decide their values. The two writes that move a host to a different Event, a repoint to another Event and `convert-to-route`, clear the Event-scoped fields instead.
