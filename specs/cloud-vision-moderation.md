...
So the publish is conditional. Inside the same transaction that resolves the claim (the read placed before every write, per Firestore's reads-before-writes contract), `confirmClaim` re-reads the **live** Proof and skips the `status` write entirely when a Vision hide stands on it. The claim still resolves, the Square still credits, the Mark is still confirmed; only the media stays hidden. There is no new write and no new field — the safety hide simply survives, and the admin lifts it where lifting belongs.

`visionHideStands` (`src/data/moderation.ts`) is that gate: `'hidden'` **or** `'flagged'` **and** an allowlisted `visionFlag`. Both states, because `'flagged'` is the Proof the trigger is about to hide — or failed to hide and will retry on the next write — and publishing it would move the doc out of the state the retry looks for. It is a **mirror** of `AUTO_HIDE_VISION_FLAGS` / `isAutoHideVisionFlag` rather than an import, because the app and the Functions package are deliberately decoupled (the same shape as the last-call-copy mirror, #800); a parity block in the functions suite feeds one verdict set to both sides so the two cannot drift apart unnoticed. Keying the gate on the verdict rather than on "is this Proof hidden at all" is deliberate: an admin's manual hide and the report-count auto-hide have their own console lifts (`Restore`, `Clear reports`), and confirm's behaviour toward them is unchanged.

`ReviewQueue`'s claims group says so on the row, so the admin is not surprised by a Confirm that does less than it used to: the pending-claim row gains the same `hidden · AI screen: <verdict>` pill the Proof row carries, plus `Confirming credits the mark; the photo stays hidden for moderation.` It reads the verdict off the Reports group the console already renders — `useReportedProofs` queues every Vision-flagged Proof — so there is no second subscription and no per-claim read. No rules change: `status` is admin-writable on the `isAdmin(eventId)` arm either way, and this narrows a client write rather than widening one.

- **Given** an admin_confirmed claim whose Proof is `'hidden'` (or still `'flagged'`) with an extreme/illegal `visionFlag` **then** `confirmClaim` writes NOTHING to the Proof — the safety hide stands — while the claim resolves `confirmed`, the cell flips to `confirmed`, and the Square credits. (Tests: `src/data/cloud-vision-moderation.test.ts`.)
...
- **Given** the client mirror and the functions allowlist **then** they export the same verdict list and agree verdict-for-verdict, including on mis-cased, non-string, and unknown values. (Tests: the "client/functions parity" block in `tests/functions/cloud-vision-moderation.test.ts`.)
- **Given** a pending-claim row whose Proof carries a standing Vision hide **then** it shows the verdict pill and says the photo stays hidden; **given** any other claim **then** it is unannotated and Confirm behaves as before. (Tests: the "Pending claims" cases in `src/components/admin/cloud-vision-moderation.test.tsx`.)
