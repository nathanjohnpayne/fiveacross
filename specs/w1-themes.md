...
The Theme set started as the eight Atlantis party Themes; it has since grown to include Gay Cruise Bingo's two tutorial-Day Themes (#206), five more party Themes, and — from #555 — Themes belonging to other **Editions**. The registry is therefore no longer the pick list: see "Registry vs. picker" below.

## Contract

- `src/theme/themes.ts` exports `THEMES: ThemeMeta[]`, one entry per `ThemeId` (`src/types.ts`), in a fixed order with `neon-playground` first. Each entry is `{ id, label, emoji, description }` — no Atlantis trademark or affiliation text in any `label`.
...
- `src/components/ThemeSwitcher.tsx` renders one chip per **pickable** Theme for the active Edition (`themesForEdition()`, see "Registry vs. picker") and calls `setTheme` on tap, which drives `ThemeContext`'s `localStorage['gcb.theme']` persistence and DOM application (above). For a signed-in Player it additionally calls `savePlayerTheme` (`src/data/api.ts`) directly, a second and independent persistence path the switcher owns itself: it writes the pick to that Player's Firestore row so the choice follows them across devices. `src/main.tsx`'s `ThemedApp` reads that saved value back out as `player?.theme`, the first candidate in the `defaultTheme` it passes to `ThemeProvider` (`player?.theme ?? event?.defaultTheme ?? 'neon-playground'`) — the same async event/player default `ThemeContext` (above) adopts only when the user has no local pick.
...
- The Editions partition the current registry: the fifteen Gay Cruise Bingo Themes are `gcb`, the three Bodega Bay Themes are `vacay`, and nothing is in both.
...
- The two admin controls use `themesForEditionIncluding(current)`, which adds an already-set off-Edition Theme to the front of the list. Narrowing what may be picked must not misreport what is set — a `<select>` whose `value` matches no `<option>` renders the first one instead, which would show a Day as a Theme it is not.
...
- **Given** any Theme in the registry, **when** it is applied, **then** every pair above meets the WCAG AA 4.5:1 threshold. Editions do not narrow this: a Theme is audited because it is in `THEMES`, not because it is pickable here.
