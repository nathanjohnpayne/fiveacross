...
- **Given** an Event that does not resolve, is not `active`, or a lookup that FAILS **then** nothing is enqueued. (Tests: "refuses to enqueue against an Event that does not resolve", "refuses to enqueue against a non-ACTIVE Event, matching the sweep's own precondition", "treats a FAILED Event lookup as unresolvable rather than assuming it exists".)
