...
**Both fields need their OWN TTL policy, and an upgrade needs a backfill on top.** Firestore TTL is scoped to a collection group, so the `adminAlerts` policy does not reach `adminAlertBatches`—without the second policy the batch's `expiresAt` is inert and the rendered email persists. And TTL only ever looks at documents that ALREADY carry a timestamp in the field, so enabling the policies does nothing for anything written before the deploy: pending rows under archived Events, which the sweep will never revisit, and frozen batches whose delivery kept failing. `scripts/backfill-alert-ttl.mjs` stamps exactly those, anchoring each deadline to the document's own `createdAt` so an old row is already due rather than granted a fresh window. Both policies and the backfill are in `docs/app/phase-1-deploy.md` § 1a.
