...
**The enqueue is idempotent under trigger redelivery.** Firestore redelivers a document-write event on retry, and the retry carries the SAME CloudEvent id, so the queue document's id is derived from it (`{transitionId}-{kind}`). A random id would mint a second alert for one transition: a duplicate row before a drain, and — if the redelivery lands after one — a whole second email whose set, and therefore whose Resend key, differs from the first. This is the guarantee [#101](https://github.com/nathanjohnpayne/fiveacross/issues/101) bought by folding the CloudEvent id into its idempotency key, carried forward. The enqueue transaction reads that deterministic document and writes only when it is absent, so it never overwrites a queued row, a delivered tombstone or a discard tombstone. It reads the Event in the same transaction, which also serializes enqueue against archive. The kind is part of the id because one write can earn more than one alert.
...
A bug report the reporter marked `abuse` earns one `abuse-reported` alert ([#670](https://github.com/nathanjohnpayne/fiveacross/issues/670)). The intake contract carries a `kind` field for exactly this—see `specs/w4-bug-report-inbox.md` § "Bug or abuse" for the field, its normalization, and the reporter control—and a plain `bug` earns nothing, which is the whole reason the field exists: bugs are answered from the inbox, and mailing an admin about each one would make the digest useless for the reports that need eyes now.

It has its own producer, `abuseAlertsForWrite(reportId, before, after)`, rather than a fourth branch of `alertsForWrite`, because its document has none of the `reportCount` / `visionFlag` vocabulary the other three are read through. The predicate is still a TRANSITION, not a state, but #853 gives it two explicit forms. An idempotent report alerts only when `before.intakeState === 'pending'`, `after.intakeState === 'complete'`, and `submissionId`, `reporterHash`, `requestHashVersion`, and `requestHash` are present with the same well-formed values on both sides; the completed report must also be `kind: 'abuse'` with `reporterInEvent === true`. A legacy report alerts only when `before` is absent, `after.intakeState` is absent, and those same abuse/membership facts hold. An absent-before COMPLETE document is NOT legacy, and PENDING, DELETING, unknown/malformed states, mismatched coordination, deletion, and every later COMPLETE update are no-ops. These strict shapes make the finalization write the single alertable event without letting a hand-written or half-migrated document manufacture an alert.

- **Given** a matching PENDING-to-COMPLETE abuse transition **then** one `abuse-reported` draft; **given** its Eventarc redelivery **then** the queue reuses that finalization event's deterministic id.
- **Given** an absent-before abuse document with no `intakeState` **then** one legacy draft; **given** an absent-before document claiming COMPLETE **then** none.
- **Given** PENDING or DELETING on either side, an unknown/malformed state, mismatched/missing coordination fields, a later COMPLETE update, or a delete **then** none.
...
That active-Event precondition is checked again inside the enqueue transaction. The transaction serializes the Event read with the deterministic queue-row write, so an archive racing the abuse trigger either commits first and refuses the row, or commits second and settles it. The scheduled archived-Event backstop repeats settlement for a late row or failed trigger. A player can still file a report against an Event long after it ended; the report lands in the inbox, but nobody is mailed about an Event that is over.

**Every permanent answer is acknowledged; every RETRYABLE one is retried.** `notifyAbuseBugReport` is declared `retry: true`, and `recordBugReportAlerts` divides its outcomes accordingly. Returning normally means "handled, do not retry", which is correct for the cases above because retrying changes nothing about them—the write was not an abuse transition, or the document names no usable, existing, active Event. Those are properties of the data. A failed Firestore READ or WRITE is usually the opposite: nothing about the report is wrong, and swallowing it silently and permanently loses a report of harm, so it propagates—but only when a retry could actually change the outcome. `isRetryableFirestoreError` classifies the gRPC status: the request-is-wrong codes (`INVALID_ARGUMENT`, `NOT_FOUND`, `PERMISSION_DENIED`, `FAILED_PRECONDITION`, `OUT_OF_RANGE`, `UNIMPLEMENTED`, `UNAUTHENTICATED`, `ALREADY_EXISTS`) are logged and acknowledged, because a retryable trigger that rethrows a misconfigured service account turns it into an Eventarc redelivery loop that can never succeed. Everything else, INCLUDING an unrecognised or absent code, is treated as retryable—retrying something permanent wastes invocations, while acknowledging something transient loses a report of harm. Retrying is safe because queue document ids derive from the triggering CloudEvent id, and enqueue never overwrites an existing deterministic row or tombstone.
...
- **Given** a description with an embedded newline or over `LABEL_MAX` **then** the label is flattened and clipped like a Prompt's words; **given** a blank or NON-STRING description **then** the report id is the label, so a malformed document cannot throw on a retryable trigger and be redelivered forever. (Also: test "survives a description that is not a string at all".) (Test: "flattens and clips the reporter description into a single-line label".)
...
- **Given** a RETRYABLE Event read or queue write failure **then** it propagates so the retryable trigger runs again; **given** a permanent one **then** it is acknowledged rather than looped; the moderation producers still swallow theirs. (Also: tests under "isRetryableFirestoreError", "ACKNOWLEDGES a permanent failure instead of looping the retryable trigger on it".) (Tests: "PROPAGATES a failed Event lookup so the retryable trigger can try again", "PROPAGATES a failed queue write, unlike the moderation producers", "does NOT propagate a queue write failure for the moderation producers".)
...
3. **Abuse reports**—one row per report marked `abuse` ([#670](https://github.com/nathanjohnpayne/fiveacross/issues/670)).
...
**A FROZEN replay checks the same thing, and abandons rather than re-renders.** The replay path deliberately re-derives nothing (§ The delivery identity is claimed, not derived), so it never passes through `currentRowFor` at all—and a batch that keeps failing to send is retried every sweep for as long as it keeps failing, which is easily long enough for the retention sweep to remove a report the frozen bytes quote. So a deleted bug report joins the changed roster as the second thing a frozen request may not simply trust: the batch is ABANDONED, its freeze dropped and its claim released, and the rows re-batch from scratch where the deleted one is retired on the way past. Re-rendering in place would change the bytes under a key the batch has already used, which is the 409 that strands a batch forever. Scoped to abuse rows deliberately: a deleted Prompt or Proof is ordinary moderation churn and replaying a stale row about one is the trade [#638](https://github.com/nathanjohnpayne/fiveacross/issues/638) made knowingly, while a deleted bug report is a deliberate act of retention on private evidence.
...
**The Theme is the most recently unlocked Day's**, so a mid-cruise digest carries the palette of the app the admin is about to open. It falls back to the first Day before the Event starts, and for an Event with no schedule `emailThemeTokens` degrades to that EDITION's default Theme—never to grey, and never to another product's identity ([#623](https://github.com/nathanjohnpayne/fiveacross/issues/623) P2). `unlockAt <= 0` is the live-pre-event sentinel ([#289](https://github.com/nathanjohnpayne/fiveacross/issues/289)), not a real instant, so it never counts as unlocked.
...
- **Given** two genuine abuse reports **then** two rows, each carrying its own report ID; a retried intake with the same submission identity is the same report and therefore produces only one source document and one trigger identity ([#853](https://github.com/nathanjohnpayne/fiveacross/issues/853)). (Test: "renders one row per REPORT, never collapsing two abuse reports into one".)
...
The origin comes from `resolveEventOrigin`—the Event's canonical `hostnames` mapping, then any active mapping, then `APP_BASE_URL` ([#599](https://github.com/nathanjohnpayne/fiveacross/issues/599)). That function moved to a narrower parameter type (`HostnameSource`) so this family reuses it instead of carrying a second copy of the preference order; `DailyEmailFirestore` satisfies it structurally, so no existing caller changed. A FAILED hostname read is not an absence: it propagates, the per-Event sweep boundary logs and skips, and the next sweep retries—rather than degrading a Vacay/Five Across digest to the legacy Edition's brand line.
...
**The frozen request carries its own `expiresAt`**, `PENDING_TTL_MS` from the freeze. `events/{eventId}/adminAlertBatches/{batchId}` holds the FULLY RENDERED email—every pending Prompt's words and every abuse description in the batch at once—so it is the densest copy of user content in the system, and until this existed it had no expiry at all: repeated delivery failures keep a frozen batch alive indefinitely, the pending TTL below reaps the claimed rows underneath it, and no later sweep can discover the batch to replay, release or delete it.

**It must OUTLIVE every row it claims, and `PENDING_TTL_MS` from the freeze is what guarantees that**: the rows were created at or before that moment and carry the same span, so the freeze's deadline is never earlier than any of theirs.

Both directions are hazards. If the freeze expires EARLY relative to its claimed rows, the next sweep finds claimed rows with no frozen document and takes the missing-freeze rebuild path—re-rendering and re-sending under a key that may still be live (a 409 that strands the batch) or one whose 24-hour Resend window has closed (a second copy of a digest that was already delivered, if the original send landed but its response or clean-up was lost). Two earlier shapes each fell into one side of this: inheriting the earliest row's deadline let a long-pending row hand the freeze a deadline minutes away, and a fixed week expired while thirty-day rows survived. Outliving the rows is the safe direction, and "not earlier" is not enough on its own: Firestore's TTL deletion is asynchronous, best-effort and unordered, and the two documents sit in DIFFERENT collection groups under separate policies—so a batch frozen minutes after its rows have deadlines minutes apart and can genuinely be deleted first. `FROZEN_TTL_MARGIN_MS` adds a week of slack on top, far beyond the sub-day latency TTL actually exhibits, which turns the race from unlikely into unreachable. The frozen bytes are therefore the one thing here retained past `PENDING_TTL_MS`, deliberately: keeping a rendered email a few days longer is bounded and self-collecting, while deleting it early is a duplicate delivery.

**And a missing freeze on PAST-DUE rows is retired rather than rebuilt.** The rebuild path exists for one legitimate case—the claim commits before the freeze is written, so a crash in between leaves claimed rows with no frozen document and nothing was sent—and that case is seconds old. A batch that keeps failing to send HAS a freeze, so claimed rows already past their own retention deadline with no freeze mean the freeze existed and was reaped; rebuilding would re-send bytes that may already have been delivered. The margin makes this unreachable in practice, and this is the second line.

**Both fields need their OWN TTL policy, and an upgrade needs a backfill on top.** Firestore TTL is scoped to a collection group, so the `adminAlerts` policy does not reach `adminAlertBatches`—without the second policy the batch's `expiresAt` is inert and the rendered email persists. And TTL only ever looks at documents that ALREADY carry a timestamp in the field, so enabling the policies does nothing for anything written before the deploy. The archived sweep now settles legacy pending rows, but frozen batches whose delivery kept failing still need their timestamps backfilled. `scripts/backfill-alert-ttl.mjs` stamps both kinds, anchoring each deadline to the document's own `createdAt` so an old row is already due rather than granted a fresh window. Both policies and the backfill are in `docs/app/phase-1-deploy.md` § 1a.

**Every row carries an `expiresAt` from the moment it is written**, not only once it is tombstoned. A queue row holds a COPY of user content—a pending Prompt's words, a hidden Proof's text, an abuse reporter's description—and until this existed that copy had exactly one exit: being drained. Archive settlement now supplies a deliberate second exit, while TTL remains the final retention bound if both the archive trigger and scheduled backstop cannot complete. `PENDING_TTL_MS` is thirty days: orders of magnitude past the five-minute sweep, well inside the ninety-day source-report retention window, and long enough that no ordinary backlog is ever reaped—so it only ever collects rows that were genuinely stranded rather than merely pending. A delivered or discarded row is REPLACED on settlement, so the shorter tombstone expiry supersedes this one rather than competing with it.
...
- **Given** a document written BEFORE this shipped **then** the rollout backfill stamps it, anchored to its own age, and leaves already-protected documents untouched. (Tests in `scripts/backfill-alert-ttl.test.mjs`.)
- **Given** a frozen batch **then** its `expiresAt` clears the oldest row it claims by `FROZEN_TTL_MARGIN_MS`, so unordered TTL deletion cannot reverse them, and it is comfortably past the delivery-idempotency window.
- **Given** claimed rows past their own deadline with no frozen request **then** they are retired rather than rebuilt and re-sent. (Test: "RETIRES past-due claimed rows with no freeze rather than risking a duplicate send".) (Test: "gives the frozen request an expiry that outlives every row it claims".)
...
The sweep DELIVERS only for ACTIVE Events. Archive handling follows [ADR 0013](../docs/adr/0013-admin-alert-archive-lifecycle.md): an archived Event has no live moderation surface, so pending work without a frozen outbound request is deliberately replaced by a payload-free discard tombstone. A claimed-and-frozen request is preserved and replayed under its existing key because it may already have been accepted by Resend; archive never rewrites that delivery identity. Enqueue, new claim, and discard each read the Event in the transaction that changes queue state, while the scheduled sweep revisits archived Events as a backstop for delayed producers and failed archive handling.

The frozen request, not the claim alone, is the archive commit boundary. A claim with no frozen batch is safe to discard because this queue always freezes before sending. A discard tombstone keeps the deterministic row id and carries `discardedAt` plus the existing seven-day timestamp-typed `expiresAt`, but no payload, `batchId`, or `sentAt`; delayed producer redelivery therefore remains a no-op without misrepresenting discard as delivery. Reactivation can make only not-yet-terminal rows live again: it never resurrects a discard tombstone or changes a frozen batch's identity.

The trigger pins `ADMIN_SDK_SERVICE_ACCOUNT` (it reads Events, the queue, the roster and `hostnames`, and stamps alerts) and binds `RESEND_API_KEY`—it is now the only admin-notification function that sends. Per [#318](https://github.com/nathanjohnpayne/fiveacross/issues/318): verify the Cloud Scheduler job exists after the deploy (`gcloud scheduler jobs list`)—the deployer service account has historically lacked `cloudscheduler.admin`, which deploys an `onSchedule` function jobless and silently.

The three DOCUMENT-WRITE PRODUCERS are `notifyItemModeration` and `notifyProofModeration` (`onDocumentWritten` on `events/{eventId}/{items,proofs}/{id}`) and `notifyAbuseBugReport` (`onDocumentWritten` on `bugReports/{reportId}`, [#670](https://github.com/nathanjohnpayne/fiveacross/issues/670)). All three pin `ADMIN_SDK_SERVICE_ACCOUNT` and none binds `RESEND_API_KEY`—they only append to the queue. The third has no `{eventId}` wildcard to read, because its collection is flat; it fires on every bug report and rejects a plain `bug` with a pure predicate before it spends a read, so the common case costs one no-op invocation. It writes only under `events/{eventId}/adminAlerts`, never back into `bugReports`, so it cannot re-fire itself.

An idempotent intake ([#853](https://github.com/nathanjohnpayne/fiveacross/issues/853)) first creates a coordination-only PENDING report and later finalizes it to COMPLETE. The abuse producer explicitly ignores PENDING and DELETING writes and treats only the matching PENDING-to-COMPLETE transition as the abuse report's alertable creation; the finalization CloudEvent id is its queue identity. Later COMPLETE updates are no-ops. A legacy one-write report with no `intakeState` retains the original absent-before/abuse-after transition, so older clients still alert exactly once.

An UNKNOWN authorization result at intake has a second, durable producer path ([#859](https://github.com/nathanjohnpayne/fiveacross/issues/859)), specified in `specs/w4-bug-report-inbox.md` § "A failed escalation lookup becomes durable work". Intake atomically creates a private `bugReportEscalations/{reportId}` task alongside the COMPLETE report; the same five-minute `adminAlertDigest` schedule runs `runAbuseEscalationSweep` independently of the ordinary queue drain. A due task transaction revalidates the immutable report, current Event status, and current admin/player relationship, then either creates `alertDocId('bug-report-escalation-' + reportId, 'abuse-reported')` and a payload-free terminal task tombstone together, or records a terminal no-alert outcome. The fast and delayed paths share the pure `abuseAlertDraft` and `pendingAdminAlertRow` builders; the resolver supplies its own authorization transaction rather than calling the transition-gated producer or nesting `enqueueAdminAlerts`. Failed reads retain no authorization decision and reschedule with bounded exponential backoff. This is not a fourth report trigger and does not update the COMPLETE report: #853's finalization trigger remains the fast path, later COMPLETE writes remain no-ops, and the deterministic task-derived identity makes concurrent sweeps converge on one alert.
...
`bugReportEscalations/{reportId}` is denied to clients for the same reason and with a stricter confidentiality consequence: while pending, it temporarily holds the raw reporter uid that the immutable report intentionally replaces with a truncated hash. No Player or Event admin may read or write it. The resolver transaction is the only path from that uid to an alert, and terminal replacement removes it before the task becomes a short-lived tombstone.

- **Given** any client **then** every read and write of a pending or terminal `bugReportEscalations` document is denied. (Test in `tests/rules/w0-firestore-rules.test.ts`.)
