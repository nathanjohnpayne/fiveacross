...
**A fixed week, deliberately, rather than a deadline inherited from the rows.** Inheriting looks tidier and introduces a delivery hazard: a row that sat pending for most of its own lifetime and is finally processed near the end of it would hand the freeze a deadline minutes away, TTL could remove the frozen bytes while the claimed row was still there, and the next sweep's missing-freeze rebuild would fire INSIDE Resend's 24-hour idempotency window—a 409 against the live key, or a duplicate once it closed. A week is unconditionally clear of that window. The trade is that the batch can outlive its rows by up to a week if their TTL fires first; that is a bounded, self-collecting window rather than an indefinite orphan, because the document reaps itself on its own deadline whether or not anything ever finds it again.
...
- **Given** a frozen batch **then** it carries a timestamp-typed `expiresAt` a week out—clear of the delivery-idempotency window in both directions—so it collects itself rather than orphaning. (Test: "gives the frozen request its own week-long expiry, clear of the idempotency window".)
