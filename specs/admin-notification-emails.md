...
- **Given** a frozen batch **then** it carries a timestamp-typed `expiresAt` never earlier than the oldest row it claims, and comfortably past the delivery-idempotency window, so it collects itself rather than orphaning or duplicating. (Test: "gives the frozen request an expiry that outlives every row it claims".)
