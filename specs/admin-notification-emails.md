...
**Both fields need their OWN TTL policy.** Firestore TTL is scoped to a collection group, so the `adminAlerts` policy does not reach `adminAlertBatches`—without the second policy the batch's `expiresAt` is inert and the rendered email persists. Both are listed in `docs/app/phase-1-deploy.md` § 1a.
