...
**Each DAY's own First to BINGO is an honour too, pinned or derived** ([#1217](https://github.com/nathanjohnpayne/fiveacross/issues/1217)). `pinnedOrDerivedDailyHonors` is the one selection the live honours strip, the podium and the frozen record all read, and it resolves a Day two ways: the write-once day-meta **pin** when the Day has one, the roster-**derived** earliest bingo when it does not. The pin branch was always right—a pin carries its own name and instant, so it is checked against `bannedUids` directly and a banned holder leaves the Day reading "—". The derived branch promoted, on the strip and in the record alike, because both callers ban-filtered their roster on the way IN and the fallback then picked the earliest bingo among whoever was left. Both now hand the selector the **RAW** roster and let it withhold on the way out, so a Day whose true First to BINGO is banned goes unheld on both surfaces and the next-earliest Player is not handed a chip. The two had to move together: the frozen record's promise is that it says what the last live strip said (`specs/post-sailing-archive.md`).
...
- The daily honours strip leaves a Day's chip reading "—" when its DERIVED First to BINGO holder is banned, and does not promote the next-earliest Player; a Day the ban does not touch is unchanged, a baseline without the ban names the earliest holder, and an unbanned holder's PIN on that same Day still renders.
...
- A DERIVED daily honour vacates its Day on BOTH honour surfaces rather than passing to the next-earliest Player: the live strip shows "—" for that Day and the frozen record carries no honour for it, while a Day the ban does not touch and an unbanned holder's pin are unaffected—`src/components/w2-ban-console.test.tsx` § "Leaderboard honours strip" and `src/data/post-sailing-archive.test.ts` § "a ban hides, it never reassigns" ([#1217](https://github.com/nathanjohnpayne/fiveacross/issues/1217)).
