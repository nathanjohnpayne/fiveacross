...
- `firestore.rules`—a report is increment-only for non-admins (`reportCount == resource.data.reportCount + 1`, `hasOnly(['reportCount'])`) on both `items` and `proofs`; admins moderate, and since #1275 an Admin client's Prompt `status` moves are bounded to `pending → rejected` and `active ↔ hidden` (approval is the `approvePrompts` callable, ADR 0015) while every other Prompt field and every Proof field stays admin-writable; `settings.reportHideThreshold` must be a number when the event write carries `settings`. These are consumed as-is; **this ticket adds no rules**.
...
- `src/components/Admin.tsx`—a **Report queue** surface. Reported Proofs (`useReportedProofs`) and reported Prompts (derived from the already-subscribed `useAllItems`, so no extra listener) are merged into **one list**. Each row keeps its per-kind hide / restore / delete controls, gains a **"Clear reports"** control when it is auto-hidden (the community-hide lift), and shows an **"auto-hidden"** pill (via the REAL predicate) marking exactly which rows the community hide has removed from every Player's Feed/pool. The full "Prompts" management list (now the Prompt-pool surface) gains the same pill. *Re-housed by `admin-console-ia`*: the queue is the Reports group of the merged Review queue at `/more/admin/queue`, ordered **oldest-first** (`createdAt` asc, uniform with the Approvals and Claims groups)—superseding this spec's original most-reported-first sort.
...
"Clear reports" is **distinct from** Restore: Restore lifts the `status` hard-hide, Clear reports lifts the community auto-hide, and a doubly-hidden row (hidden `status` AND over threshold) shows both so an Admin can fully lift it. The control appears exactly when there is a hide to lift—never on a below-threshold reported row. This is permitted because an admin `update` of `reportCount` is unconstrained on both `items` and `proofs` (`firestore.rules`: the `isAdmin(eventId)` arm; on `items` it has bounded only `status` moves since #1275, via `itemStatusMoveAllowedForAdminClient()`), so an admin write of `reportCount` is authorized; the rules suite pins it. This is the Phase-0 console affordance; server-authoritative removal/lift remains #43.
...
- An Admin moderates: hard-hide (`status`) of an active row, restore of a hidden one, clear `reportCount` (lifting an auto-hide), and delete—Prompt and Proof. On a Prompt those are the only admin status moves besides `pending → rejected` (#1275; the approval and laundering denials are pinned in `tests/rules/d15-approvals.test.ts`).
...
- The report queue is ONE list across Proofs and Prompts—the component suite (finding 4). *Ordering superseded by `admin-console-ia`*: OLDEST-FIRST (`createdAt` asc), uniform with the merged Review queue's other groups; the component suite pins the new order.
