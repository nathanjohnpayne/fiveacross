...
That applies to the shared `useDocSub` / `useColSub` hooks, the Day-meta and Day-board listener fans, and `useTallyCards`; the Day-board fan also keys the board-freshness registry to the captured Event. A per-snapshot OBSERVER a subscription carries is given that captured Event id as an argument rather than reading the live `EVENT_ID` itself: the callback runs when the snapshot lands, and a snapshot that lands after the switch but before cleanup has retired the old listener would otherwise read the binding as Event B and persist Event A's observation under B's key. Side-effect-only observers (`RetractWinMoments` and the durable direct-Mark analytics listener) expose no React data to neutralize, but they capture the Event in their effect/subscription lifetime and synchronously retire callbacks during cleanup. `users/{uid}` remains the deliberate exception: profile identity is global across Events, so its subscription key is uid-scoped rather than Event-scoped.
...
That App boundary is the ONE Event key in the client tree ([#1082](https://github.com/nathanjohnpayne/fiveacross/issues/1082)). Event-owned surfaces below it — the Board included — do not re-key themselves on `EVENT_ID`: a key inside an already-remounted subtree can never observe a change its parent has not already acted on, so the "Event changed while still mounted" refs and layout effects a second key implies are unreachable in the shipped tree and are not written. A surface whose late continuation must stay on the Event it acted under captures that Event id in its own closure, per § Async operations and uploads, rather than watching a live binding from inside a mounted instance.
...
- The keyed App boundary clears Event-owned route state while ancestor-owned global state survives, remounts the Board itself rather than re-rendering it, and Auth bootstrap/deal generations cannot cross Events: `src/App.test.tsx` and `src/auth/AuthContext.test.tsx`. Board's own Event-transition regressions drive the switch through a stand-in for that same boundary: `src/components/Board.test.tsx`.
