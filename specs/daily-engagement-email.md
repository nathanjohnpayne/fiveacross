...
**One email per Event-local CALENDAR DAY, not per Day document** ([#723](https://github.com/nathanjohnpayne/gaycruisebingo/issues/723)). An Event's `days` array does not map one-to-one onto its calendar days, and keying the send off raw `unlockAt` values assumed it did. Bodega Bay puts two Days on Sunday—the 06:00 competitive card and the 11:00 ceremonial wrap-up whose unlock IS the Standings Freeze—and opens with a Day carrying the `unlockAt: 0` sentinel, which is not an instant at all; that Event was mailed twice on Sunday and never on Friday. So the due check first asks which Day OWNS today in the Event's timezone (the lowest-indexed Day on that local date; a later Day sharing it is not a new day and gets no email of its own), then asks whether that Day's send window is open. The Event zone is load-bearing rather than cosmetic: two Days at 05:00 and 18:00 on one Los Angeles date fall on two different UTC dates, so a UTC-grouped check would still hand the second one its own email.

For a Day with a real unlock the window is unchanged—`[unlockAt, unlockAt + SEND_WINDOW_MS)`, an absolute instant already derived from the Event's local schedule. Ownership decides WHICH Day may speak for a date; it never shortens that window. A Day unlocking at 23:00 local is still due at 01:00, when the calendar date has already rolled over, because otherwise an outage or a late enable in those two hours would lose that Day's email permanently. For the open sentinel there is no instant to anchor to, so the same span is measured on the Event's wall clock from `SENTINEL_SEND_LOCAL_HOUR` (08:00, the schedule convention `src/domainTypes.d.ts` documents for `unlockAt`) on the Day's own `date`. That one is bounded to exactly one local morning on exactly one date, which preserves what the previous blanket "a sentinel Day is never due" skip was actually protecting: a Day that would otherwise look permanently overdue and mail forever. A sentinel Day with no usable `date` still belongs to no calendar day and is still never due. In the rare case where yesterday's Day and today's are both inside their windows, today's owner wins, so the send is never a stale card.

**Only the RECOGNISED sentinel is scheduled off a `date` label.** `unlockAt: 0` is a deliberate convention; a missing, `NaN`, non-numeric or negative `unlockAt` is a partially written or corrupted schedule, and mailing on it would be an unrecallable email sent on garbage. Such a Day is never due. It still HOLDS its `date` if it has one—silencing that date rather than being dropped, since dropping it would promote the next Day on the date (Bodega's 11:00 wrap-up) into exactly the email this rule exists to withhold. No email beats the wrong email.
...
- **Given** two Days on one Event-local calendar date **then** only the lowest-indexed one is ever due, and the Event zone—not UTC—decides which dates those are. (Tests: "collapses two Days that share one Event-local calendar date onto the FIRST of them", "groups Days by the EVENT zone, not UTC", "resolves the due Day in the EVENT's timezone, not the sweep's UTC".)
- **Given** the `unlockAt: 0` live-pre-event sentinel ([#289](https://github.com/nathanjohnpayne/gaycruisebingo/issues/289)) on a DATED Day **then** it is due for one six-hour local morning on that date and never again; **given** no usable `date` **then** it is never due, rather than permanently overdue. (Tests: "sends on the opening Day, which carries the open sentinel by design", "keeps that sentinel Day bounded to one local morning, on one date", "never fires for an UNDATED sentinel Day".)
...
- **Given** a Day whose `unlockAt` is missing, `NaN`, non-numeric or negative—valid `date` or not—**then** it is never due, and it still holds its date rather than promoting a later Day on it. (Tests: "never mails a Day whose unlockAt is malformed, even with a valid date", "lets a malformed Day silence its date rather than promoting the next Day on it".)
- **Given** the live `bodega-bay-2026` schedule replayed sweep by sweep across the whole trip **then** exactly three emails go out, one per calendar day, Day indexes 0/1/2, and the wrap-up Day's `place` never reaches the arrival line. (Tests under "the Bodega Bay POC, replayed sweep by sweep".)
