...
For a Day with a real unlock the window is unchanged—`[unlockAt, unlockAt + SEND_WINDOW_MS)`, an absolute instant already derived from the Event's local schedule. For the open sentinel there is no instant to anchor to, so the same span is measured on the Event's wall clock from `SENTINEL_SEND_LOCAL_HOUR` (08:00, the schedule convention `src/domainTypes.d.ts` documents for `unlockAt`) on the Day's own `date`. That is bounded to exactly one local morning on exactly one date, which preserves what the previous blanket "a sentinel Day is never due" skip was actually protecting: a Day that would otherwise look permanently overdue and mail forever. A sentinel Day with no usable `date` still belongs to no calendar day and is still never due.
