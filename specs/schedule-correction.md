...
- **Refuses legacy-shaped Days (#652)**—it also refuses when any live Day still carries the pre-[#566](https://github.com/nathanjohnpayne/gaycruisebingo/issues/566) `port`/`portEmoji` keys, and that check runs FIRST because it invalidates every other verdict in the plan. The corrected Day is the live Day with only the allowed four overwritten, so legacy keys survive the write—and `migrateDayFields` (`src/data/converters.ts`) deliberately gives a retained `portEmoji` display precedence over `placeEmoji`. Without the refusal, a Day whose location emoji changed would be reported and applied as corrected while clients kept rendering the old emoji, and every rerun would look idempotent. `LEGACY_FIELDS` is detection-only and is never written: this script stays on the neutral vocabulary, and a legacy-shaped Day must be migrated to it before the correction can run.
- **Is idempotent**—a second run reports "already correct" and writes nothing. The legacy refusal is what keeps that claim honest: a legacy-shaped Day is never reported as `unchanged`.
...
- `src/data/schedule-correction.test.ts`—the corrected day → theme/place/placeEmoji/tonight mapping asserted against the seed, exactly-two-entries per day, the no-Atlantis-mark guard on every `tonight` line, and the migration planning core (corrects the old live schedule with no forbidden change, preserves the scheduler snapshot and every immutable field, is idempotent, and refuses on immutable-field drift / date shift / count mismatch). The #652 legacy-shape cases live here too, with fixtures in the RAW `port`/`portEmoji` vocabulary—`place`-vocabulary fixtures cannot reach that case, which is why it went unnoticed—covering detection (including an empty-string legacy value), the plan-level abort, the retained-legacy-key behaviour that motivates the refusal, and that `LEGACY_FIELDS` never enters the writable set.
