...
`blockPlayer({ me, target, eventId })` commits `{ set direction, set pair }`. Idempotent under the arms above. `unblockPlayer({ me, target, eventId })` first tries `{ delete direction, delete pair }`; on `permission-denied`, which the rules issue exactly when the other direction still stands, it retries `{ delete direction }`, then makes one best-effort `{ delete pair }` and returns `{ stillHidden: true }` when that is denied (the ordinary mutual case: the other direction still stands) or fails. Any other error on the first two attempts rethrows. The third attempt covers a concurrent mutual unblock: both first attempts are denied, both direction-only retries can land, each authorized while the other direction still stood, and the pair is left with no direction. The pair delete arm lets either party remove a pair once neither direction exists, and whichever retry commits last runs its cleanup after both directions are gone, so the orphan is removed and that caller gets `{ stillHidden: false }`. The earlier caller may still be told `stillHidden: true`; the provider reads the committed pair, so what renders is correct either way. The retry succeeding is how the unblocker learns the block was mutual; reciprocity makes that inherent, and the UI copy (part 3) says so. Both capture the acted Event once, as `setHeart` does, so a hostname change cannot move the write.
