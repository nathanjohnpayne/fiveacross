...
The `approvePrompts` callable (#1275, `functions/src/approvePrompts.ts`; the Review queue reaches it through `approveItems`, now a thin wrapper) reads the authoritative pending row in the same server-side Admin-SDK transaction that reads the Event schedule and writes the approval, and it carries this classification and the malformed-row isolation below unchanged. For a still-pending row it validates the caller's requested difficulty as `main` or `easy`, persists it through the existing pool vocabulary seam (`main` or `embark`), writes the caller's exact boolean spicy choice for Exploratory (or the authoritative stored value for an older caller that omitted it), forces `spicy: false` for Easy, and stamps status, routing, and approval metadata atomically. A stale/non-pending or missing row remains a no-op exactly as in `community-prompt-targeting`; no classification-only write escapes the approval guard. A requested classification the guards refuse—a pool outside easy/exploratory, a non-boolean spicy—is a no-op for that row too: it is reported as the row's own `malformed` outcome with a short reason, the row stays `pending`, and the rest of the batch still approves, so one bad row in an "Approve all" can no longer fail every other valid row in the same transaction (#1070). This caller-supplied classification is the one deliberate exception to the otherwise authoritative stored-row rule: it is an Admin decision made at approval, not stale routing state. Closing is never offered or accepted for Community Prompts.

The queue's `setItemSpicy` write is transactional too: it updates only an authoritative row that is still `pending` and still normalizes to `main`. Each successful correction increments the row's optional `spicyRevision` in that same transaction and returns the committed revision. An absent revision reads as zero silently: the field is optional and only this transaction ever writes it, so absence is the ordinary opening state of a legacy row and of every Prompt's first correction. A stored revision outside the contract (present but non-numeric, fractional, negative, or past the safe-integer range) reads as zero too, but that restart is logged with the item id and the shape of the offending value rather than absorbed silently, because it re-bases the acknowledgement the overlay retires on and nothing this app writes can produce it. The checkbox stays optimistic only until its subscription reaches that revision or a newer one. This is an acknowledgement boundary rather than a second classification source: it handles both listener orderings (echo after Promise settlement, or this write's echo followed by another Admin's correction before settlement) without masking the later correction. If a spicy toggle races approval, Firestore retries the loser; once approval wins, the toggle sees an active row and becomes a no-op. The approval itself carries the same exact UI choice, so this safe no-op loses neither a tick nor an un-tick. `firestore.rules` independently enforce the resulting-state invariant on every item create and every Admin update that changes `pool` or `spicy`, including old clients: `main` (and a legacy row with no pool, which reads as main) may be spicy or tame, while `easy`/`embark`/`closing`/`farewell` must be `spicy: false`. Unrelated Admin edits and the existing report-only arm stay compatible with legacy rows whose pool is absent.
...
- A row whose requested classification approval cannot act on is skipped as its own `malformed` outcome with nothing written for it, while every other row in the same bulk approve is still approved under the one shared `approvedAt`; the Review queue names the skipped row rather than leaving it to look untouched.
...
- `tests/functions/approve-prompts.test.ts`: single/bulk approval through the `approvePrompts` callable (#1275) writes the selected normalized difficulty atomically with the status/routing transaction and leaves stale/missing rows untouched.
- `src/data/community-prompt-targeting.test.ts`: the `approveItems` wrapper sends each row's `pool`/`spicy` and narrows the callable's response; pending-main spicy correction increments and returns its acknowledgement revision in the same transaction.
- `src/components/Admin.test.tsx` and `src/components/admin-async-feedback.test.tsx`: the Review queue exposes Easy/Exploratory classification, supplies the selected value to single and bulk approval, retires its optimistic spicy choice at the committed-or-newer revision under both listener/Promise orderings, and names a row a successful approve skipped as `malformed` in a `role="status"` notice that the next clean approve retires.
