...
- `src/components/Admin.test.tsx` and `src/components/admin-async-feedback.test.tsx`: the Review queue exposes Easy/Exploratory classification, supplies the selected value to single and bulk approval, and retires its optimistic spicy choice at the committed-or-newer revision under both listener/Promise orderings.
