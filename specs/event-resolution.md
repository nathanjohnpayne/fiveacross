...
- **An Event is not mounted on an origin whose auth stack is unconfigured.** `isAuthConfiguredForHost` (`src/auth-domain.ts`) asks whether `resolveAuthDomain` points back at this host — true for a registered first-party host, and for a build that pins `authDomain` to its own host (ADR 0010's same-origin escape hatch, which a single-Edition build uses). Anywhere else the OAuth helper is cross-origin, which Safari's storage partitioning breaks on mobile and which needs a console-only `/__/auth/handler` registration. Those origins render the `auth-unconfigured` state instead of a sign-in button that cannot return. Hostname resolution is what created this gap: before it, one build served one hostname and running implied signing-in worked.
...
- **Given** an Event that resolves on an origin whose `authDomain` is a different host, **when** the app boots, **then** it renders the `auth-unconfigured` state rather than mounting sign-in.
...
- `src/auth-domain.test.ts` — the auth-readiness predicate: first-party hosts, the same-origin pin, an unconfigured wildcard host, and agreement with `resolveAuthDomain` by construction.
