...
- `scripts/event-invitations-invoker.test.mjs`, `scripts/event-invitations-deploy.test.mjs`, and `tests/test_deploy.sh` — exact/whole/unknown Functions scopes preflight and reconcile the selected Invitation services, strict per Functions codebase (#1282): a service the resolved codebase's index exports must exist after publish, while one it does not export, or a codebase that cannot be inventoried, is reconciled with absence allowed (see `docs/app/deploy-targets.md`).
