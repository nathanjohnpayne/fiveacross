...
- `src/pendingEventInvitation.test.ts` and `src/handoffBoot.test.ts` — strict fragment capture, dual-store persistence, compare-delete, removal-before-app, telemetry suppression, and a newer cross-tab capture superseding this tab's memory copy on read (two module instances over one jsdom localStorage).
