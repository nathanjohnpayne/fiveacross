...
`src/auth/admissionCoordinator.ts` owns steps 3 and 4 of that order as a pure state machine with every seam injected — the origin's pending record, the compare-delete, the redeem callable and the clock — so the decision table is proved with fakes and `AuthContext` only wires it. A **visit** is one `(eventId, uid, origin)`. `classify(visit)` runs the moment an account is known for an Event — synchronously, before authority and before connectivity — and answers `clear` (no usable record) or `held` (a usable record whose redemption cannot start yet), so the first render already carries the gate and a cached render permission never releases Board, Nav or their subscriptions to a visit whose Invitation was never checked; `begin(visit)` then reads the record again once the visit is signed in, online and authoritative and, when one exists, redeems it for the visit's Event. The deal gate admits only `clear` and `admitted` (`membership-created` or `already-member`); `held`, `pending`, `retryable` and `blocked` each hold `joinAndDeal` at zero calls. Success and terminal invalidity (`invitation-unavailable`, the callable's deliberate collapse of unknown, consumed, expired, revoked, cross-Event and concurrent-loser) compare-delete exactly the record that was redeemed, never a newer capture; every other failure, and a seam that rejects, keeps the record and offers Retry, which re-reads the origin so a newer capture supersedes the failed one and an expired one clears. Retry is the deal's own Retry: it re-establishes connectivity and authority first (the cache-first bootstrap offline, the authoritative bootstrap when the fresh read has not settled) and restarts the redemption only under settled authority, so a reconnect can never consume an Invitation ahead of the attestation the fresh read may require. The Event-switch reset retires `admitted` to `clear` and re-classifies for the new Event; the deal effect deals once per (visit, admission answer, gate inputs), so that transition deals for the new Event exactly once. The published answer carries the Event it was classified for, and a consumer rendering a different Event reads a provisional `held`/`clear` computed from the origin's pending record rather than the previous Event's verdict, because React flushes a committed tree's subscriptions before a layout effect's reclassification can land. A visit whose bootstrap failed while its Invitation is held keeps the deal's Retry surface (Event content stays withheld), so a bootstrap error is never hidden behind the loading state. Three more consequences of the gate being the WHOLE shell: the provider-level Event watchers (`ConfirmWinMoments`, `RetractWinMoments`, `PoolRecoveryWatcher`) mount only under `clear` or `admitted`, because they open board, Event, leaderboard and claim subscriptions the moment they mount and a held visit is not a member; a `retryable` or `blocked` answer clears the deal spinner a bootstrap retry left armed, so the Retry surface is never rendered disabled; and a pending record that only memory holds (`durable: false`, both browser stores refused) keeps the document alive by signing in through the popup rather than the top-level redirect that would destroy the only copy — a popup that fails surfaces as a sign-in error with the record still in memory (SignIn's static failure copy, #1134: the Firebase error never reaches the DOM). The production wildcard hosts sign in through the central-auth handoff instead, which is a top-level navigation with no popup alternative, so that tap first asks the capture seam to give a memory-only record a stored copy (`persistPendingEventInvitation`, a fresh immutable key under which the memory copy is re-identified so a later compare-delete names one record) and, if the stores still refuse, stays on the document and says so rather than destroying the only bearer. The automatic signed-out hop from the `web.app` alias to the canonical Firebase origin fires from the auth callback before any sign-in surface renders, after the fragment has already been captured and cleared; it carries a pending Invitation to the destination in the fragment (the one form that survives a cross-origin replacement and never reaches a server) and retires the alias origin's copy, because the destination captures its own before its app boots. A successful result naming another Event admits nobody. Each `begin`, `retry` and `reset` advances a generation, and a result whose generation is stale — another Event, another account, the same visit begun again, or a reset — is dropped without deleting or admitting. Public state carries the capture's opaque id only: the bearer reaches the callable body and nothing else, so React state, the DOM, error text and analytics cannot observe it.
...
- `scripts/event-invitations-invoker.test.mjs`, `scripts/event-invitations-deploy.test.mjs`, and `tests/test_deploy.sh` — exact/whole/unknown Functions scopes preflight and reconcile the selected Invitation services, strict per Functions codebase (#1282): a service the resolved codebase's index exports must exist after publish, while a scope whose codebase does not export it, or cannot be inventoried, reconciles with that service allowed to be absent (see `docs/app/deploy-targets.md`).
...
- `src/auth/admission-wiring.test.tsx` — the coordinator inside `AuthContext`: a visit with no Invitation dealing as before, a pending redemption holding `joinAndDeal` at zero calls until it succeeds (then one deal and the record forgotten), a terminal refusal never dealing and surfacing the one message, a transient failure offering Retry that deals on success, a deal Retry routed into the admission retry rather than past it, the visit classified `held` the moment the account is known while the bootstrap is still unsettled, Retry after a reconnect not restarting redemption until the fresh authority read settles, an admitted visit switching Events dealing exactly once for the new Event, the spinner cleared when admission settles retryable after a bootstrap retry, the provider-level Event watchers unmounted until the visit is admitted (and never for a blocked one), a memory-only Invitation signing in through the popup while a durable one still redirects, a popup that fails rejecting the sign-in with the record still in memory and nothing redeemed or redirected (#1134), the watchers withheld on the very first render of a new Event that holds an Invitation, the automatic web.app handoff carrying a pending Invitation in the fragment (and retiring the alias copy) or handing off bare when there is none, and an account change retiring a redemption still in flight without deleting or dealing.
