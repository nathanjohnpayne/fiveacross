...
- The owner deleting their own proof object is ALLOWED while the Event is open to play, and—whatever the Event's state—whenever the object is an ORPHAN that no Proof document points at. It is DENIED for media a live Proof document still points at once the Event is closing or archived ([#1149](https://github.com/nathanjohnpayne/fiveacross/issues/1149); rationale and the frozen cases in `specs/post-sailing-archive.md` § "The enforcement" and `tests/rules/post-sailing-archive.test.ts`). This suite seeds no Proof document, so it is the orphan branch that answers its case.
