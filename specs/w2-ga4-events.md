...
- Later tickets appended further catalogued events after the PRD set, each documented at its own call site and in the catalog test's enumeration: `text_size_change` (#215), `reshuffle_card` (#378, `specs/reshuffle.md`), `heart_post` (`specs/feed-hearts.md`), and the operational `mark_rejected` (#387, below). The catalog test in `src/w2-ga4-events.test.tsx` remains the canonical in-order enumeration; this spec's PRD-set bullet pins the original 12+1 prefix.
