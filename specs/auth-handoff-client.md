...
- **The `fiveacross` invoker reconciliation.** While `skipInvokerReconcile: true`, the repo-owned deploy path refuses before build or publish rather than leaving either callable 403ing on that project. Completing #547 requires `run.services.update` for the `fiveacross` deploy service account, a successful real application of `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh` to both callables, and only then a reviewed flip to `false`. Once false, the named target wrapper re-applies that pair before constructing any hosting-only or full build/deploy invocation; a read-only describe is not accepted as readiness proof. Recorded in [auth-handoff](auth-handoff.md).
