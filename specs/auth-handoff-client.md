...
**The 18+ acknowledgement is completed from the same transaction, never inferred from whichever session happens to be current later.** After the bounded custom-token sign-in succeeds and that exact credential is promoted to primary Auth, literal `true` on the redeemed transaction stages a one-shot `{ uid, refreshToken }` acknowledgement in tab-local module memory. `AuthProvider` consumes that marker unconditionally on its first Auth settle: an exact session match enters the provider's existing optimistic/committed `persistAttestation` state machine before profile bootstrap begins, while `null`, a different uid, or the same uid with a different or blank refresh token retires the marker without writing. The refresh token never leaves memory and is never logged. This closes the server-null bootstrap race without letting consent follow a replacement session from another tab. Expired, abandoned, replaced, legacy, and no-checkbox records all fail toward no marker, no write, and the existing settled-profile re-prompt.
...
**A timed-out sign-in is isolated rather than reconciled after it mutates shared auth.** A bound rejects rather than cancels, and Firebase's `signInWithCustomToken` persists the Auth instance it receives before its promise resolves. Running that abandonable request against the app's primary Auth would therefore let a late result overwrite a newer cross-tab session before any continuation could inspect it; calling global `signOut` afterward would destroy the newer session outright. Each handoff instead signs into a uniquely named secondary Firebase app whose Auth uses in-memory persistence. Its app name comes from a naming-only helper, not the security-sensitive transaction-verifier generator. Primary Auth initialization first settles under the same cumulative deadline, before the code is spent. Only a credential that arrives inside that deadline is then copied into the ready primary Auth with `updateCurrentUser`; that shared commit is intentionally not raced, because returning failure while it continued would recreate a late shared mutation. The package-lock-pinned Firebase 12.18.0 / `@firebase/auth` 1.13.5 implementation clones the credential onto primary Auth, queues `directlySetCurrentUser`, assigns `currentUser`, and only then awaits `assertedPersistence.setCurrentUser`; this source-dependent order must be re-audited on SDK upgrades (#1067) and is modeled by the partial-primary-commit regression. If persistence rejects after that assignment, the client treats the handoff as successful only when primary Auth carries the exact uid and refresh token from this attempt; a different same-uid token remains a failure and is never signed out. Package-lock-pinned `@firebase/app` 0.16.1 removes the secondary app's registry name synchronously before awaiting provider deletion, so provider cleanup continues detached without accumulating named app entries, with a fixed-text debug signal on rejection. A late isolated sign-in completion has no path to origin-wide persistence. This preserves a truthful return value without a global cleanup operation, including when another same-origin tab has established a newer session.

**Primary Auth readiness, exchange, and isolated sign-in share one cumulative `HANDOFF_EXCHANGE_TIMEOUT_MS` (15s) pre-commit deadline, and that deadline guards a blank screen rather than a slow one.** The deadline uses a monotonic clock; a step is not started after the budget is exhausted, and each abandonable step rechecks the same deadline when it fulfills so an exact-deadline result or a delayed timer callback cannot reach the shared Auth mutation. `main.tsx` awaits this leg before it renders anything, so abandonable work that never settles would otherwise render *nothing at all*—the 2026-07-24 incident shape the bootstrap path is written to avoid. Captive and shipboard wifi produce exactly that: `navigator.onLine` true and a request that hangs forever. The primary persistence commit remains intentionally awaited rather than raced because it mutates shared Auth and the SDK exposes no safe cancellation boundary. Returning from an outer watchdog would not cancel the queued mutation; it would mount a retry/failure state while the exact session could still appear later. The `primary-commit-not-raced` regression deliberately proves that once this boundary begins, expiration of the pre-commit deadline does not manufacture a result. If the commit rejects after installing the exact session in memory, a fixed-text diagnostic records the degraded persistence. A commit that never settles still holds mount; #1060 explicitly owns replacing that unsafe boundary with a disposable Worker, origin-wide lock, attempt fence, and recovery reload. Secondary-provider cleanup is detached from mount. An adopted acknowledgement write is also fire-and-forget from `AuthProvider`: its synchronous optimistic prefix prevents a fast server-null bootstrap from re-showing the prompt, only a committed write grants authority and permits dealing, rejection emits fixed text and restores the ordinary re-prompt, and a never-settling write remains optimistic but non-authoritative without holding the pre-mount return.
