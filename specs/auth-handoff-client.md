...
Run by `entry.tsx` **before `main.tsx` is imported**, when the captured fragment carries a code. Pre-app because the session must exist by the time the app's `onAuthStateChanged` first settles; importing `main.tsx` first also creates the page's primary Auth queue before the only mutation this return is trying to supervise. The return coordinator is a deep boundary: its public result says only `continue` or `recover`; the Worker protocol, deadline, lock, fence, and Firebase adapters stay behind it.
...
**And the clear is confirmed, not assumed.** A `replaceState` that throws, is refused, or is accepted and silently no-ops leaves a still-live code in `window.location`; proceeding to start analytics anyway would copy it into PostHog and GA4, which is exactly the leak the ordering exists to prevent. `clearHandoffFragment` therefore re-reads the URL and reports whether the code is actually gone, and an unconfirmed clear **fails closed**: analytics are suppressed for that one page load, on both the resolved and bootstrap-failure branches. Sign-in can still complete, and one lost page view on a handoff return is not comparable to exporting a bearer credential.

The return coordinator reads the transaction, confirms the entry origin, recomputes the existing transaction digest, and adds a fresh owner nonce. Before spending the code it requires dedicated Workers, Web Locks, and writable IndexedDB on both the page and Worker sides. The Worker initializes the default Firebase app with explicit IndexedDB Auth persistence and proves that storage with a write/read/delete transaction whose **transaction completion** is awaited; Firebase may otherwise fall back silently to memory. The page registers `{ version, transactionDigest, ownerNonce }` in a dedicated IndexedDB fence under one named exclusive Web Lock. This lock serializes participating **handoff commits** across same-origin contexts; it does not claim to serialize popup or redirect sign-in, sign-out, SDK token refresh, or an older build that does not participate.

Only then does the page call `exchangeAuthHandoff`, through a pre-auth Firebase module that initializes App, Functions, and App Check but never primary Auth. Keeping exchange on the page preserves the DOM-backed reCAPTCHA App Check provider. The custom token exists only in memory and is sent to the dedicated Worker, where a uniquely named, in-memory secondary Auth signs it in. Before any persistent mutation the Worker returns the exact `{ uid, refreshToken }` candidate. The persistent Worker Auth uses the **default** app name and the same API key as the page, because both values are part of Firebase Auth's persistence key.

The page then reacquires the same lock, verifies that the two-part fence still names this attempt, initializes its primary Auth, and starts observing the exact candidate before telling the Worker to commit with `updateCurrentUser`. The Worker never requests this lock. The page holds the lock and keeps the Worker alive until its independent Auth read observes both the candidate uid and refresh token. A Worker success message alone is not proof: the pinned SDK resolves an IndexedDB request before the enclosing transaction's `complete` event, while a later page read cannot observe the row until that write transaction has committed. A commit rejection is therefore still success only when the page independently adopts the exact candidate; a same-uid/different-token session is never claimed.

Only exact page adoption stages the existing one-shot acknowledgement marker. Fence removal and transaction cleanup are compare-and-delete operations, so an older tab cannot erase a newer owner's record. The fixed `localStorage` transaction key remains a latest-fallback-only recovery channel: preserving several concurrent fallback transactions would require an attempt id in the return wire and is outside this client-only repair.
...
**The 18+ acknowledgement is completed from the same transaction, never inferred from whichever session happens to be current later.** Literal `true` on the redeemed transaction stages a one-shot `{ uid, refreshToken }` marker only after the page has independently observed that exact Worker-committed session. `AuthProvider` consumes it unconditionally on its first Auth settle: an exact session match enters the provider's existing optimistic/committed `persistAttestation` state machine before profile bootstrap begins, while `null`, a different uid, or the same uid with a different or blank refresh token retires the marker without writing. The refresh token is never logged or written by the handoff code. Expired, abandoned, replaced, superseded, legacy, and no-checkbox records all fail toward no marker, no write, and the existing settled-profile re-prompt.

**Attestation adoption is detached from the pre-mount authentication return.** Staging the in-memory marker is synchronous and best-effort, so a bridge failure cannot turn an already-successful custom-token sign-in into `sign-in-failed`; it falls back to the ordinary settled-profile re-prompt. The eventual Firestore write belongs to `AuthProvider`, where a rejection rolls back the optimistic lift unless the same uid already has server-confirmed or committed authority. That rejection emits one best-effort fixed-text debug signal without an error object, uid, refresh token, or other transaction data. A write that remains pending cannot hold mount and cannot grant deal authority.

**The whole return uses one cumulative `HANDOFF_EXCHANGE_TIMEOUT_MS` (15s) deadline.** Capability proof, Worker readiness, fence registration, page Auth readiness, exchange, isolated sign-in, lock acquisition, Worker commit, and exact page adoption all spend the same monotonic budget. Work is not started after exhaustion, and exact-deadline fulfillment is rejected before it can cross the persistent mutation boundary.

Before commit begins, a timeout or rejection is a known safe failure: terminate the Worker, compare-delete this attempt's fence and verifier, record the ordinary named failure, and continue into `main.tsx` signed out. Once commit is signalled, a hang, malformed or late message, rejection without exact page adoption, reported Worker success that the page never observes, supersession, unload, or any otherwise uncertain outcome is **recovery-only**. The page terminates the Worker inside the lock callback before releasing the lock, ignores every later message, never calls global `signOut`, never imports `main.tsx`, and renders a dependency-free alert whose only action reloads into a fresh Auth queue. This makes an unknown outcome visible without mounting a document whose Auth state may still be changing. An adopted acknowledgement write remains fire-and-forget from `AuthProvider`: its optimistic prefix prevents a fast server-null bootstrap from re-showing the prompt, only a committed write grants authority and permits dealing, rejection restores the ordinary re-prompt, and a never-settling write remains optimistic but non-authoritative without holding pre-app return completion.
...
- **Given** a handoff whose Sign in tap showed and collected the 18+ acknowledgement, **when** that exact promoted session is `AuthProvider`'s first settle, **then** the provider consumes the one-shot marker before bootstrap, starts `attestAdult` with that User, and a racing server-null profile cannot re-prompt; only the later commit grants deal authority. (Tests: handoff-ack-collected, exact-session-ack-staged, pending-ack-beats-server-null.)
- **Given** a no-checkbox Event, a legacy record, an expired or abandoned transaction, a timed-out sign-in that lands late, a signed-out first settle, or a different session—including the same uid with a different refresh token—**when** handoff state is read or completed, **then** no stale acknowledgement can be written or adopted later. (Tests: handoff-ack-not-collected, legacy-ack-false, expired-ack-refused, replacement-isolation, late-sign-in-no-attestation, first-settle-consumes-marker.)
- **Given** the exact session is adopted but the attestation write rejects or never settles, **when** the app mounts, **then** rejection emits fixed text and restores the settled-profile re-prompt, while a pending write can lift the prompt optimistically but remains non-authoritative and cannot deal. (Tests: adopted-ack-rejection-reprompts, pending-ack-no-deal.)
- **Given** primary persistence rejects after Firebase has already installed this handoff's exact uid and refresh token in memory, **when** the handoff completes, **then** it reports success instead of presenting a failure beside a live session; a different same-uid refresh token is never claimed as this attempt. (Tests: partial-primary-commit, same-uid-session-mismatch.)
- **Given** primary persistence starts and remains pending beyond the pre-commit deadline, **when** the deadline passes, **then** the client does not return a false failure while the shared mutation can still land; completion remains pending until that commit settles, with the cancellable replacement tracked in #1060. (Test: primary-commit-not-raced.)
...
- **Given** primary Auth initialization that never settles, **when** the handoff completes, **then** it gives up inside the bound before spending the code or starting isolated sign-in. (Test: auth-state-ready-hangs.)
- **Given** Auth readiness, exchange, and isolated sign-in each consume part of the return-leg budget, **when** their cumulative duration reaches the monotonic deadline—including an exact-deadline fulfillment whose timer callback has not run—**then** no later step or primary mutation starts. (Tests: cumulative-precommit-deadline, exhausted-deadline-does-not-start, exact-deadline-sign-in-does-not-commit.)
- **Given** a sign-in that succeeds after the bound gave up, **then** it remains quarantined in the deleted in-memory Auth and is never promoted into the app. (Test: late-sign-in-isolation.)
- **Given** secondary-app provider cleanup hangs or rejects after an attempt, **then** it cannot hold the app mount and rejection emits fixed text only. (Tests: detached-isolated-cleanup, isolated-cleanup-debug.)
...
- **Given** a timed-out attempt whose sign-in lands after a LATER attempt already succeeded in this tab or another same-origin tab, **then** the late result remains isolated and the newer persisted session is left alone. (Tests: late-sign-in-isolation, cross-tab-late-sign-in-isolation.)
...
- `src/auth/handoffExchange.test.ts`—the two callable legs: mint returning the server URL untouched and never sending a uid, and complete's happy path, cumulative monotonic deadline, exact-session acknowledgement staging, and every named failure.
- `src/auth/handoffAttestation.test.ts`—the one-shot tab-local acknowledgement identity: exact uid plus non-empty refresh token, unconditional retirement on the first consume, and failure closed for incomplete or replacement sessions.
- `src/auth/AuthContext.test.tsx`—provider adoption of the exact handoff session before bootstrap, pending/committed/rejected authority behavior, first-settle retirement, and StrictMode replay.
...
- **The `fiveacross` invoker reconciliation.** While `skipInvokerReconcile: true`, the repo-owned deploy path refuses before build or publish rather than leaving either callable 403ing on that project. Completing #547 requires `run.services.update` for `firebase-deployer@fiveacross.iam.gserviceaccount.com`, a successful `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh --prove-update` on both callables, and only then a reviewed flip to `false`. Once false, the named target pins its repository root and readiness into the canonical `deploy.sh` process. Its one Node adapter parses the raw Firebase arguments, applies the pinned firebase-tools grammar plus repository Hosting-config validators, and rejects unknown sites, invalid option-looking values, and named-target project/config overrides; valid site-qualified exclusions feed the same scope state. Every remaining nonmutating pre-build guard then runs before the forced pair under the exact deploy service-account identity and before any Hosting or handoff-callable build/release. Unrelated rules/storage/known-endpoint scopes skip readiness. A read-only describe, already-correct no-op, ambient user, readiness-only ambient impersonation override, other-project service account, or vault authentication/infrastructure failure is not accepted as readiness proof or automatic reconciliation identity; only a proven absent vault item permits the keyless fallback. Recorded in [auth-handoff](auth-handoff.md).
