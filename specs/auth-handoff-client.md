...
**A timed-out sign-in is isolated rather than reconciled after it mutates shared auth.** A bound rejects rather than cancels, and Firebase's `signInWithCustomToken` persists the Auth instance it receives before its promise resolves. Running that abandonable request against the app's primary Auth would therefore let a late result overwrite a newer cross-tab session before any continuation could inspect it; calling global `signOut` afterward would destroy the newer session outright. Each handoff instead signs into a uniquely named secondary Firebase app whose Auth uses in-memory persistence. Primary Auth initialization first settles under the same bound, before the code is spent. Only a credential that arrives inside the sign-in bound is then copied into the ready primary Auth with `updateCurrentUser`; that short local commit is intentionally not raced, because returning failure while it continued would recreate a late shared mutation. Timeout deletes the secondary app, and any late sign-in completion has no path to origin-wide persistence. This preserves the retry screen's signed-out outcome without a global cleanup operation, including when another same-origin tab has since established a valid session.

**Primary Auth readiness and all three network steps are each bounded by `HANDOFF_EXCHANGE_TIMEOUT_MS` (15s), and those bounds guard a blank screen rather than a slow one.** `main.tsx` awaits this leg before it renders anything, so Auth initialization, exchange, sign-in, or attestation that never settles renders *nothing at all*—the 2026-07-24 incident shape the whole bootstrap path is written to avoid. Captive and shipboard wifi produce exactly that: `navigator.onLine` true and a request that hangs forever. Auth-readiness, exchange, or sign-in timeouts cost the player one re-sign-in; an attestation timeout preserves the session and costs one re-prompt.
