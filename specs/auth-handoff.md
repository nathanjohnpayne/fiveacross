...
**Outstanding prerequisite: Five Across skips this reconciliation today.** The handoff lives in the `fiveacross` project, where `skipInvokerReconcile` remains `true`. Because its central handoff origin is active, `scripts/deploy-target.mjs` refuses every named Five Across deploy before build or publish instead of releasing a handoff-enabled client while either callable is 403. **Before the flow carries real sign-ins, the `fiveacross` deploy service account needs `run.services.update` on `fiveacross`, `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh` must successfully apply to both existing callables, and only then may `skipInvokerReconcile` be flipped to `false`.** Once false, the named target wrapper re-applies that real idempotent pair before constructing any hosting-only or full build/deploy invocation; a read-only describe is not accepted as proof of update permission or reachability. The existing post-Functions reconciliation still repairs an annotation reset after a release. Those IAM and live-service actions belong with #547 as human prerequisites: without them, sign-in through the handoff is unavailable on every Event origin no matter what the client does.
