...
**Reconciliation is live on Five Across (#547, completed 2026-09-10).** The handoff lives in the `fiveacross` project, where `skipInvokerReconcile` is now `false`. The prerequisites the flag was guarding are met and recorded on #547: `firebase-deployer@fiveacross.iam.gserviceaccount.com` holds `roles/run.admin` (which carries `run.services.update`), both callables exist on `fiveacross`, and `AUTH_HANDOFF_PROJECT=fiveacross scripts/apply-auth-handoff-deploy-readiness.sh` completed a forced update on `mintauthhandoff` and `exchangeauthhandoff` under that exact identity. Because the central handoff origin is active, every named Five Across deploy whose scope can release Hosting or a handoff callable repeats that exact-SA readiness proof before `BUILD_CMD` or Firebase, aborts on a `PERMISSION_DENIED` rather than releasing a handoff-enabled client while either callable is 403, and runs the post-Functions handoff repair on every deploy; the target refuses `--skip-invoker`. The flow may carry real sign-ins once the first named `npm run deploy:fiveacross` after the flip ships the handoff-mode client.
