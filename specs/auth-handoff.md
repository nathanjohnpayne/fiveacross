...
**Outstanding prerequisite: Five Across skips this reconciliation today.** The handoff lives in the `fiveacross` project, where `skipInvokerReconcile` remains `true`. Because its central handoff origin is active, `scripts/deploy-target.mjs` refuses every named Five Across deploy before build or publish instead of releasing a handoff-enabled client while either callable is 403. **Before the flow carries real sign-ins, `firebase-deployer@fiveacross.iam.gserviceaccount.com` needs `run.services.update` on `fiveacross`, `AUTH_HANDOFF_PROJECT=fiveacross scripts/set-auth-handoff-invoker.sh --prove-update` must force a successful update on both existing callables, and only then may `skipInvokerReconcile` be flipped to `false`.** Once false, the named target pins its repository root and passes a project-pinned readiness marker to `deploy.sh`. Its single canonical Firebase option/scope parser runs the pinned firebase-tools grammar and repository Hosting-config validators before every remaining nonmutating pre-build guard, including Functions parameter coverage, then repeats that real pair before any build or publish when Hosting or either handoff callable may be released. Unknown Hosting sites and other invalid filters fail locally before readiness; a valid site-qualified exclusion removes that Hosting surface from the same readiness decision. Rules/storage and exact known-unrelated Function scopes skip the live update; unfamiliar Function groups/codebases stay conservative. Firebase `--dry-run` publishes no app or Function release and skips the wrapper-owned readiness/post-deploy IAM updates after the same guards, although Firebase may still enable project APIs. The one Node adapter consumes every pinned deploy/global value-taking option before dry-run classification and rejects option-looking values Firebase later rejects (for example, `--only --dry-run`). Named targets also reject every `-P`/`--project` and `-c`/`--config` spelling so trailing Firebase arguments cannot override their pinned destination. `--skip-invoker` is rejected for this handoff-enabled target so a Functions release cannot suppress the repair. The credential seam directly accepts only an exact deploy-service-account key that activates successfully and otherwise pins keyless impersonation to that identity only when the vault lookup proves the project key is absent; an authentication, biometric, or infrastructure lookup failure aborts before gcloud, so a read-only describe, already-correct no-op, unusable key fallback, ambient user, or other-project service account cannot masquerade as proof. Automatic ordinary reconciliation scrubs readiness-only impersonation controls before choosing its credential, preserves its already-correct no-op optimization, and still repairs an annotation reset after a release. Those IAM and live-service actions belong with #547 as human prerequisites: without them, sign-in through the handoff is unavailable on every Event origin no matter what the client does.
