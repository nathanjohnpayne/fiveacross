...
**Outstanding prerequisite: Five Across skips this reconciliation today.** The handoff lives in the `fiveacross` project, where `skipInvokerReconcile` remains `true`. Because its central handoff origin is active, `scripts/deploy-target.mjs` now refuses every named Five Across deploy before build or publish instead of releasing both callables and leaving them 403. **Before the flow carries real sign-ins, the `fiveacross` deploy service account needs `run.services.update` on `fiveacross` and `skipInvokerReconcile` must be flipped to `false`.** The existing pre-publish invoker check then proves both callable policies before Firebase publishes. That IAM action belongs with #547 as a human prerequisite: without it, sign-in through the handoff is unavailable on every Event origin no matter what the client does.
