...
Writes additionally require: own `uid`, `keys().hasOnly` the five contract fields, a known `targetKind`, and `createdAt` in the shared +60s/−24h clock window. Read is signed-in public. Admin delete is the moderation lever (delete, never edit—no admin update exists). Since #689 the same target read also denies a Heart whose author sits across a Player block from the caller (`heartTargetAccepts`; `specs/player-blocking.md`): the one interaction a block is rules-enforced on. A server-authored Moment never names a pair.

## Reads and derivation

ONE flat subscription (`useAllHearts`, src/hooks/useData.ts) feeds the whole Feed, mirroring `useAllDoubts`; per-card state is the pure `heartState(hearts, kind, id, targetCreatedAt, viewerUid, bannedUids)` → `{ count, hearted }`, incarnation-scoped as above. Ban semantics: a banned Player's hearts vanish from everyone else's counts, but their OWN heart stays visible to themselves—the own-content exception, load-bearing so a banned viewer's button reads true and their retap doesn't silently re-assert the same slot forever. The Feed passes the ban roster plus the viewer's Player-block hidden set as `bannedUids` (`withBlockExclusions`, specs/player-blocking.md § Where hiding applies), so a blocked counterpart's hearts leave that viewer's counts too; the viewer is never in their own set, so the own-content exception is unchanged.
