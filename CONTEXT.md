...
**Canonical hostname**: The Event's primary address—in the `fiveacross.app` Namespace going forward (#599 as amended): the host new provisioning targets first and the name we print on things. It is the hostname analytics **report** (one canonical host per Event, so cross-host traffic aggregates), but not the host they **ingest to**: ingestion is Brand-level, so every Edition sends to the one Five Across host. It is NOT the only address an Event serves from—every registered host **in service** serves in place with correct branding via hostname resolution (a registered zone being retired, like `fiveacrossbingo.com`, is not in service and redirects instead)—and share links carry the entry-point host the sharer is standing on (#607), so recipients land on the same-branded surface. _Avoid_: Primary domain, main URL
...
**Block**: A Player's per-Event, reciprocal, self-serviceable hide of another Player: display-only on shared reads, enforced by rules on the block records and on Hearts. Distinct from a Ban (Admin-issued, Event-wide hide of one Player's content) and from Membership revocation (removes admission). Only the blocker can reverse it. Specified in `specs/player-blocking.md`. _Avoid_: mute, ban, ignore

**Invitation**: A server-owned, high-entropy bearer capability that an organizer shares so one User can receive a Membership in one Event. Its code is distinct from the public Event address and Slug, expires and can be revoked, travels only in a URL fragment, and is stored server-side only as a hash. Redemption—not merely reaching the hostname or signing in—is the admission grant. Specified in `specs/event-invitations.md`; its product-policy choices remain proposed until the owner answers them. _Avoid_: Slug, join code, Event link, QR (a QR may carry an Invitation, but is not itself authority)
